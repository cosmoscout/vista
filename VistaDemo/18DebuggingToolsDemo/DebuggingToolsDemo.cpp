...
class DebuggingToolsDemoAppl::PrintColorStreamPairsKeyCallback
  ...
  virtual bool Do() {
    ...
    for (VistaColorOutstream::CONSOLE_COLOR nBackground = VistaColorOutstream::CC_FIRST_COLOR;
      ...
      for (VistaColorOutstream::CONSOLE_COLOR nText = VistaColorOutstream::CC_FIRST_COLOR;
        ...
        oStream << std::setw(13) << VistaColorOutstream::GetConsoleColorName(nText) << "  on "
                << std::setw(13) << VistaColorOutstream::GetConsoleColorName(nBackground)
                << std::endl;
        // increment
...
DebuggingToolsDemoAppl::DebuggingToolsDemoAppl(int argc, char* argv[])
  ...
  VistaColorOutstream* pRedStream =
      new VistaColorOutstream(VistaColorOutstream::CC_YELLOW, VistaColorOutstream::CC_RED, 1);
  ...
  m_pVistaSystem->GetEventManager()->AddEventHandler(
      this, VistaSystemEvent::GetTypeId(), VistaEventManager::NVET_ALL);
  ...
  m_pVistaSystem->GetKeyboardSystemControl()->BindAction(
      's', m_pStackWalkerCallback, "Print StackTrace");

  //////////////////////////////////////////////
  /////  COLOR-, LOG-, SPLITSTREAM USAGE   /////
  //////////////////////////////////////////////
  // For our own output, we first create a new colorstream
  VistaColorOutstream* pGreenStream =
      new VistaColorOutstream(VistaColorOutstream::CC_GREEN, VistaColorOutstream::CC_DEFAULT);
  // we now register the red and green streams with the StreamManager. It gains access to all
  // streams registered with it, independent of their type. Since it is a singleton, we can access
  // it from anywhere - this gives us the option to write to a common output, without having to know
  // the actual stream everywhere. Since we are lazy, we also allow the StreamManager to handle
  // deletion of the stream, so we dont have to care about this anymore.
  ...
  pLogger->SetStreamEventMask(vstr::Stream("log"),
      VistaSystemEvent::VSE_POSTGRAPHICS | VistaSystemEvent::VSE_UPDATE_INTERACTION);
  ...
  m_pVistaSystem->GetKeyboardSystemControl()->BindAction(
      'c', m_pColorCallback, "Print Color Stream Combinations");
}
...
void DebuggingToolsDemoAppl::HandleEvent(VistaEvent* pEvent) {
  ...
  // scope it is currently in - it forms either a new entry, or is grouped together with other
  // calls. Additionally, it is important to only profile code in the MAIN THREAD, since the
  // implementation is not thread-safe, and because it doesn't allow normalizing the data.
  ...
  // In general, it is best to ensure this by using (exactly one)
  // VistaBasicProfiler::ProfileScopeObject per scope that should be profiled.
  switch (pEvent->GetId()) {
