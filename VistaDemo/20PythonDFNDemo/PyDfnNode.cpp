...
PyDfnNode::PyDfnNode(const std::string& path)
    : IVdfnNode() {
  ...
  try {
    ...
    py::handle<> ignored(PyRun_FileEx(fd, path.c_str(), Py_file_input, main_namespace.ptr(),
        main_namespace.ptr(), 1)); // closes fd after executing the script
...
void PyDfnNode::pySetOutPortValue(const std::string& portName, boost::python::object obj) {
  ...
  if (it == _outPorts.end()) {
    std::cerr << "PyDfnNode::pySetOutPortValue(): Port name \"" << portName << "\" not registered!"
              << std::endl;
    exit(-1);
  }
  ...
  if (get_float.check()) {
    ...
    if (!floatPort) {
      std::cerr << "PyDfnNode::pySetOutPortValue(): Type mismatch in setting value for port "
                << portName << std::endl;
      exit(-1);
    }
  ...
  std::cerr << "PyDfnNode::pySetOutPortValue(): Unsupported python value type for port " << portName
            << std::endl;
  exit(-1);
}
...
py::object PyDfnNode::pyGetInPortValue(const std::string& portName) {
  ...
  if (it == _inPorts.end()) {
    std::cerr << "PyDfnNode::pyGetInPortValue(): Port name \"" << portName << "\" not registered!"
              << std::endl;
    exit(-1);
  }
  ...
  std::cerr << "PyDfnNode::pyGetInPortValue(): Unsupported port type for port " << portName
            << std::endl;
  exit(-1);
}
