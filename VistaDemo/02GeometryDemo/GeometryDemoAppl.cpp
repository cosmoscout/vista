...
bool GeometryDemoAppl::Init( int argc, char  *argv[] )
{
	...
	VistaSceneGraph *pSceneGraph =
		m_pVistaSystem->GetGraphicsManager()->GetSceneGraph();
	...
	// Create a plane
	m_pPlane = oGeometryFactory.CreatePlane(
		1.0f, 1.0f, 1, 1, Vista::Z,
		VistaColor::MAROON);
	...
	// Create a disk
	m_pDisk = oGeometryFactory.CreateDisk(
		0.7f, 16, 4, Vista::Z, VistaColor::DARK_TURQUOISE);
	// we set rendering attributes to wireframe to make the topology
	// visible as an example.
	VistaRenderingAttributes raWireFrame(
		VistaRenderingAttributes::SHADING_DEFAULT,
		VistaRenderingAttributes::CULL_DEFAULT,
		VistaRenderingAttributes::STYLE_WIREFRAME);
	...
	// Create a torus
	m_pTorus = oGeometryFactory.CreateTorus(
		0.3f, 0.4f, 30, 20, VistaColor::ROYAL_BLUE);

	// Create an ellipsoid
	m_pEllipsoid = oGeometryFactory.CreateEllipsoid(
		0.3f, 1.2f, 0.2f);

	// Create a sphere
	m_pSphere = oGeometryFactory.CreateSphere(
		1, 300, VistaColor::WHITE );
	// set a nice texture
	...
	VistaTransformNode* pSceneryTrans =
		pSceneGraph->NewTransformNode( pSceneGraph->GetRoot() );
	...
	VistaTransformNode* pPlaneTrans =
		pSceneGraph->NewTransformNode( pSceneryTrans );
	pPlaneTrans->Translate( -3, 1, -1 );

	VistaTransformNode* pBoxTrans =
		pSceneGraph->NewTransformNode( pSceneryTrans );
	pBoxTrans->Translate( -1, 1, -1 );

	VistaTransformNode* pDiskTrans =
		pSceneGraph->NewTransformNode( pSceneryTrans );
	pDiskTrans->Translate( 1, 1, -1 );

	VistaTransformNode* pConeTrans =
		pSceneGraph->NewTransformNode( pSceneryTrans );
	pConeTrans->Translate( 3, 1, -1 );

	VistaTransformNode* pTorusTrans =
		pSceneGraph->NewTransformNode( pSceneryTrans );
	pTorusTrans->Translate( -3, -1, -1 );

	VistaTransformNode* pEllipsoidTrans =
		pSceneGraph->NewTransformNode( pSceneryTrans );
	pEllipsoidTrans->Translate( -1, -1, -1 );

	VistaTransformNode* pSphereTrans =
		pSceneGraph->NewTransformNode( pSceneryTrans );
	pSphereTrans->Translate( 1, -1, -1 );

	VistaTransformNode* pTriangleTrans =
		pSceneGraph->NewTransformNode( pSceneryTrans );
	...
	m_pVistaSystem->GetDisplayManager()
		->GetWindowByName("MAIN_WINDOW")->GetWindowProperties()
		->SetTitle( argv[0] );
