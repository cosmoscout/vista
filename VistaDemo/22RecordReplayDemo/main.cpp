...
public:
	...
	NaviKeyCallback( VistaVirtualPlatform* pFrame,
						NaviMode eMode,
						float nValue)
	: m_eMode( eMode )
	, m_nValue( nValue )
	...
	static bool RegisterDefaultCallbacks( VistaKeyboardSystemControl* pKeyControl,
											...
											const float fMovementSpeed = 0.15f,
											const float fRotationSpeed = 0.1f )
	{
		bool bRet = true;
		bRet &= pKeyControl->BindAction( 'e', new NaviKeyCallback( pPlatform, NaviKeyCallback::NM_FORWARD, fMovementSpeed ),
							"move forward by " + VistaConversion::ToString( fMovementSpeed ) + "m" );
		bRet &= pKeyControl->BindAction( 'd', new NaviKeyCallback( pPlatform, NaviKeyCallback::NM_BACKWARD, fMovementSpeed ),
							"move backward by " + VistaConversion::ToString( fMovementSpeed ) + "m" );
		bRet &= pKeyControl->BindAction( 's', new NaviKeyCallback( pPlatform, NaviKeyCallback::NM_LEFT, fMovementSpeed ),
							"move left by " + VistaConversion::ToString( fMovementSpeed ) + "m" );
		bRet &= pKeyControl->BindAction( 'f', new NaviKeyCallback( pPlatform, NaviKeyCallback::NM_RIGHT, fMovementSpeed ),
							"move right by " + VistaConversion::ToString( fMovementSpeed ) + "m" );
		bRet &= pKeyControl->BindAction( 't', new NaviKeyCallback( pPlatform, NaviKeyCallback::NM_UP, fMovementSpeed ),
							"move left by " + VistaConversion::ToString( fMovementSpeed ) + "m" );
		bRet &= pKeyControl->BindAction( 'g', new NaviKeyCallback( pPlatform, NaviKeyCallback::NM_DOWN, fMovementSpeed ),
							"move right by " + VistaConversion::ToString( fMovementSpeed ) + "m" );
		bRet &= pKeyControl->BindAction( 'w', new NaviKeyCallback( pPlatform, NaviKeyCallback::NM_TURN_LEFT, fRotationSpeed ),
							"rotate left by " + VistaConversion::ToString( Vista::RadToDeg( fRotationSpeed ) ) + "degree" );
		bRet &= pKeyControl->BindAction( 'r', new NaviKeyCallback( pPlatform, NaviKeyCallback::NM_TURN_RIGHT, fRotationSpeed ),
							"rotate right by " + VistaConversion::ToString( Vista::RadToDeg( fRotationSpeed ) ) + "degree" );
		bRet &= pKeyControl->BindAction( 'z', new NaviKeyCallback( pPlatform, NaviKeyCallback::NM_TURN_UP, fRotationSpeed ),
							"rotate left by " + VistaConversion::ToString( Vista::RadToDeg( fRotationSpeed ) ) + "degree" );
		bRet &= pKeyControl->BindAction( 'h', new NaviKeyCallback( pPlatform, NaviKeyCallback::NM_TURN_DOWN, fRotationSpeed ),
							"rotate right by " + VistaConversion::ToString( Vista::RadToDeg( fRotationSpeed ) ) + "degree" );
		return bRet;
	}
