...
On the one hand it'll show you how to use your own particles system the old fashioned way (create
them all yourself), or the better practice way: ParticleManager. This manager will take several
tasks from you (e.g. Creating, managing them). The only things you have to hand over are two objects
that tell the ParticleManager what to do with them on creation and while they are "traveling".
Additionally there's also one example that adds a (more or less exact) physical environment to your
particles. It is possible, that you could use that environment also for other objects. Feel free to
...
class GrindingSparksOriginSet : public VistaOpenSGParticleManager::IParticleChanger {
 ...
 public:
  GrindingSparksOriginSet(VistaVector3D v3ParticleVelocities, float fVelocitiesFactor,
      VistaVector3D v3ParticleOrigin, float fRedGlowFactor, VistaVector3D v3ParticleSize)
      : IParticleChanger()
  ...
  void CalculatePositionAndVelocities(SParticle& oParticle, VistaType::microtime fEventTime) {
    ...
    oParticle.m_a3fVelocity[0] =
        m_v3ParticleVelocities[0] - (float)(m_pRand->GenerateDouble2()) * m_fVelocitiesFactor;
    oParticle.m_a3fVelocity[1] =
        m_v3ParticleVelocities[1] - (float)(m_pRand->GenerateDouble2()) * m_fVelocitiesFactor;
    oParticle.m_a3fVelocity[2] =
        m_v3ParticleVelocities[2] - (float)(m_pRand->GenerateDouble2()) * m_fVelocitiesFactor;
...
class IncValue : public IVistaExplicitCallbackInterface {
 public:
  IncValue(T& tVarRef, const T& tIncValue)
      : m_oRef(tVarRef)
      , m_tVal(tIncValue) {
  }
  virtual ~IncValue() {
...
class GrindingSparks : public VistaEventHandler {
 ...
 private:
  ...
  // For performance issues we rather get some RAM instead of fetching our arrays each time for each
  // particle ...
...
void CreateScene(VistaSystem* pSystem) {
  ...
  // create a skybox
  VistaOpenSGSkybox* pSkybox = new VistaOpenSGSkybox("../../data/top.jpg", "../../data/bottom.jpg",
      "../../data/left.jpg", "../../data/right.jpg", "../../data/front.jpg", "../../data/back.jpg");

  for (std::map<std::string, VistaViewport*>::const_iterator iter =
           pSystem->GetDisplayManager()->GetViewportsConstRef().begin();
       iter != pSystem->GetDisplayManager()->GetViewportsConstRef().end(); ++iter) {
  ...
  // New Way. See RollingBalls.h for further detail. See further below for more detail on this
  // matter.
  VistaTransformNode*         pRollingBallsNode = pSG->NewTransformNode(pRoot);
  VistaOpenSGParticleManager* mRollingBallsManager =
      new VistaOpenSGParticleManager(pSG, pRollingBallsNode);
  ...
  mRollingBallsManager->SetOriginObject(new RollingBallsOriginSet(
      VistaVector3D(10, 0, 10), VistaVector3D(4, 0, 10), VistaVector3D(0.25, 0.25, 0.25)));
  ...
  pSystem->GetEventManager()->AddEventHandler(
      mRollingBallsManager, VistaSystemEvent::GetTypeId(), VistaSystemEvent::VSE_PREGRAPHICS);
  ...
  VistaOpenSGParticleManager* mGrindingSparksManager =
      new VistaOpenSGParticleManager(pSG, pGrindingSparksNode);
  ...
  mGrindingSparksManager->SetOriginObject(
      new GrindingSparksOriginSet(VistaVector3D(0.05f, 0, -1.3f), 0.1f, VistaVector3D(0, 0, -1),
          0.7f, VistaVector3D(0.01f, 0.01f, 0.7f)));
  ...
  pSystem->GetEventManager()->AddEventHandler(
      mGrindingSparksManager, VistaSystemEvent::GetTypeId(), VistaSystemEvent::VSE_PREGRAPHICS);
  ...
  VistaOpenSGParticleManager* mVistaParticleManager =
      new VistaOpenSGParticleManager(pSG, pVistaParticleManagerNode);
  // Set the ViewMode. See the OpenSGParticles for reference for the two supported ViewModes.
  // ATTENTION: This needs to be set BEFORE setting the SpriteImage!
  ...
  // VistaparticleManagerOriginSet and VistaParticleManagerLinearPerFrameChange are inherited from
  // VistaParticleManagerParticle::IParticleChanger. You can create your own! Creating the Object
  // that sets the Particles at their Origin (at the very beginning, as well as after lifetime < 0).
  // See OpenSGParticleManager.h for reference!
  mVistaParticleManager->SetOriginObject(
      new PMOriginSet(VistaVector3D(0, -0.4f, 0), 0.0, VistaVector3D(0, 0, -1.025f), 0.0,
          VistaVector3D(0, 0, 0), 1.0f, 1.0f, VistaVector3D(0.1f, 0.1f, 0.1f), 0.0));
  // Creating the Object that will change Particle related values each frame (or whenever the
  // HandleEvent/UpdateParticles method gets called!).
  mVistaParticleManager->SetChangeObject(new PMLinearFrameChange(0.0, 0.0, 0.0));
  // Defines the maximum Number of Particles. This value is used to determine the Lifetime of the
  // Particles. When calling this function, all Particles will be reset!
  mVistaParticleManager->SetMaximumParticles(12);
  // This function will calculate the Lifetime of the Particles
  // (maxNumberofParticles/ParticlesPerSecond = Lifetime). Attention: This doesn't mean you'll get 10
  // particles every second ... but you'll get ParticlesperSecond * timedelay Particles per timedelay
  // ;) You can change PartcilesPerSecond anytime during the run. However, the new Lifetime will only
  // be available to NEW Particles!
  mVistaParticleManager->SetParticlesPerSecond(1);
  mVistaParticleManager->SetParticlesLifetime(3.7f);
  // This kind of sets the ParticleManager active. To start the particle system,
  // StartParticleManager();!
  pSystem->GetEventManager()->AddEventHandler(
      mVistaParticleManager, VistaSystemEvent::GetTypeId(), VistaSystemEvent::VSE_PREGRAPHICS);
  // You now can still change the Particles, Stop the Manager, Continue or Reload the Manager. You
  // can change number of Particles, too.
  ...
  VistaOpenSGParticleManager* mPMPhysicsManager =
      new VistaOpenSGParticleManager(pSG, pPMPhysicsNode);
  ...
  // This is the Mega-Lemmings-Mode that includes static/dynamic lift, air drag, gravity, thrust,
  // winds
  PMPhysicalEnvironment* PMPhysEnv =
      new PMPhysicalEnvironment(0.5f, 0, 0.18f, 9.81f, 1.23f, VistaVector3D(0, 3.0f, 0),
          VistaVector3D(0.5f, 2.0f, 0.5f), VistaVector3D(0.05f, 0.1f, 0.05f));
  // This is the low power mode that only includes gravity and air drag.
  // PMPhysicalEnvironment *PMPhysEnv = new
  // PMPhysicalEnvironment(0.5f,9.81f,1.23f,VistaVector3D(0.5f,2.0f,0.5f),VistaVector3D(0.05f,0.1f,0.05f));
  PMPhysEnv->SetConstantWind(VistaVector3D(0, 2.8f, 0));
  PMPhysEnv->SetThrust(VistaVector3D(0, 0, 0));

  PMOriginSet* VMPhysOriginSet =
      new PMOriginSet(VistaVector3D(0, 0, 0), 0, VistaVector3D(0, 0, -1.025f), 0,
          VistaVector3D(0, 0, 0), 0, 1.0, VistaVector3D(0.1f, 0.1f, 0.1f), 0.0);
  ...
  pSystem->GetEventManager()->AddEventHandler(
      mPMPhysicsManager, VistaSystemEvent::GetTypeId(), VistaSystemEvent::VSE_PREGRAPHICS);

  pSystem->GetEventManager()->AddEventHandler(
      poLightSaber, VistaSystemEvent::GetTypeId(), VistaSystemEvent::VSE_PREGRAPHICS);
  pSystem->GetEventManager()->AddEventHandler(
      poSmoke, VistaSystemEvent::GetTypeId(), VistaSystemEvent::VSE_PREGRAPHICS);
