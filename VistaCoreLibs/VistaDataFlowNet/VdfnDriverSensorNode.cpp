...
/*============================================================================*/
VdfnDriverSensorNode::VdfnDriverSensorNode( VistaDeviceSensor *pSensor, VistaSensorReadState *readstate )
: IVdfnNode(),
  m_pSensor(pSensor),
...
VdfnDriverSensorNodeCreate::VdfnDriverSensorNodeCreate(VistaDriverMap *pMap, VdfnObjectRegistry *pReg )
    : m_pMap(pMap)
    , m_pReg(pReg)
    {
    }
...
IVdfnNode *VdfnDriverSensorNodeCreate::CreateNode( const VistaPropertyList &oParams ) const
{
	try
	{
		...
		if( pSensor )
		{
			...
			IVistaReadStateSource* pReader = dynamic_cast<IVistaReadStateSource*>(  m_pReg->RetrieveNameable( sReaderTag ) );

			if( !pReader ) 
			{
				if( sReaderTag.empty() == false )
					vstr::out() << "NOTE: Could not retrieve read-state-source for tag [" << sReaderTag << "]. Using default." << std::endl;
					//vstr::warnp() << "Could not retrieve read-state-source for tag [" << sReaderTag << "]" << std::endl;
