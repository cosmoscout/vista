 ...
 public:
  ...
  class VISTADFNAPI IActionGet : public IVistaPropertyGetFunctor {
   public:
     ...
     */
    IActionGet(const std::string& sPropname, const std::string& sClassName,
        const std::string& sDescription = "<none>", unsigned int nMsgTag = ~0)
    ...
    unsigned int                  GetMessageTag() const {
      return m_nMsgTag;
    }
  ...
  class TActionGet : public IActionGet {
   public:
    TActionGet(const std::string& sPropname, const std::string& sClassName,
        const std::string& sDescription = "<none>", unsigned int nMsgTag = ~0)
     ...
     */
    virtual const std::type_info& GetGetterType() const {
      return typeid(T);
    };
  ...
  class VISTADFNAPI IActionSet : public IVistaPropertySetFunctor {
   public:
     ...
     */
    IActionSet(const std::string& sPropname, const std::string& sClassName,
  ...
  class TActionSet : public IActionSet {
   public:
    TActionSet(const std::string& sPropname, const std::string& sClassName,
        const std::string& sDescription = "<none>")
     ...
     */
    virtual const std::type_info& GetSetterType() const {
      return typeid(T);
    }
  };
   ...
   */
  virtual void SetUpdateTimeStamp(
...
class TActionGetFunctor : public IVdfnActionObject::TActionGet<Ret> {
 public:
   ...
   */
  TActionGetFunctor(GetF fct, ConvFunc cFct, const std::string& strPropName,
      const std::string& strClassName, const std::string& strDescriptor)
...
class TActionSetFunctor : public IVdfnActionObject::TActionSet<Ret> {
 public:
  ...
  TActionSetFunctor(SetF fct, ConvFunc cFct, const std::string& strPropName,
      const std::string& strClassName, const std::string& strDescriptor)
      // note on the syntax here:
      // gcc does (correctly) not recognize the base TActionSet<T>
      // without a proper qualification, as it does not know during
...
class TActionSet : public VdfnPortFactory::CPortGetFunctor {
 public:
  virtual bool Set(IVistaReflectionable* pObj, IVistaPropertySetFunctor* pSet, IVdfnPort* pPort,
   ...
   */
  virtual bool Get(IVistaReflectionable* pObj, IVistaPropertyGetFunctor* pGet, IVdfnPort* pPort,
      double nTs, unsigned int nIndex) {
...
// 					<< "pActionSet == " << pActionSet << " ; pT == " << pT <<
// std::endl; 			vstr::erri() << vstr::singleindent
// 					<< (pActionSet ? pActionSet->GetNameForNameable() : "<none>") <<
// std::endl;
// 		}
// #endif
//
