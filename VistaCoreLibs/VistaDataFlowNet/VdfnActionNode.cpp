...
VdfnActionNode::VdfnActionNode(
    VdfnObjectRegistry* pReg, const std::string& strKey, IVdfnActionObject* pActionObject)
    : m_pReg(pReg)
...
bool VdfnActionNode::PrepareEvaluationRun() {
  if (GetIsValid() == false) {
    vstr::warnp() << "[VdfnActionNode]: No valid action object found for name [" << m_strKey << "]"
                  << std::endl;
    return false;
  }
  ...
  for (std::map<std::string, IVdfnPort*>::iterator itInPort = m_mpInPorts.begin();
    ...
    if (itRequest != m_mpBuildMap.end()) {
      m_liPortSets.push_back(
          _sHlp((*itInPort).second, (*itRequest).second.m_pSet, (*itRequest).second.m_pFunctor));
    } else {
...
bool VdfnActionNode::DoEvalNode() {
  ...
  for (std::list<_sHlp>::iterator cit = m_liPortSets.begin(); cit != m_liPortSets.end(); ++cit) {
    ...
    if ((*cit).m_pPort->GetUpdateCounter() != (*cit).m_nRevision) {
      (*cit).m_pSet->Set(
          m_pActionObject, (*cit).m_pFunctor, (*cit).m_pPort, GetUpdateTimeStamp(), ~0u);
  ...
  {
    for (std::list<_sOutHlp>::iterator lit = m_mpOutMap.begin(); lit != m_mpOutMap.end(); ++lit) {
...
bool VdfnActionNode::UpdatePorts() {
  ...
  for (std::list<std::string>::const_iterator cit = liSyms.begin(); cit != liSyms.end(); ++cit) {
    ...
    if (pSet) {
      ...
      if (pFunc) {
        ...
        m_mpBuildMap[*cit] =
            _sHlp(NULL, pFunc->m_pGetFunctor, pSet); // for later use in prepare evaluation run
        //				std::cout << "[VdfnActionNode::UpdatePorts()] -- adding set for
        //["
        //				          << *cit << "]" << std::endl;
      } else {
        vstr::warnp() << "[VdfnActionNode::UpdatePorts()] -- not adding [" << *cit
                      << "] with type [" << pSet->GetSetterType().name()
    ...
    if (pGet) {
      ...
      if (pFunc) {
        ...
        if (pPort) {
          ...
          vstr::warnp() << "[VdfnActionNode::UpdatePorts()] -- could not create port for [" << *cit
                        << "]" << std::endl;
        }
      } else {
        vstr::warnp() << "[VdfnActionNode::UpdatePorts()] -- not creating out port for [" << *cit
                      << "] -- no functor access found for type [" << pGet->GetGetterType().name()
                      << "]" << std::endl;
      }
    }
  }
