 ...
 * @return true
 */
VISTADFNAPI bool RegisterNodeCreators(
...
class VdfnThresholdNodeCreate : public VdfnNodeFactory::IVdfnNodeCreator {
 public:
  ...
  virtual IVdfnNode* CreateNode(const VistaPropertyList& oParams) const {
    ...
    return new VdfnThresholdNode<T>(
        static_cast<T>(dThreshold), bUseAbsoluteValue, bSubtractThreshold, iMode);
  }
};
    ...
    try {
      ...
      if (prams.GetValue("value", sValue)) {
        ...
        if (m_pfConvert(sValue, oVal)) {
          ...
          vstr::warnp() << "[VdfnConstantValueNodeCreate]: Default value [" << sValue
                        << "] could not be converted to " << VistaConversion::GetTypeName<T>()
                        << std::endl;
        }
      } else {
  ...
  virtual IVdfnNode* CreateNode(const VistaPropertyList& oParams) const {
    try {
      ...
      if (oSubs.GetValue<int>("num_outports", nNumPorts) == false) {
        vstr::warnp() << "[TVdfnDemultiplexNodeCreate]: no or invalid \"num_outports\" specified"
                      << std::endl;
        return NULL;
      }
...
class TVdfnVariableNodeCreate : public VdfnNodeFactory::IVdfnNodeCreator {
 ...
 public:
  enum NodeType { VN_GETTER, VN_SETTER };
  ...
  virtual IVdfnNode* CreateNode(const VistaPropertyList& oParams) const {
    try {
      ...
      if (oSubs.GetValue<std::string>("variable", sVariableName) == false) {
        vstr::warnp() << "[TVdfnVariableNodeCreate]: no parameter \"variable\" specified"
                      << std::endl;
        return NULL;
      }
...
template <typename T>
TVdfnTypedVariableRegistry<T>* TVdfnVariableNodeCreate<T>::m_pRegistry =
    new TVdfnTypedVariableRegistry<T>;
