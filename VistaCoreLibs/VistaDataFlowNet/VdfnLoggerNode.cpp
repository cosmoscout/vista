...
/*============================================================================*/
VdfnLoggerNode::VdfnLoggerNode(const std::string& strPrefix, bool bWriteHeader,
    bool bWriteTimeStamp, bool bWriteTimeDiff, bool bLogToConsole,
    const std::list<std::string>& liPorts, const std::list<std::string>& liTrigger,
...
bool VdfnLoggerNode::SetInPort(const std::string& sName, IVdfnPort* pPort) {
...
void VdfnLoggerNode::PrepareFile() {
  ...
  if (oFile.Exists() == false && oFile.CreateWithParentDirectories() == false) {
    vstr::warnp() << "[VdfnLoggerNode]: Couldnot create file [" << m_strPrefix + m_strFileName
                  << "]" << std::endl;
    return;
  }
...
bool VdfnLoggerNode::PrepareEvaluationRun() {
  ...
  for (std::list<std::string>::const_iterator cit = m_liTrigger.begin(); cit != m_liTrigger.end();
       ++cit) {
  ...
  for (std::list<std::string>::const_iterator cit = m_liInPorts.begin(); cit != m_liInPorts.end();
       ++cit) {
  ...
  // calc diff in sets
  std::set_difference(Ports.begin(), Ports.end(), SetPorts.begin(), SetPorts.end(),
      std::inserter(DiffPorts, DiffPorts.end()));

  for (std::set<std::pair<std::string, IVdfnPort*>>::const_iterator dit = DiffPorts.begin();
       dit != DiffPorts.end(); ++dit) {
    TVdfnPort<std::string>* pPort = dynamic_cast<TVdfnPort<std::string>*>((*dit).second);
    if (pPort)
      m_vecPorts.push_back(pPort);
    else
      m_vecPorts.push_back(m_pNotSet);
...
unsigned int VdfnLoggerNode::CalcUpdateNeededScore() const {
  if (m_mpRevisions.empty())
    ...
    for (REVMAP::const_iterator cit = m_mpRevisions.begin(); cit != m_mpRevisions.end(); ++cit) {
...
bool VdfnLoggerNode::DoEvalNode() {
  if (m_pFileName != NULL && m_pFileName->GetUpdateCounter() != m_nFileNameCnt) {
    PrepareFile();
    if (m_bWriteHeader)
      WriteHeader();
  ...
  if (m_bWriteTimeStamp) {
    ...
    if (m_bWriteTimeDiff) {
      if (m_nLastUpdate) {
        if (m_ofstream) {
          (*m_ofstream) << m_sSep << VistaConversion::ToString(nLastUpdate - m_nLastUpdate);
        }
        if (m_bLogToConsole) {
          vstr::outi() << m_sSep << VistaConversion::ToString(nLastUpdate - m_nLastUpdate);
        }
      } else {
        if (m_ofstream)
          (*m_ofstream) << m_sSep << "0";
