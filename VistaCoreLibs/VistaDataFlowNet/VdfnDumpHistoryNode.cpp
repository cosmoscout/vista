...
bool VdfnDumpHistoryNode::DoEvalNode() {
  ...
  for (unsigned int bfIdx = 0; bfIdx < pHistory.m_rbHistory.GetBufferSize(); ++bfIdx) {
    m_ofstream << bfIdx << "\t" << (*pHistory.m_rbHistory.index(bfIdx)).m_nMeasureIdx << "\t\t"
               << (*pHistory.m_rbHistory.index(bfIdx)).m_nSwapTime << "\t\t"
               << (*pHistory.m_rbHistory.index(bfIdx)).m_nMeasureTs << std::endl;

    //			m_ofstream << std::endl << "buffer" << std::endl;
    //			m_ofstream.setf(std::ios::hex);
    //			for( int nIdx = 0; nIdx < (*pHistory.m_rbHistory.index(bfIdx)).m_vecMeasures.size();
    //++nIdx )
    //			{
    //
    //				m_ofstream <<
    //(char)(*pHistory.m_rbHistory.index(bfIdx)).m_vecMeasures[nIdx]; 				if( (nIdx % 16 == 0) )
    //					m_ofstream << std::endl;
    //				else
    //					m_ofstream << " ";
    //			}
