...
public:
	...
	, m_pCon( pCon )
	, m_sName( sSlaveName )
	, m_bSwap( pCon->GetByteorderSwapFlag() == VistaSerializingToolset::SWAPS_MULTIBYTE_VALUES ? true : false )
	, m_bSwapSyncs( bDoSync )
	, m_pAckCon( pAckCon )
	, m_nIndex( nIndex )
	{
...
public:
	...
	, m_nCounter(0)
	{
		m_pSerializer->SetByteorderSwapFlag( bDoSwap ? VistaSerializingToolset::SWAPS_MULTIBYTE_VALUES : VistaSerializingToolset::DOES_NOT_SWAP_MULTIBYTE_VALUES );
	}
		...
		try
		{
			...
			if( pEventChannel->GetIsOpen() )
			{
				...
				if( pAckChannel->GetIsOpen() )
				{
					...
					pAckChannel->SetByteorderSwapFlag( VistaSerializingToolset::DOES_NOT_SWAP_MULTIBYTE_VALUES );
	...
	void TransmitClusterSetupInfo( const std::string& m_sMasterName)
	{
			 ...
			 itSlave != m_vecSendList.end(); ++itSlave )
		{
			...
			if( bRefreshSerializer )
			{
				...
				{
					nMessageSize += m_oSerializer.WriteInt32( (VistaType::sint32)(*itSlaveData)->m_sName.size() );
...
, m_pDefaultDataSync( NULL )
{
	...
	vstr::warnp() << "Using outdated VistaClusterMaster -- please switch to the new VistaNewClusterMaster instead, "
