...
/*============================================================================*/
/**
 * VistaMasterDataTunnel is the IVistaDataTunnel implementation for usage on the cluster master
 * system. Basically the upstream is just a direct connection to whatever consumer is connected to
 * its end. Thus the data on the upstream is merely forwarded. The trick of VistaMaterDataTunnel
 * lies in the downstream 'tunnel tube'. This pipeline contains a T junction which is used in order
 * to mirror any downstream data packets to all the clients. The T junction has two outbounds. The
 * first one is a special dispatcher which distributes the inbound data. The second one (mind the
 * order) is the standard application defined consumer that yields the data to the application. By
 * subtyping and redifinition of the 'connect' routines this behaviour can be changed.
 *
 * NOTE: Big data packets that need some time to be transmitted might result in cluster display
 * inconsistencies, since each client will display the incoming data ASAP. We might consider
 * developing some kind of data sync here.
 *
 * @todo We might try a CloneTeeFilter instead of the currently use SerialTeeFilter. This might
 * allow faster data processing on the master at the cost of additional memory overhead.
 *
 * @todo I recently added an activator to actively push data through the T -> check if this should
 * be optional (new flag for the constructor)
 *
 *
 */
...
class VISTAKERNELAPI VistaMasterDataTunnel : public IVistaDataTunnel {
 public:
  /**
   * Construct a data tunnel comprising the clients given in vecHostNames. It is assumed, that each
   * client is listening on the port given in vecClientPorts
   */
  VistaMasterDataTunnel(
 ...
 private:
  ...
  /*
   *	It might be useful to make the dispatcher an active component with its own thread. Then we
   *would need an additional DLVistaActiveDataProducer and a buffering component. This might
   *alleviate timing problems, when distributing large amounts of data.
   */
};
/*============================================================================*/
