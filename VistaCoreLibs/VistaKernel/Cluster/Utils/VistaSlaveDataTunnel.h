...
/**
 * VistaSlaveDataTunnel is a IVistaDataTunnel for usage on a cluster client machine. It implements
 * the following behavior: Any packets given to the upstream will be deliberately lost. It is
 * assumed, that only the cluster master handles upstream requests. In contrast, the downstream is
 * mirrored from the master. For this purpose VistaSlaveDataTunnel interfaces with its master
 * counterpart, the VistaMasterDataTunnel and receives serialized packet data via a dedicated TCP/IP
 * connection. This data is then passed downstream where it is eventually made available to the
 * application.
 *
 * @todo I recently added an activator to actively read data from the net -> check if this should be
 * optional (new flag for the constructor)
 *
 */
class VISTAKERNELAPI VistaSlaveDataTunnel : public IVistaDataTunnel {
 public:
   ...
   */
  VistaSlaveDataTunnel(
      const std::string& strHostName, int iPortNum, int iQueueSize, IDLVistaDataPacket* pProto);
  /**
   ...
   * The prototype data packet pProto will be used to deserialize incoming data packets.
   */
  VistaSlaveDataTunnel(VistaConnectionIP* pConnection, int iQueueSize, IDLVistaDataPacket* pProto);
 ...
 private:
  ...
  class DeadProd;
  /**
   * the collector is the data source for the downstream pipe i.e. it receives downstream data
   * packets from the cluster-master.
   */
  ...
  /**
   * these are simple dead end consumers for the up/downsstream pipes. They basically do nothing but
   * recycle incoming packets. They are used to terminate the pipes correctly in order to prevent
   * any crashes. NOTE that it's generally a bad idea to do anything active on the backend of a
   * VistaDataTunnel (i.e. pull data from the upstream or push data to the downstream
   */
