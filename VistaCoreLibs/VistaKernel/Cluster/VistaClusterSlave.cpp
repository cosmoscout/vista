...
VistaClusterSlave::VistaClusterSlave(VistaEventManager* pEventManager,
    const std::string& sSlaveName, VistaInteractionManager* pInteractionMngr)
  ...
  vstr::warnp() << "Using outdated VistaClusterSlave -- please switch to the new "
                   "VistaNewClusterSlave instead, "
...
bool VistaClusterSlave::Init(const std::string& sClusterSection, const VistaPropertyList& oConfig) {
  vstr::outi() << "[VistaClusterSlave]: Initializing" << std::endl;
  {
    ...
    if (oSection.GetValue("SLAVEIP", m_sSlaveHostName) == false || m_sSlaveHostName.empty()) {
    ...
    m_bDoSwap    = oSection.GetValueOrDefault<bool>("BYTESWAP",
        m_pMessageDeSer->GetByteorderSwapFlag() == VistaSerializingToolset::SWAPS_MULTIBYTE_VALUES
            ? true
            : false);
    ...
    if (bDoSync) {
      ...
      vstr::outi() << "Slave is told to do a sync on IP [" << strSyncIp << "] - Port [" << nSyncPort
                   << "]" << std::endl;
    ...
    if (WaitForClusterMaster(m_sSlaveHostName, nPort, nAckPort) == false) {
      vstr::errp() << "WaitForClusterMaster() failed on IP [" << m_sSlaveHostName << "] - Port ["
                   << nPort << "]" << std::endl;
      return false;
    }
...
bool VistaClusterSlave::CreateNamedConnections(
  ...
  vecConnections.push_back(
      std::pair<VistaConnectionIP*, std::string>(pConnection, m_oMasterInfo.m_sNodeName));
...
void VistaClusterSlave::Debug(std::ostream& oStream) const {
  ...
  oStream << "Host      : [" << (m_pConnection ? m_pConnection->GetPeerName() : "<->") << "]"
          << std::endl;
  oStream << "Port      : [" << (m_pConnection ? m_pConnection->GetPeerPort() : -1) << "]"
          << std::endl;
...
bool VistaClusterSlave::WaitForClusterMaster(const std::string& strIp, int nPort, int nAckPort) {
  try {
    vstr::outi() << "Waiting for master on [" << strIp << ":" << nPort << "]..." << std::endl;
    ...
    if (oServer.GetIsValid()) {
      ...
      m_pConnection->SetByteorderSwapFlag(
          m_bDoSwap == true ? VistaSerializingToolset::SWAPS_MULTIBYTE_VALUES
      ...
      m_pMessageDeSer->SetByteorderSwapFlag(
          m_bDoSwap == true ? VistaSerializingToolset::SWAPS_MULTIBYTE_VALUES
                            : VistaSerializingToolset::DOES_NOT_SWAP_MULTIBYTE_VALUES);
    } else {
    ...
    vstr::outi() << "Creating ACK connection on [" << strIp << " ; " << nAckPort << "]"
                 << std::endl;
    ...
    if (m_pAckConnection->GetIsOpen()) {
      ...
      m_pAckConnection->SetByteorderSwapFlag(
          m_bDoSwap == true ? VistaSerializingToolset::SWAPS_MULTIBYTE_VALUES
                            : VistaSerializingToolset::DOES_NOT_SWAP_MULTIBYTE_VALUES);
    } else {
...
void VistaClusterSlave::SwapSync() {
  ...
  if (m_pSyncSocket) {
    ...
    if (m_pSyncSocket->ReceiveRaw(&iDummy, sizeof(int), m_nSyncTimeout) != sizeof(int)) {
      vstr::warnp() << "[ViClSlave]: wait for swap sync failed - timeout of " << m_nSyncTimeout
                    << "ms exceeded" << std::endl;
    }
    if (iDummy != m_iFrameCount) {
      vstr::errp() << "[ViClSlave]: wait for swap sync returned the wrong "
                   << "token, something is out of sync.\n"
                   << vstr::indent << "expected [" << m_iFrameCount << "], but got: [" << iDummy
                   << "]"
                   << " pending: " << m_pSyncSocket->HasBytesPending() << std::endl;
    }
...
VistaConnectionIP* VistaClusterSlave::CreateConnectionToMaster() {
  ...
  try {
    ...
    vstr::outi() << "VistaClusterSlave::CreateConnectionToMaster() -- "
                 << "Waiting for connection from master on IP [" << m_sSlaveHostName << "] - Port ["
                 << iPort << "]" << std::endl;
    ...
    if (oServer.GetIsValid() == false) {
      vstr::errp() << "VistaClusterSlave::CreateConnectionToMaster() -- "
                   << "Could not create Main TCP socket on IP [" << m_sSlaveHostName << "] - Port ["
                   << iPort << "]" << std::endl;
    ...
    if (pSocket == NULL) {
      vstr::errp() << "VistaClusterSlave::CreateConnectionToMaster() -- "
                   << "No connection from master on IP [" << m_sSlaveHostName << "] - Port ["
                   << iPort << "]" << std::endl;
      return NULL;
    }
