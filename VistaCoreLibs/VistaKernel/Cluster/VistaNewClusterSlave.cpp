...
/*============================================================================*/
VistaNewClusterSlave::VistaNewClusterSlave(VistaSystem* pVistaSystem, const std::string& sSlaveName)
...
VistaNewClusterSlave::~VistaNewClusterSlave() {
  ...
  vstr::outi() << "VistaNewClusterSlave::~VistaNewClusterSlave() -- closing connection"
               << std::endl;
  if (m_pConnection)
...
bool VistaNewClusterSlave::Init(
  ...
  if (oConfig.HasSubList(sClusterSection) == false) {
    vstr::errp() << "Cluster Slave cannot be initialized - section [" << sClusterSection
  ...
  vstr::outi() << "Slave is told " << (m_bDoOglFinish ? "" : "not ")
               << "to do glFinish() before swap" << std::endl;

  if (WaitForClusterMaster(m_nServerPort) == false) {
    vstr::errp() << "WaitForClusterMaster() failed on IP [" << m_sOwnIP << ":" << m_nServerPort
                 << "]" << std::endl;
    return false;
  }
...
bool VistaNewClusterSlave::ParseParameters(const VistaPropertyList& oSection) {
  if (oSection.GetValue("SLAVEIP", m_sOwnIP) == false || m_sOwnIP.empty()) {
...
void VistaNewClusterSlave::ParseSyncForward(
    const VistaPropertyList& oSection, const std::string& sName, ForwardMode& eForwardVariable) {
...
bool VistaNewClusterSlave::StartFrame() {
  ...
  if (m_oClusterInfo.m_bClusterDebuggingActive) {
    GetDebugStream() << "    StartFrame finished, frameid " << m_iFrameCount << ", frameclock "
                     << m_dFrameClock << std::endl;
  }
  return true;
}
      ...
      if (pSysEvent) {
        ...
        if (m_oClusterInfo.m_bClusterDebuggingActive) {
          GetDebugStream() << "ProcessFrame SystemEventEvent "
                           << VistaSystemEvent::GetIdString(pSysEvent->GetId()) << ", time "
                           << vstr::formattime(pEvent->GetTime(), 6) << std::endl;
        }
        m_pVistaSystem->GetEventManager()->ProcessEvent(pSysEvent);
      } else {
        if (m_oClusterInfo.m_bClusterDebuggingActive) {
          GetDebugStream() << "ProcessFrame Event type " << pEvent->GetType() << ", id "
                           << pEvent->GetId() << ", time " << vstr::formattime(pEvent->GetTime(), 6)
                           << std::endl;
        }
...
void VistaNewClusterSlave::Debug(std::ostream& oStream) const {
  ...
  if (m_pConnection) {
    ...
    oStream << "    Slave Address    : " << sOwnHostname << ":" << oOwnAddress.GetPortNumber()
            << "\n"
...
bool VistaNewClusterSlave::WaitForClusterMaster(int nPort) {
  ...
  try {
    vstr::outi() << "Waiting for master on [" << m_sOwnIP << ":" << nPort << "]..." << std::endl;
    ...
    if (m_bByteSwapSpecified) {
      if (bSwapIndicated != m_bDoByteSwap) {
        vstr::warnp() << "Configuration set ByteSwapping [" << (m_bDoByteSwap ? "ON" : "OFF")
    ...
    m_pConnection->SetByteorderSwapFlag(
        m_bDoByteSwap == true ? VistaSerializingToolset::SWAPS_MULTIBYTE_VALUES
    ...
    while (pSyncSocket == NULL) {
      ...
      if (oSyncServer.GetIsValid() == false) {
        vstr::warnp() << "Could not create sync TCP server on [" << m_sOwnIP << ":" << nSyncPort
                      << "]" << std::endl;
        if (++nFailCount == 10) {
    ...
    m_pSyncConnection->SetByteorderSwapFlag(
        m_bDoByteSwap == true ? VistaSerializingToolset::SWAPS_MULTIBYTE_VALUES
                              : VistaSerializingToolset::DOES_NOT_SWAP_MULTIBYTE_VALUES);
...
VistaConnectionIP* VistaNewClusterSlave::CreateConnectionToMaster() {
  ...
  try {
    ...
    while (pSocket == NULL) {
      ...
      if (oServer.GetIsValid() == false) {
        vstr::warnp() << "VistaClusterSlave::CreateConnectionToMaster() -- "
                      ...
                      << m_sOwnIP << ":" << iPort << "]" << std::endl;
        if (++nFailCount == 10) {
          vstr::errp() << "VistaClusterSlave::CreateConnectionToMaster() -- "
                          "Repeatedly failed to open server - aborting"
                       << std::endl;
      ...
      vstr::outi() << "VistaClusterSlave::CreateConnectionToMaster() -- "
                   << "Waiting for connection from master on IP [" << m_sOwnIP << "] - Port ["
                   << iPort << "]" << std::endl;
      ...
      if (pSocket == NULL) {
        vstr::errp() << "VistaClusterSlave::CreateConnectionToMaster() -- "
                     << "No connection from master on IP [" << m_sOwnIP << "] - Port [" << iPort
                     << "]" << std::endl;
        return NULL;
      }
    }
    ...
    pConnection->SetByteorderSwapFlag(
        m_bDoByteSwap ? VistaSerializingToolset::SWAPS_MULTIBYTE_VALUES
                      : VistaSerializingToolset::DOES_NOT_SWAP_MULTIBYTE_VALUES);
  } catch (VistaExceptionBase& e) {
...
IVistaClusterBarrier* VistaNewClusterSlave::CreateTypedBarrier(
  ...
  if (eMode) {
    ...
    if (eMode == FORWARD_MASTER) {
      VistaInterProcClusterLeaderBarrier* pIPSync =
          new VistaInterProcClusterLeaderBarrier(pBarrier);
...
IVistaClusterBarrier* VistaNewClusterSlave::CreateTCPBarrier(bool bUseDefaultConnection) {
  ...
  if (pConnection == NULL) {
    vstr::warnp() << "[VistaNewClusterSlave::CreateTCPBarrier]:"
                  << "Could not create barrier - connection failed" << std::endl;
  }
  VistaTCPIPClusterFollowerBarrier* pBarrier =
      new VistaTCPIPClusterFollowerBarrier(pConnection, !bUseDefaultConnection);
  ...
  try {
    ...
    if (pSocket->OpenSocket() == false) {
      ...
      if (pSocket->BindToAddress(oAddress) == false) {
        vstr::warnp() << "[VistaNewClusterSlave::CreateBCBarrier]: "
                      << "Could not bind Broadcast-Socket to [" << sIP << ":" << nPort << "]. "
    ...
    pBarrier = new VistaBroadcastClusterFollowerBarrier(
        pSocket, pConnection, true, !bUseDefaultConnection);
  } catch (VistaExceptionBase& e) {
    vstr::warnp() << "[VistaNewClusterSlave::CreateBCBarrier]: "
...
IVistaClusterBarrier* VistaNewClusterSlave::CreateZeroMQBarrier(bool bUseDefaultConnection) {
  ...
  VistaZeroMQClusterFollowerBarrier* pBarrier =
      new VistaZeroMQClusterFollowerBarrier(sAddress, pConnection, true, !bUseDefaultConnection);
...
IVistaClusterBarrier* VistaNewClusterSlave::CreateInterProcBarrier() {
  try {
    ...
    VistaInterProcClusterFollowerBarrier* pBarrier =
        new VistaInterProcClusterFollowerBarrier(m_sInterProcName);
    if (pBarrier->GetIsValid() == false) {
      delete pBarrier;
      vstr::warnp() << "VistaNewClusterMaster::CreateInterProcBarrier() -- "
                    << "creating of barrier with name [" << m_sInterProcName << "failed!"
                    << std::endl;
      return NULL;
    }
    return pBarrier;
...
IVistaClusterDataSync* VistaNewClusterSlave::CreateTypedDataSync(
  ...
  if (m_eForwardDataSync == FORWARD_SLAVE) {
    ...
    assert(bDummy == true);
    std::string sFullName = m_sInterProcName + "_" + VistaConversion::ToString(m_nUsedInterProcs++);
    VistaInterProcClusterFollowerDataSync* pIPSync =
        new VistaInterProcClusterFollowerDataSync(sFullName);
    pIPSync->ConnectPipe();
    pSync = pIPSync;
  } else {
...
//		{
//			vstr::warnp() << "[VistaNewClusterSlave::CreateBCBarrier]: "
//						<< "Master did not report broadcast port - aborting" <<
...
//		{
//			vstr::warnp() << "[VistaNewClusterSlave::CreateBCBarrier]: "
//						<< "Could not open broadcast socket. This slave will report
//it's rediness, "
//						<< "but not wait for the common go signal" <<
//std::endl; 			delete pSocket; 			pSocket = NULL;
//		}
//		else
//		{
//			pSocket->SetIsBlocking( true );
...
//std::endl; 				delete pSocket; 				pSocket = NULL;
//			}
//			pDataSync = new VistaBroadcastClusterFollowerDataSync( pSocket, m_bDoByteSwap,
//true ); 			pDataSync->WaitForConnection( m_pConnection );
//		}
//
//	}
//	catch( VistaExceptionBase& e )
...
IVistaClusterDataSync* VistaNewClusterSlave::CreateInterProcDataSync() {
  ...
  VistaInterProcClusterFollowerDataSync* pSync =
      new VistaInterProcClusterFollowerDataSync(sName, true);
  try {
    // send our string to master (he also checks for empty strings)
