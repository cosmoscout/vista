...
class VISTAKERNELAPI VistaViewport : public VistaDisplayEntity {
 ...
 public:
   ...
   * TEXTURE_SIZE            -   [list of ints - 2 items] size of texture, if it should be different
   *from viewport size PASSIVE_BACKGROUND      -   [bool] TYPE                    -
   *[string][either NORMAL, QUADBUFFERED_STEREO, RENDER_TO_TEXTURE, POSTPROCESS_MONO,
   *POSTPROCESS_STEREO, OCULUS, OPENVR ANAGLYPH, or ANAGLYPH_MONOCHROME] ENABLED                 -
   *[bool] PRIORITY                -   [int] POSTPROCESS_SHADER      -   [string]
  ...
  class VISTAKERNELAPI VistaViewportProperties : public IVistaDisplayEntityProperties {
   public:
    ...
    enum ViewportType {
      ...
      VT_POSTPROCESS, // renders once to a texture, and then executes a shader on the result to
                      // generate new image
      VT_POSTPROCESS_STEREO, // renders twice in stereo to two textures, and then executes a shader
                             // on the result to generate new image
      VT_POSTPROCESS_QUADBUFFERED_STEREO, // renders twice in stereo to two textures, and then
                                          // executes a shader on both of them to generate new image
      ...
      VT_ANAGLYPH_MONOCHROME, // renders twice in stereo, and then calculates a monochrome
                              // anaglyphic image
    };
    ...
    bool         SetRepositionWithWindow(const bool bSet);
   ...
   private:
    VistaViewportProperties(VistaDisplayEntity*, IVistaDisplayBridge*);
 ...
 protected:
  ...
  VistaViewport(VistaDisplaySystem* pDisplaySystem, VistaWindow* pWindow,
