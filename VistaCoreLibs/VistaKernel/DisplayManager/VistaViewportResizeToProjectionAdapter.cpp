...
VistaViewportResizeToProjectionAdapter::VistaViewportResizeToProjectionAdapter( VistaViewport* pViewport )
: m_pViewport( NULL )
...
void VistaViewportResizeToProjectionAdapter::ObserverUpdate( IVistaObserveable* pObserveable, int nMsg, int nTicket )
{
...
std::string VistaViewportResizeToProjectionAdapter::GetNameForProjectionUpdateMode( const ProjectionUpdateMode eMode )
{
	switch( eMode )
	{
...
VistaViewportResizeToProjectionAdapter::ProjectionUpdateMode VistaViewportResizeToProjectionAdapter::GetProjectionUpdateModeFromString( const std::string& sString )
{
	std::string sReducedString = VistaConversion::StringToUpper( sString );
	sReducedString.erase( std::remove( sReducedString.begin(), sReducedString.end(), ' ' ) ); // remove spaces
	sReducedString.erase( std::remove( sReducedString.begin(), sReducedString.end(), '_' ) ); // remove underscores
...
VistaViewportResizeToProjectionAdapter::ProjectionUpdateMode VistaViewportResizeToProjectionAdapter::GetUpdateMode() const
{
...
void VistaViewportResizeToProjectionAdapter::UpdateProjectionAfterViewportChange()
{
	...
	VistaProjection::VistaProjectionProperties* pProjProps = m_pViewport->GetProjection()->GetProjectionProperties();
	double dLeft = 0.0f;
	double dRight = 0.0f;
	double dBottom = 0.0f;
	double dTop = 0.0f;
