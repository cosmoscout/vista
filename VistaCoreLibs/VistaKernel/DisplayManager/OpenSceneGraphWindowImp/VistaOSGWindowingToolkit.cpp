  ...
  while (m_bQuitLoop == false) {
    for (std::map<const VistaWindow*, OSGWindowInfo*>::iterator itWin = m_mapWindowInfo.begin();
      ...
      for (itEvent = (*itWin).second->m_oEvents.begin();
           itEvent != (*itWin).second->m_oEvents.end(); ++itEvent) {
...
bool VistaOSGWindowingToolkit::InitWindow(VistaWindow* pWindow) {
  ...
  if (pInfo->m_bIsDebugContext) {
    vstr::warnp() << "[OSGWindow]: Debug context currently not supported for OpenSceneGraph windows"
                  << std::endl;
  }
...
bool VistaOSGWindowingToolkit::SetWindowUpdateCallback(IVistaExplicitCallbackInterface* pCallback) {
  m_pUpdateCallback = pCallback;
  return true;
}
bool VistaOSGWindowingToolkit::GetWindowPosition(
...
bool VistaOSGWindowingToolkit::SetWindowPosition(VistaWindow* pWindow, const int iX, const int iY) {
  OSGWindowInfo* pInfo = GetWindowInfo(pWindow);
  ...
  if (pInfo->m_pOSGWindow) {
    if (pInfo->m_bFullscreenActive) {
      ...
      pInfo->m_pOSGWindow->setWindowRectangle(pInfo->m_iCurrentPosX, pInfo->m_iCurrentPosY,
...
bool VistaOSGWindowingToolkit::GetWindowSize(
...
bool VistaOSGWindowingToolkit::SetWindowSize(VistaWindow* pWindow, int iWidth, int iHeight) {
  OSGWindowInfo* pInfo = GetWindowInfo(pWindow);
...
bool VistaOSGWindowingToolkit::SetFullscreen(VistaWindow* pWindow, const bool bEnabled) {
  OSGWindowInfo* pInfo = GetWindowInfo(pWindow);
...
bool VistaOSGWindowingToolkit::SetWindowTitle(VistaWindow* pWindow, const std::string& sTitle) {
  OSGWindowInfo* pInfo  = GetWindowInfo(pWindow);
...
bool VistaOSGWindowingToolkit::SetMultiSamples(const VistaWindow* pWindow, const int nNumSamples) {
  OSGWindowInfo* pInfo = GetWindowInfo(pWindow);
...
osgViewer::GraphicsWindow* VistaOSGWindowingToolkit::GetOsgWindowForWindow(
    const VistaWindow* pWindow) {
  WindowInfoMap::const_iterator itWindow = m_mapWindowInfo.find(pWindow);
  if (itWindow == m_mapWindowInfo.end())
...
bool VistaOSGWindowingToolkit::SetUseOffscreenBuffer(VistaWindow* pWindow, const bool bSet) {
  VISTA_FUNCTION_NOT_IMPLEMENTED(
      "::VistaOSGWindowingToolkit::SetUseOffscreenBuffer( pWindow, bSet )");
}

int VistaOSGWindowingToolkit::GetRGBImage(
...
bool VistaOSGWindowingToolkit::GetRGBImage(
...
int VistaOSGWindowingToolkit::GetDepthImage(
    const VistaWindow* pWindow, VistaType::byte* pData, const int nBufferSize) const {
  VISTA_FUNCTION_NOT_IMPLEMENTED("::VistaOSGWindowingToolkit::GetDepthImage()");
  return 0;
}

bool VistaOSGWindowingToolkit::GetDepthImage(
    const VistaWindow* pWindow, std::vector<VistaType::byte>& vecData) const {
  OSGWindowInfo* pInfo = GetWindowInfo(pWindow);
  pInfo->m_pOSGWindow->makeCurrent();
...
bool VistaOSGWindowingToolkit::GetContextVersion(
...
bool VistaOSGWindowingToolkit::SetIsForwardCompatible(
