...
VistaViveGlutWindowingToolkit::VistaViveGlutWindowingToolkit()
  ...
  if (!vr::VR_IsHmdPresent()) {
    vstr::errp()
        << "Error: No Vive HMD present while constructing VistaViveGlutWindowingToolkit!\n";
  }
  ...
  if (eError != vr::VRInitError_None) {
    vstr::errp() << "Error while connecting to Vive API in VistaViveGlutWindowingToolkit ctor: "
                 << vr::VR_GetVRInitErrorAsEnglishDescription(eError) << std::endl;
  }

  if (!vr::VRCompositor()) {
    vstr::errp()
        << "Error: Vive compositor initialization failed in VistaViveGlutWindowingToolkit ctor!\n";
  }
...
bool VistaViveGlutWindowingToolkit::RegisterWindow(VistaWindow* pWindow) {
  ...
  // {
  // 	vstr::warnp() << "[VistaViveGlutWindowingToolkit]: New Vive HMD requested, but windows for
  // all "
  ...
  // {
  // Internal::WindowDataExt& oData = m_pData->m_mapWindowData[pWindow];
  // pWindow->GetWindowProperties()->SetPosition( oData.m_pHmd->WindowsPos.x,
  // oData.m_pHmd->WindowsPos.y ); pWindow->GetWindowProperties()->SetSize(
  // oData.m_pHmd->Resolution.w, oData.m_pHmd->Resolution.h );
...
bool VistaViveGlutWindowingToolkit::InitWindow(VistaWindow* pWindow) {
  ...
  BindWindow(pWindow);
  // std::map< const VistaWindow*, Internal::WindowDataExt >::const_iterator itEntry =
  // m_pData->m_mapWindowData.find( pWindow ); if( itEntry == m_pData->m_mapWindowData.end() )
  // 	return true;
  // @IMGTODO: hacky
  // #ifdef WIN32
  // 		void* pHandle = FindWindow( NULL, pWindow->GetWindowProperties()->GetTitle().c_str()
  // ); #else 		void* pHandle = (void*)( (long)pWindow->GetWindowProperties()->GetWindowId );
  // #endif ovrHmd_AttachToWindow( (*itEntry).second.m_pHmd, pHandle, NULL, NULL );

  // setting capabilities @TODO: how exactly?
  // ovrHmd_SetEnabledCaps( (*itEntry).second.m_pHmd, ovrHmdCap_LowPersistence |
  // ovrHmdCap_DynamicPrediction );
  // ovrHmd_ConfigureTracking( (*itEntry).second.m_pHmd, ovrTrackingCap_Orientation |
...
void VistaViveGlutWindowingToolkit::DisplayWindow(const VistaWindow* pWindow) {
  ...
  if (/*itEntry == m_pData->m_mapWindowData.end()
      ...
      VistaViewport::VistaViewportProperties::VT_VIVE) {
...
// m_pData->m_mapWindowData.find( pWindow ); 	if( itEntry == m_pData->m_mapWindowData.end() )
// return NULL; 	return (*itEntry).second.m_pHmd;
// }

// bool VistaViveGlutWindowingToolkit::FillOculusRenderConfigForWindow( VistaWindow* pWindow,
// ovrGLConfig& oOculusConfig )
// {
// 	std::map< const VistaWindow*, Internal::WindowDataExt >::iterator itEntry =
// m_pData->m_mapWindowData.find( pWindow ); 	if( itEntry == m_pData->m_mapWindowData.end() )
// return false; 	oOculusConfig.OGL.Header.API = ovrRenderAPI_OpenGL;
...
// #ifdef WIN32
// 	oOculusConfig.OGL.Window = FindWindow( NULL,
// pWindow->GetWindowProperties()->GetTitle().c_str() ); 	oOculusConfig.OGL.DC = GetDC(
// oOculusConfig.OGL.Window ); #else 	VISTA_FUNCTION_NOT_IMPLEMENTED(
// "::VistaViveGlutWindowingToolkit::FillOculusRenderConfigForWindow( pWindow, oOculusConfig )" );
// #endif
// 	return true;
// }
