...
public:
	...
	class VISTAKERNELAPI VistaKernelWindowHandle : public VistaDriverAbstractWindowAspect::NativeWindowHandle
	{
	public:
		...
		virtual bool operator< ( const IWindowHandle &other )
		{
			...
			return ( m_pWindow->GetWindowProperties()->GetTitle() < pOther->GetWindow()->GetWindowProperties()->GetTitle() );
		}
		...
		VistaWindow *GetWindow() const { return m_pWindow; }
	...
	virtual int GetRGBImage( const VistaWindow* pWindow, VistaType::byte* pData, const int nBufferSize ) const = 0;
	virtual bool GetRGBImage( const VistaWindow* pWindow, std::vector< VistaType::byte >& vecData ) const = 0;
	virtual int GetDepthImage( const VistaWindow* pWindow, VistaType::byte* pData, const int nBufferSize ) const = 0;
