...
bool VistaOculusGlutWindowingToolkit::RegisterWindow( VistaWindow* pWindow )
{
	...
	{
		vstr::warnp() << "[VistaOculusGlutWindowingToolkit]: New Oculus HMD requested, but windows for all "
...
void VistaOculusGlutWindowingToolkit::DisplayWindow( const VistaWindow* pWindow )
{
	std::map< const VistaWindow*, Internal::WindowDataExt >::const_iterator itEntry = m_pData->m_mapWindowData.find( pWindow );
	if( itEntry == m_pData->m_mapWindowData.end()
		|| pWindow->GetViewport( 0 )->GetViewportProperties()->GetType() != VistaViewport::VistaViewportProperties::VT_OCULUS_RIFT )
	{
...
bool VistaOculusGlutWindowingToolkit::FillOculusRenderConfigForWindow( VistaWindow* pWindow, ovrGLConfig& oOculusConfig )
{	
	std::map< const VistaWindow*, Internal::WindowDataExt >::iterator itEntry = m_pData->m_mapWindowData.find( pWindow );
	if( itEntry == m_pData->m_mapWindowData.end() )
		return false;
...
#else
	VISTA_FUNCTION_NOT_IMPLEMENTED( "::VistaOculusGlutWindowingToolkit::FillOculusRenderConfigForWindow( pWindow, oOculusConfig )" );
#endif
	return true;
}
