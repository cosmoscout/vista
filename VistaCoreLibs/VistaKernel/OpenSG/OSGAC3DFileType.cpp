...
OSGAC3DSceneFileType OSGAC3DSceneFileType::_the(
...
static Prototype ACObject* ac3d_load_ac3d(
    std::istream& file, ACMaterial* pPalette, int& iNumPalette);
...
static ACSurface* read_surface(
    std::istream& file, ACSurface* s, ACObject* ob, char* buff, int& line) {
  char t[20];
...
static ACObject* ac3d_load_object(std::istream& file, ACObject* parent, ACMaterial* pPalette,
    int& iNumPalette, char* buff, int& line) {
  char      t[20];
  ACObject* ob = NULL;
...
static OSG::GeometryPtr createGeometry(
    ACObject* pObj, ACMaterial* pMatTable, std::map<std::string, OSG::ImagePtr>& mpTextures) {

  if (pObj->num_surf == 0 && pObj->num_vert == 0) {
    vstr::warnp() << "[AC3DLoader]: Degenerated object [" << (pObj->name ? pObj->name : "<no-name>")
  ...
  for (int i = 0; i < pObj->num_vert; ++i) {
    coordPtr->addValue(Pnt3f(pObj->vertices[i].x, pObj->vertices[i].y, pObj->vertices[i].z));
    normalPtr->addValue(
        Vec3f(pObj->vertices[i].normal.x, pObj->vertices[i].normal.y, pObj->vertices[i].normal.z));
  }
  endEditCP(coordPtr);
  endEditCP(normalPtr);
  ...
  if (pObj->num_surf == 0) {
    ...
    for (int j = 0; j < pObj->num_surf; ++j) {
      if ((pObj->surfaces[j].flags & AC3D_SURFACE_TYPE_CLOSEDLINE) ==
          AC3D_SURFACE_TYPE_CLOSEDLINE) {
        ...
        lens->addValue(pObj->surfaces[j].num_vertref);
      } else if ((pObj->surfaces[j].flags & AC3D_SURFACE_TYPE_POLYGON) ==
                 AC3D_SURFACE_TYPE_POLYGON) {
      ...
      for (int k = 0; k < pObj->surfaces[j].num_vertref; ++k) {
        texCoordPtr->addValue(Vec2f(
            pObj->surfaces[j].uvs[k].u * float(pObj->texture_repeat_x) + pObj->texture_offset_x,
        ...
        std::map<unsigned int, unsigned int>::const_iterator cit =
            colMap.find(pObj->surfaces[j].mat);
        if (cit == colMap.end()) {
    ...
    beginEditCP(stm);
    {
      ...
      if (cit == mpTextures.end()) {
        ...
        if (texture != NullFC) {
          ...
          vstr::warnp() << "[AC3DLoader]:  Could not read texture [" << pObj->texture << "]"
                        << std::endl;
        }
  ...
  if (matSet.empty()) {
    ...
    if (matSet.size() != 1) {
      vstr::warnp() << "[AC3DLoader]: More than one material in geometry [" << pObj->name
                    << "] -- assigning first material" << std::endl;
    }
  ...
  beginEditCP(geo, Geometry::TypesFieldMask | Geometry::LengthsFieldMask |
                       ...
                       Geometry::NormalsFieldMask | Geometry::TexCoords2FieldMask |
...
static NodePtr createKid(
    ACObject* pParentObj, ACMaterial* pMatTab, std::map<std::string, ImagePtr>& mpTextures) {
  // stop recursion
  if (pParentObj->type == AC3D_OBJECT_NORMAL
      // can happen with some importers: GROUP type obj with no kids but geometry attached
      || ((pParentObj->type == AC3D_OBJECT_GROUP && pParentObj->kids == 0) &&
             (pParentObj->num_surf > 0))) {
    // this is a geometry
    // std::cout << "Encounter normal object [" << (pParentObj->name ? pParentObj->name :
    // std::string("<noname>")) << "]\n";
    GeometryPtr pGeo = createGeometry(pParentObj, pMatTab, mpTextures);
    if (pGeo != NullFC) {
      if (pParentObj->loc.x || pParentObj->loc.y || pParentObj->loc.z ||
          (pParentObj->data && (std::string(pParentObj->data) == "transform"))) {
        NodePtr      gNode = Node::create();
        TransformPtr trans = Transform::create();

        std::string sName = pParentObj->name
                                ? std::string(pParentObj->name) + std::string(" transform")
                                : "kid <transform>";
        setName(gNode, sName);
  ...
  } else if (pParentObj->type == AC3D_OBJECT_GROUP || pParentObj->type == AC3D_OBJECT_WORLD) {

    // std::cout << "Encounter group [" << (pParentObj->name ? pParentObj->name :
    // std::string("<noname>")) << "]\n";
    ...
    if (pParentObj->loc.x || pParentObj->loc.y || pParentObj->loc.z) {
      ...
      setName(gNode,
          (pParentObj->name ? (std::string(pParentObj->name) + std::string(" transform")).c_str()
                            : "<none> transform"));
    } else {
      if (pParentObj->num_kids > 0) {
        ...
        setName(group,
            (pParentObj->name ? (std::string(pParentObj->name) + std::string(" group")).c_str()
                              : "<none> group"));
        beginEditCP(gNode);
        gNode->setCore(group);
        endEditCP(gNode);
 ...
 *public
 ***************************/
NodePtr OSGAC3DSceneFileType::read(std::istream& is, const Char8* fileNameOrExtension) const {
  ...
  if (world) {
    ...
    NodePtr                         gScene = createKid(world, &vecPalette[0], mpTextures);
...
OSGAC3DSceneFileType::OSGAC3DSceneFileType(const Char8* suffixArray[], UInt16 suffixByteCount,
