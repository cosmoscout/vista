...
namespace
{
	...
	bool ViewportSortCompare( const VistaViewport* pLeft, const VistaViewport* pRight )
	{
		...
		return ( pLeft->GetViewportProperties()->GetPriority() >  pRight->GetViewportProperties()->GetPriority() );
	}
	...
	public:
		...
		{
		public:
			ProjectionObserver( OculusViewportData* pData ) : m_pData( pData ) {}
...
void VistaOpenSGDisplayBridge::ViewportData::ReplaceViewport( osg::ViewportPtr pNewPort, bool bCopyData,	 bool bRight )
{
...
void VistaOpenSGDisplayBridge::ViewportData::OnWindowResize(  VistaViewport* pViewport, const int nNewWindowWidth, const int nNewWindowHeight )
{
...
void VistaOpenSGDisplayBridge::ViewportData::SetupRenderTargets( const int nWidth, const int nHeight )
{
	...
	for( int i = 0; i < nCount; ++i )
	{
		...
		if( m_bStencilBufferEnabled )
		{
			...
			glFramebufferRenderbuffer( GL_FRAMEBUFFER, GL_DEPTH_STENCIL_ATTACHMENT, GL_RENDERBUFFER, m_nDepthBufferId[i] ); 
		...
		else
		{
			...
			glFramebufferRenderbuffer( GL_FRAMEBUFFER, GL_DEPTH_ATTACHMENT, GL_RENDERBUFFER, m_nDepthBufferId[i] ); 
			glBindRenderbuffer( GL_RENDERBUFFER, 0 );
		}
		...
		{
			vstr::warnp() << "[VistaOpenSGDisplayBridge]: SetupRenderTargets reports incomplete framebuffer status" << std::endl;
		}
...
void VistaOpenSGDisplayBridge::ViewportData::UpdateRenderTargetSize( const int nWidth, const int nHeight )
{
	...
	for( int i = 0; i < nCount; ++i )
	{
		...
		if( nStatus != GL_FRAMEBUFFER_COMPLETE )
		{
			vstr::warnp() << "[VistaOpenSGDisplayBridge]: UpdateRenderTargetSize reports incomplete framebuffer status" << std::endl;
		}
...
{	
	...
	glFramebufferTexture( GL_FRAMEBUFFER, GL_COLOR_ATTACHMENT0, m_pWriteTextureTarget[nIndex]->GetGLId(), 0 );
	
#ifdef DEBUG
	GLenum nStatus = glCheckFramebufferStatus( GL_FRAMEBUFFER );
	if( nStatus != GL_FRAMEBUFFER_COMPLETE )
	{
		vstr::warnp() << "[VistaOpenSGDisplayBridge]: BindRenderTargets reports incomplete framebuffer status" << std::endl;
	}
#endif
...
void VistaOpenSGDisplayBridge::ViewportData::DrawViewportQuad( const bool bSplit, const bool bSplitIsLeft )
{
	if( bSplit == false )
	{
		glViewport( m_nRTTViewportSizePosX, m_nRTTViewportSizePosY,
					m_nRTTViewportSizeWidth, m_nRTTViewportSizeHeight );
		glScissor( m_nRTTViewportSizePosX, m_nRTTViewportSizePosY,
					m_nRTTViewportSizeWidth, m_nRTTViewportSizeHeight );
	}
	else
	{
		...
		glScissor( m_nRTTViewportSizePosX + nOffset, m_nRTTViewportSizePosY,
					m_nRTTViewportSizeWidth / 2, m_nRTTViewportSizeHeight );
	}
...
void VistaOpenSGDisplayBridge::ProjectionData::SetProjectionPlane()
{
	...
	// debug output
	//vstr::warnp() << "##############################"
	//	<< "\n" << pProjPlaneEdges[0][0] << ", " << pProjPlaneEdges[0][1] << ", " << pProjPlaneEdges[0][2]
	//	<< "\n" << pProjPlaneEdges[1][0] << ", " << pProjPlaneEdges[1][1] << ", " << pProjPlaneEdges[1][2]
	//	<< "\n" << pProjPlaneEdges[2][0] << ", " << pProjPlaneEdges[2][1] << ", " << pProjPlaneEdges[2][2]
	//	<< "\n" << pProjPlaneEdges[3][0] << ", " << pProjPlaneEdges[3][1] << ", " << pProjPlaneEdges[3][2]
	//	<< std::endl;
...
void VistaOpenSGDisplayBridge::ProjectionData::SetEyes( const VistaVector3D & v3LeftOffset, const VistaVector3D & v3RightOffset )
{
...
void VistaOpenSGDisplayBridge::FillRenderInfo( VistaViewport* pViewport, osg::ViewportPtr pOsgViewport) const
{
...
bool VistaOpenSGDisplayBridge::RenderViewport(VistaViewport* pViewport)
{
	...
	switch( pData->GetType() )
	{
		...
		case VistaViewport::VistaViewportProperties::VT_OCULUS_RIFT:
		{
			...
			ovrHmd_GetEyePoses( pOcuData->m_pHmd, 0, a2v3HmdToEyeViewOffset, a2EyeRenderPose, &pOcuData->m_oTrackingState );
			...
			if( m_bOculusOffsetPositionRequiresInitialization )
			{
				...
				//vstr::outi() << "Resetting oculus pos offset to " << v3Offset << "(result: " << v3HeadPosition + v3Offset << " )" << std::endl;
			...
			if( m_bOculusOffsetOrientationRequiresInitialization )
			{
				...
				//qOffset[2] = 0;
				//qOffset.Normalize();
				//vstr::outi() << "Resetting oculus ori offset to " << qOffset << "(result: " << qOffset * qHeadOrientation << " )" << std::endl;
			}
			...
			//assert( pSecondaryViewport );
			//pOcuData->BindRenderTargets( 1 );
			//( pCustomRender ? pCustomRender->DoRender( pData, true ) : pSecondaryViewport->render( m_pRenderAction ) );
			//pOcuData->UnbindRenderTargets( 1 );
			...
			glFinish();
#else
			VISTA_THROW( "Oculus-based rendering requested, but VistaKernel was not build with OculusSDK support", -1 );
#endif			
			break;
		}
		...
		{
#ifdef VISTA_WITH_OCULUSSDK
			const std::map< std::string, VistaViewport* >& mapViewports = GetDisplayManager()->GetViewportsConstRef();
			...
			{
				if( (*itVp).second->GetViewportProperties()->GetType() == VistaViewport::VistaViewportProperties::VT_OCULUS_RIFT )
				{
			...
			{
				vstr::warnp() << "RenderViewport: OculusClone Viewport did not find an original oculus viewport" << std::endl;
				return false;
			}

			OculusViewportData* pOcuData = Vista::assert_cast< OculusViewportData* >( pOculusViewport->GetData() );
...
bool VistaOpenSGDisplayBridge::GetWindowRGBImage( const VistaWindow* pWindow, std::vector< VistaType::byte >& vecData ) const
{
	return m_pWindowingToolkit->GetRGBImage( pWindow, vecData );
}

int VistaOpenSGDisplayBridge::GetWindowRGBImage( const VistaWindow* pWindow, VistaType::byte* pData, const int nBufferSize ) const
{
...
bool VistaOpenSGDisplayBridge::GetWindowDepthImage( const VistaWindow* pWindow, std::vector< VistaType::byte >& vecData ) const
{
	return m_pWindowingToolkit->GetDepthImage( pWindow, vecData );
}

int VistaOpenSGDisplayBridge::GetWindowDepthImage( const VistaWindow* pWindow, VistaType::byte* pData, const int nBufferSize ) const
{
...
void VistaOpenSGDisplayBridge::SetViewportTextureSize( int nWidth, int nHeight, VistaViewport* pTarget )
{
	ViewportData* pData = static_cast<ViewportData*>( pTarget->GetData() );
...
void VistaOpenSGDisplayBridge::GetViewportTextureSize( int& nWidth, int& nHeight, const VistaViewport* pTarget )
{
	ViewportData* pData = static_cast<ViewportData*>( pTarget->GetData() );
...
VistaViewport::VistaViewportProperties::ViewportType VistaOpenSGDisplayBridge::GetViewportType( const VistaViewport* pTarget )
{
	const ViewportData* pData = static_cast<const ViewportData*>( pTarget->GetData() );
	return pData->GetType();
}

bool VistaOpenSGDisplayBridge::SetViewportType( VistaViewport::VistaViewportProperties::ViewportType eType, VistaViewport* pTarget )
{
	ViewportData* pData = static_cast<ViewportData*>( pTarget->GetData() );
	...
	switch( eType )
	{
		...
		case VistaViewport::VistaViewportProperties::VT_QUADBUFFERED_STEREO:
		{
			if( pTarget->GetWindow()->GetWindowProperties()->GetStereo() == false )
			{
				vstr::warnp() << "[VistaViewport::SetType]: Viewport \"" << pTarget->GetNameForNameable()
							<< "\" set to QUADBUFFERED_STEREO, but Window is only MONO - reverting Viewport to MONO" << std::endl;
				pData->m_bStereo = false;
				pData->m_bUseRenderToTexture = false;
			}
			else
		...
		{			
			...
			pData->m_pPostProcessShader->setVertexProgram( VistaOSGViewportShader::GetStandardVertexShader() );
			pData->m_pPostProcessShader->setFragmentProgram( VistaOSGViewportShader::GetNullPostProcessFragmentShader() );
		...
		{	
			...
			{
				pData->m_pPostProcessShader->setFragmentProgram( VistaOSGViewportShader::GetAnaglyphCyanMagentaFragmentShader() );
			}
			else if( eType == VistaViewport::VistaViewportProperties::VT_ANAGLYPH_MONOCHROME )
			{
				pData->m_pPostProcessShader->setFragmentProgram( VistaOSGViewportShader::GetAnaglyphCyanMagentaMonochromeFragmentShader() );
			}
			else
				break;			
			break;
		...
		case VistaViewport::VistaViewportProperties::VT_POSTPROCESS_QUADBUFFERED_STEREO:
		{
			if( pTarget->GetWindow()->GetWindowProperties()->GetStereo() == false )
			{
				vstr::warnp() << "[VistaViewport::SetType]: Viewport \"" 
							<< pTarget->GetNameForNameable()
							<< "\" set to POSTPROCESS_QUADBUFFERED_STEREO, but Window is only MONO - reverting Viewport to POSTPROCESS" << std::endl;
				pData->m_bStereo = false;
			}
			else
			{
	...
	if( eType == VistaViewport::VistaViewportProperties::VT_OCULUS_RIFT )
	{
		...
		VistaOculusGlutWindowingToolkit* pOcuWinTK = dynamic_cast< VistaOculusGlutWindowingToolkit* >( GetWindowingToolkit() );
		if( pOcuWinTK == NULL )
		{
			vstr::errp() << "Trying to create Oculus Viewport without using Oculus Window Toolkit" << std::endl;
			return false;
		}
		pOcuData->m_pHmd = pOcuWinTK->GetHmdForWindow( pTarget->GetWindow() );
		if( pOcuData->m_pHmd == NULL )
		{
			vstr::errp() << "Trying to create Oculus Viewport, but no Hmd connected for corresponding window" << std::endl;
			return false;
		}
		...
		for( int i = 0; i < 2; ++i )
		{
			...
			pOcuData->m_aTextures[i].OGL.Header.RenderViewport.Size.w = pOcuData->m_pHmd->Resolution.w / 2;
		...
		oProjProps.SetValueInSubList( "NAME", "OCULUS_PROJ", pTarget->GetNameForNameable() + "OCULUS_PROJ" );
		VistaProjection* pProj = GetDisplayManager()->CreateProjection( pTarget, "OCULUS_PROJ", oProjProps );
...
VistaGLTexture* VistaOpenSGDisplayBridge::GetViewportTextureTarget( const VistaViewport* pTarget ) const
{
	ViewportData* pData = static_cast<ViewportData*>( pTarget->GetData() );
	return pData->GetMainTextureTarget();
}

VistaGLTexture* VistaOpenSGDisplayBridge::GetViewportSecondaryTextureTarget( const VistaViewport* pTarget ) const
{
	ViewportData* pData = static_cast<ViewportData*>( pTarget->GetData() );
	return pData->GetSecondaryTextureTarget();
}
std::string VistaOpenSGDisplayBridge::GetViewportPostProcessShaderFile( const VistaViewport* pTarget ) const
{
	const ViewportData* pData = static_cast<const ViewportData*>( pTarget->GetData() );
	return pData->m_sShaderFilename;
}

bool VistaOpenSGDisplayBridge::SetViewportPostProcessShaderFile( const std::string& sShader, VistaViewport* pTarget  )
{
	ViewportData* pData = static_cast<ViewportData*>( pTarget->GetData() );
	switch( pData->m_eType )
	{
		...
		default:
		{
			vstr::warnp() << "[Viewport::SetPostProcessShaderFile] only supported for postprocess types" << std::endl;
			return false;
		}
	}
	...
	if( pData->m_pPostProcessShader != osg::NullFC )
	{
		...
		{
			vstr::warnp() << "[VistaViewport::SetViewportPostProcessShaderFile] - Loading fragment shader \""
...
void VistaOpenSGDisplayBridge::SetViewportResizeWithWindowResize( bool bSet, VistaViewport* pTarget )
{
	ViewportData* pData = static_cast<ViewportData*>( pTarget->GetData() );
...
void VistaOpenSGDisplayBridge::SetViewportRepositionWithWindowResize( bool bSet, VistaViewport* pTarget )
{
	ViewportData* pData = static_cast<ViewportData*>( pTarget->GetData() );
...
/*                                                                            */
/*============================================================================*/
VistaProjection * VistaOpenSGDisplayBridge::CreateProjection( VistaViewport * pViewport, const VistaPropertyList & refProps )
{
	if (!pViewport)
	{
	...
	DisplaySystemData * pDS = Vista::assert_cast< DisplaySystemData *>( pViewport->GetDisplaySystem()->GetData() );
	...
	if( pProjData->m_iStereoMode == VistaProjection::VistaProjectionProperties::SM_FULL_STEREO && bVPStereo == false )
	{
		vstr::warnp() << "[VisraProjection::Create] Requests FullStereo Projection, but viewport is mono" << std::endl;
		pProjData->m_iStereoMode = VistaProjection::VistaProjectionProperties::SM_MONO;
	}
	else if( pProjData->m_iStereoMode != VistaProjection::VistaProjectionProperties::SM_FULL_STEREO && bVPStereo == true )
	{
		vstr::warnp() << "[VisraProjection::Create] Requests non-stereo projection, but viewport is stereo" << std::endl;
		pProjData->m_iStereoMode = VistaProjection::VistaProjectionProperties::SM_FULL_STEREO;
	}
...
void VistaOpenSGDisplayBridge::SetProjStereoMode( int iMode, VistaProjection * pTarget )
{
		...
		&& iMode <= VistaProjection::VistaProjectionProperties::SM_FULL_STEREO)
	{
		...
		ViewportData* pViewportData = Vista::assert_cast< ViewportData* >( pTarget->GetViewport()->GetData() );
		...
		{
			vstr::warnp() << "[VistaOpenSGDisplayBridge] - invalid stereo mode for stereo viewport - falling back to FULL_STEREO" << std::endl;
			iMode = VistaProjection::VistaProjectionProperties::SM_FULL_STEREO;
		}
		else if( !bShouldBeStereo && iMode == VistaProjection::VistaProjectionProperties::SM_FULL_STEREO )
		{
			vstr::warnp() << "[VistaOpenSGDisplayBridge] - invalid stereo mode FULL_STEREO for non-stereo viewport - falling back to MONO" << std::endl;
...
bool VistaOpenSGDisplayBridge::MakeScreenshot( const VistaWindow& oWin, const std::string& strFilename ) const
{
	int nSizeX, nSizeY;
	...
	if( nRead != nDataSize )
	{
		vstr::warnp() << "[VistaOpenSGDisplayBridge::MakeScreenshot]: Reading image from window failed" << std::endl;
		return false;
	}
	...
	{
		vstr::warnp() << "[VistaOpenSGDisplayBridge::MakeScreenshot]: Cannot create directory for file \""
