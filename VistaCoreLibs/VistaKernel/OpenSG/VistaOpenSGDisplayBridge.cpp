...
class ViveViewportData : public VistaOpenSGDisplayBridge::ViewportData {
 public:
  class ProjectionObserver : public IVistaObserver {
   public:
    ProjectionObserver(ViveViewportData* pData)
        : m_pData(pData) {
    }
...
bool VistaOpenSGDisplayBridge::DrawFrame() {
  ...
  auto                                       itWindow = mapWindows.rbegin();
...
bool VistaOpenSGDisplayBridge::RenderWindow(VistaWindow* pWindow) {
  ...
  for (auto & vecViewport : vecViewports) {
    if (vecViewport->GetViewportProperties()->GetIsEnabled())
      vecViewport->Render();
  }
...
bool VistaOpenSGDisplayBridge::RenderViewport(VistaViewport* pViewport) {
  ...
  case VistaViewport::VistaViewportProperties::VT_VIVE: {
    ...
    // }; ovrPosef a2EyeRenderPose[2]; ovrHmd_GetEyePoses( pViveData->m_pHmd, 0,
    // a2v3HmdToEyeViewOffset, a2EyeRenderPose, &pViveData->m_oTrackingState );
    ...
    // ); VistaQuaternion qHeadOrientation(
    // pViveData->m_oTrackingState.HeadPose.ThePose.Orientation.x,
    ...
    auto compositorError = vr::VRCompositor()->WaitGetPoses(devices, vr::k_unMaxTrackedDeviceCount, NULL, 0);
    if (compositorError != vr::VRCompositorError_None) {
      vstr::errp() << "Error while trying to get poses: " << compositorError << std::endl;
    }
    ...
    VistaTransformMatrix orientation( // column or row major????
        pose.m[0][0], pose.m[1][0], pose.m[2][0], 0.0, pose.m[0][1], pose.m[1][1], pose.m[2][1],
        0.0, pose.m[0][2], pose.m[1][2], pose.m[2][2], 0.0, pose.m[0][3], pose.m[1][3],
        pose.m[2][3], 1.0);

    orientation = orientation.GetInverted();

    vr::HmdMatrix34_t left_eye =
        pViveData->m_pVRSystem->GetEyeToHeadTransform(vr::EVREye::Eye_Left);
    vr::HmdMatrix34_t right_eye =
        pViveData->m_pVRSystem->GetEyeToHeadTransform(vr::EVREye::Eye_Right);
    // float standard_horizontal_eye_offset_left = std::fabs(left_eye.m[0][3]) +
    // std::fabs(right_eye.m[0][3])/-2.0; float standard_horizontal_eye_offset_right =
    // std::fabs(left_eye.m[0][3]) + std::fabs(right_eye.m[0][3])/2.0;

    // calculate eye offsets
    // VistaQuaternion qInverseHeadOri = qHeadOrientation.GetInverted();
    // VistaVector3D v3LeftEyeOffset( a2EyeRenderPose[0].Position.x, a2EyeRenderPose[0].Position.y,
    // a2EyeRenderPose[0].Position.z ); v3LeftEyeOffset -= v3HeadPosition; v3LeftEyeOffset =
    // qInverseHeadOri.Rotate( v3LeftEyeOffset ); VistaVector3D v3RightEyeOffset(
    // a2EyeRenderPose[1].Position.x, a2EyeRenderPose[1].Position.y, a2EyeRenderPose[1].Position.z
    // ); v3RightEyeOffset -= v3HeadPosition; v3RightEyeOffset = qInverseHeadOri.Rotate(
    // v3RightEyeOffset );
    ...
    vr::Texture_t leftEyeTexture{(void*)(pViveData->m_pWriteTextureTarget[0]->GetGLId()),
        vr::ETextureType::TextureType_OpenGL, vr::ColorSpace_Gamma};
    auto error = vr::VRCompositor()->Submit(vr::Eye_Left, &leftEyeTexture);
    if (error != vr::VRCompositorError_None) {
      vstr::errp() << "Error while submitting left eye texture: " << error << std::endl;
    }

    vr::Texture_t rightEyeTexture{(void*)(pViveData->m_pWriteTextureTarget[1]->GetGLId()),
        vr::ETextureType::TextureType_OpenGL, vr::ColorSpace_Gamma};
    error = vr::VRCompositor()->Submit(vr::Eye_Right, &rightEyeTexture);
    if (error != vr::VRCompositorError_None) {
      vstr::errp() << "Error while submitting right eye texture: " << error << std::endl;
    }
...
#else
    VISTA_THROW(
        "Vive-based rendering requested, but VistaKernel was not build with OculusSDK support", -1);
#endif
    break;
  }
...
VistaViewport* VistaOpenSGDisplayBridge::CreateViewport(
  ...
  ViewportData* pData = (eType == VistaViewport::VistaViewportProperties::VT_VIVE)
                            ? new ViveViewportData()
                            : new ViewportData();
#else
...
bool VistaOpenSGDisplayBridge::SetViewportType(
  ...
  if (eType == VistaViewport::VistaViewportProperties::VT_VIVE) {
    ...
    VistaViveGlutWindowingToolkit* pViveWinTK =
        dynamic_cast<VistaViveGlutWindowingToolkit*>(GetWindowingToolkit());
    if (pViveWinTK == NULL) {
      vstr::errp() << "Trying to create Vive Viewport without using Vive Window Toolkit"
                   << std::endl;
      return false;
    }
    ...
    VistaPropertyList oProjProps;
    oProjProps.SetValueInSubList(
        "NAME", "PROJECTION_VIVE", pTarget->GetNameForNameable() + "PROJECTION_VIVE");
    VistaProjection* pProj =
        GetDisplayManager()->CreateProjection(pTarget, "PROJECTION_VIVE", oProjProps);
    ProjectionData* pProjData = Vista::assert_cast<ProjectionData*>(pProj->GetData());
    VistaProjection::VistaProjectionProperties* pProjProps = pProj->GetProjectionProperties();
    pProjProps->SetStereoMode(VistaProjection::VistaProjectionProperties::SM_FULL_STEREO);
