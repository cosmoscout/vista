...
A NodeCore that references a Vista::IVistaOpenGLDraw Object to draw some plain OpenGL. As the
referenced object is no OpenSG FieldContainer, we don't name it here but  manually add it in the
implementation!
*/
...
#ifdef DEBUG
    vstr::warnp() << "[VistaOpenGLDrawCore]: got no bounding box from OpenGLDraw object! Creating "
                     "zero-volume around the node's origin..."
                  << std::endl;
#endif
  } else {
    volume.setEmpty();
...
void VistaOpenGLDrawCore::dump(UInt32, const BitVector) const {
