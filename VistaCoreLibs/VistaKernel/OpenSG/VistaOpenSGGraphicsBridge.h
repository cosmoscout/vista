...
class VISTAKERNELAPI VistaOpenSGGraphicsBridge : public IVistaGraphicsBridge {
 public:
  VistaOpenSGGraphicsBridge(osg::RenderAction* pAction, VistaOpenSGSystemClassFactory* pFactory);
  ...
  virtual bool CreateIndexedGeometry(const std::vector<VistaIndexedVertex>& vecVertices,
      ...
      const std::vector<VistaVector3D>& vecNormals, const std::vector<VistaColor>& vecColorsRGB,
  ...
  virtual bool GetVertices(
      std::vector<VistaIndexedVertex>& vecVertices, const IVistaGeometryData* pData) const;
  virtual bool GetCoordinates(
      std::vector<VistaVector3D>& vecCoords, const IVistaGeometryData* pData) const;
  virtual bool GetCoordinates(std::vector<float>& vecCoords, const IVistaGeometryData* pData) const;
  virtual bool GetTextureCoords2D(
      std::vector<VistaVector3D>& vecTexCoords, const IVistaGeometryData* pData) const;
  virtual bool GetTextureCoords2D(
      std::vector<float>& vecTexCoords, const IVistaGeometryData* pData) const;
  virtual bool GetNormals(
      std::vector<VistaVector3D>& vecNormals, const IVistaGeometryData* pData) const;
  virtual bool GetNormals(std::vector<float>& vecNormals, const IVistaGeometryData* pData) const;
  virtual bool GetColors(std::vector<VistaColor>& vecColors, const IVistaGeometryData* pData) const;
  virtual bool GetColors(std::vector<float>& vecColors, const IVistaGeometryData* pData) const;

  /**
   * Gives indices for all triangles in the geometry
   */
  virtual bool GetTrianglesVertexIndices(
      std::vector<int>& vecVertexIndices, const IVistaGeometryData* pData) const;
  virtual bool GetTrianglesNormalIndices(
      std::vector<int>& vecNormalIndices, const IVistaGeometryData* pData) const;
  ...
  virtual int  GetCoordinateIndex(const int nIndex, const IVistaGeometryData* pData) const;
  virtual bool GetCoordinate(
      const int nIndex, float a3fCoord[3], const IVistaGeometryData* pData) const;
  virtual VistaVector3D GetCoordinate(const int nIndex, const IVistaGeometryData* pData) const;

  virtual int  GetNormalIndex(const int nIndex, const IVistaGeometryData* pData) const;
  virtual bool GetNormal(
      const int nIndex, float a3fNormal[3], const IVistaGeometryData* pData) const;
  ...
  virtual bool GetTextureCoord(
      const int nIndex, float a3fTexCoord[3], const IVistaGeometryData* pData) const;
  virtual VistaVector3D GetTextureCoord(const int nIndex, const IVistaGeometryData* pData) const;
  ...
  virtual bool SetCoordinates(
      const int nStartIndex, const std::vector<float>& vecCoords, IVistaGeometryData* pData);

  virtual bool SetCoordinate(const int nIndex, const float a3fCoords[3], IVistaGeometryData* pData);
  virtual bool SetCoordinate(
      const int nIndex, const VistaVector3D& v3Coord, IVistaGeometryData* pData);
  virtual bool SetCoordIndex(const int nIndex, const int nValue, IVistaGeometryData* pData);
  virtual bool SetCoordIndices(
      const int nStartIndex, const std::vector<int>& indices, IVistaGeometryData* pData);

  virtual bool SetTextureCoords2D(const int nStartIndex,
      const std::vector<VistaVector3D>& vecTextureCoords2D, IVistaGeometryData* pData);
  virtual bool SetTextureCoords2D(const int nStartIndex,
      const std::vector<float>& vecTextureCoords2D, IVistaGeometryData* pData);

  virtual bool SetTextureCoord2D(
      const int nIndex, const float a3fCoord[3], IVistaGeometryData* pData);
  virtual bool SetTextureCoord2D(
      const int nIndex, const VistaVector3D& v3Coord, IVistaGeometryData* pData);

  virtual bool SetTextureCoordIndex(const int nIndex, const int value, IVistaGeometryData* pData);
  virtual bool SetTextureCoordsIndices(
      const int nStartIndex, const std::vector<int>& vecIndices, IVistaGeometryData* pData);

  virtual bool SetNormals(
      const int nStartIndex, const std::vector<float>& vecNormals, IVistaGeometryData* pData);
  virtual bool SetNormals(const int nStartIndex, const std::vector<VistaVector3D>& vecNormals,
      IVistaGeometryData* pData);

  virtual bool SetNormal(const int nIndex, const float a3fNormal[3], IVistaGeometryData* pData);
  virtual bool SetNormal(
      const int nIndex, const VistaVector3D& v3Normal, IVistaGeometryData* pData);
  virtual bool SetNormalIndex(const int nIndex, const int value, IVistaGeometryData* pData);
  virtual bool SetNormalIndices(
      const int nStartIndex, const std::vector<int>& vecIndices, IVistaGeometryData* pData);

  virtual bool SetColors(
      const int nStartIndex, const std::vector<VistaColor>& vecColors, IVistaGeometryData* pData);
  virtual bool SetColors(
      const int nStartIndex, const int nBufferLength, float* afColors, IVistaGeometryData* pData);

  virtual bool SetColor(const int nIndex, const VistaColor& oColor, IVistaGeometryData* pData);
  virtual bool SetColorIndex(const int nIndex, const int nValue, IVistaGeometryData* pData);
   ...
   */
  virtual bool GetFaces(std::vector<int>& vecFaces, const IVistaGeometryData* pData) const;
  ...
  virtual bool GetFaceCoords(const int nIndex, VistaVector3D& v3VertexA, VistaVector3D& v3VertexB,
      VistaVector3D& v3VertexC, const IVistaGeometryData* pData) const;
  ...
  virtual bool SetMaterialIndex(const int& materialIndex, IVistaGeometryData* pData);
  ...
  virtual bool SetMaterial(const VistaMaterial& oMaterial, IVistaGeometryData* pData);
  ...
  virtual bool GetBoundingBox(
      VistaVector3D& v3Min, VistaVector3D& v3Max, const IVistaGeometryData* pData) const;

  virtual bool GetRenderingAttributes(
      VistaRenderingAttributes& oAttrib, const IVistaGeometryData* pData) const;
  virtual bool SetRenderingAttributes(
      const VistaRenderingAttributes& oAttrib, IVistaGeometryData* pData);
