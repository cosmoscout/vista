...
void VistaOSGThreadImp::threadFct(void *vp)
{
	...
	{
		vstr::err() << "OSG-VistaThreadImp -- [" << sName << "] -- execution terminated, given exception: " << x.what() << std::endl;
	}
	catch( ... )
	{
		vstr::err() << "OSG-VistaThreadImp -- [" << sName << "] -- execution terminated due to unknown exception (ellipse catch).";
	}
