...
class VistaAcceptWork : public IVistaAcceptor {
 public:
  VistaAcceptWork(VistaMsgSource* pSrc, DLVistaActiveDataProducer* pActivator,
      const string& sInterfaceName, int iPort, const string& strIniFileName);
...
VistaAcceptWork::VistaAcceptWork(VistaMsgSource* pSrc, DLVistaActiveDataProducer* pActivator,
    const string& sInterfaceName, int iPort, const string& strIniFileName)
...
void VistaMsgSource::FillPacket(IDLVistaDataPacket* pPacket) {
  ...
  if (iRet < 4) {
#if defined(DEBUG)
    vstr::errp() << "[VistaMsgSource::FillPacket]: should read size (int32) -- but can not read "
                    "enough bytes ["
...
bool VistaMsgSource::RecycleDataPacket(
...
//        cout << "[ViMspPort]: Section (" << sIniSection << ") in file (" << sIniFile << ") tells
//        me "
...
//		Profile.GetTheProfileString(sIniSection, "PROGRESSHOST", "localhost", sProgressHost,
//sIniFile); 		int iProgressPort = Profile.GetTheProfileInt(sIniSection, "PROGRESSPORT", 6667,
//sIniFile); 		try
//		{
//			m_pProgressConnection = new VistaConnectionIP(VistaConnectionIP::CT_UDP,
//sProgressHost, iProgressPort);
//		}
//		catch(VistaExceptionBase &x)
//		{
//			x.PrintException();
...
VistaKernelMsgPort::VistaKernelMsgPort(VistaSystem* pVistaSystem, const string& sHost, int iPort,
    const string& sApplicationName, bool bCreateIndicator, const string& sIndicatorHost,
    int iIndicatorPort, IDLVistaPipeComponent* pFrontPipe) {
  m_sApplicationName = sApplicationName;
  m_pMsg             = new VistaProgressMessage;
  (*m_pMsg).SetApplicationName(m_sApplicationName);
  ...
  if (pFrontPipe) {
    ...
    m_pAcceptWork =
        new VistaAcceptWork(m_pSource, m_pProducer, sHost, iPort, pVistaSystem->GetIniFile());
    m_pPool->AddWork(m_pAcceptWork);
  ...
  if (bCreateIndicator) {
    try {
      m_pProgressConnection =
          new VistaConnectionIP(VistaConnectionIP::CT_UDP, sIndicatorHost, iIndicatorPort);
...
bool VistaKernelMsgPort::GetIsConnected() const {
  return m_pAcceptWork->GetIsDone() &&
         m_pAcceptWork->GetIsConnected(); // we are connected when this job is done?
}
...
bool VistaKernelMsgPort::DispatchInteractionMsg(
  ...
  case GETAVAILABLETRACKER:
    ...
    //		{
    //			liDevices =
    //m_pSystem->GetOldInteractionManager()->GetAvailableTrackerDrivers(); 			break;
    //		}
    //	case GETCURRENTDEVICES:
    //		{
    //			liDevices =
    //m_pSystem->GetOldInteractionManager()->GetCurrentDeviceDrivers(); 			break;
    //		}
    //	case GETCURRENTTRACKER:
    //		{
    //			liDevices =
    //m_pSystem->GetOldInteractionManager()->GetCurrentTrackerDrivers(); 			break;
    //		}
    //	case GETAVAILABLEDEVICES:
    //		{
    //			liDevices =
    //m_pSystem->GetOldInteractionManager()->GetAvailableDeviceDrivers(); 			break;
    //		}
    //	default:
    //		break;
    //	}
...
bool VistaKernelMsgPort::DispatchSystemMsg(
    int iMsgType, VistaMsg* pMsg, VistaByteBufferDeSerializer& dsSer) {
  switch (iMsgType) {
  case GETVISTAVERSION: {
    string answer = string(VistaVersion::GetVersion()) + string(" ") +
                    string(VistaVersion::GetBuildDateString());
    VistaMsg::MSG veAnswer;
...
bool VistaKernelMsgPort::DispatchDisplayMsg(
    int iMsgType, VistaMsg* pMsg, VistaByteBufferDeSerializer& dsSer) {
  switch (iMsgType) {
  case GETROOT: {
    VistaByteBufferSerializer ser;
...
bool VistaKernelMsgPort::DispatchInteractionDeviceMsg(
    int iMsgType, VistaMsg* pMsg, VistaByteBufferDeSerializer& dsSer) {
  switch (iMsgType) {
  case GETDEVICE:
    //{
    ...
    //		VistaInputDevice *pDevice =
    //m_pSystem->GetOldInteractionManager()->GetInputDeviceByRole(sDevRole); 		if(pDevice)
    //		{
    //			VistaByteBufferSerializer ser;
