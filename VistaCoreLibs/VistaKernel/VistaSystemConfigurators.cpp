	...
	VistaConnection *CreateConnection(const VistaPropertyList &oProps)
		{
			...
			pCon->SetByteorderSwapFlag(bBufferSwap ? VistaSerializingToolset::SWAPS_MULTIBYTE_VALUES : VistaSerializingToolset::DOES_NOT_SWAP_MULTIBYTE_VALUES );
	...
	VistaConnection *CreateConnection(const VistaPropertyList &oProps)
		{
			...
			pConn->SetByteorderSwapFlag(bByteSwap ? VistaSerializingToolset::SWAPS_MULTIBYTE_VALUES : VistaSerializingToolset::DOES_NOT_SWAP_MULTIBYTE_VALUES );
...
{
	for( std::pair< VistaWindow*, VistaDriverAbstractWindowAspect::IWindowHandle* > oHandle : m_mapHandles )
	{
										...
										const VistaPropertyList& oConfig )
{
	...
	std::list<VistaDriverAbstractWindowAspect::IWindowHandle*> liCurrentWindows = pAsp->GetWindowList();
	for(std::list<VistaDriverAbstractWindowAspect::IWindowHandle*>::const_iterator cit = liCurrentWindows.begin();
...
VistaDriverAbstractWindowAspect::IWindowHandle* VistaWindowConfigurator::GetHandleForWindow( VistaWindow* pWindow )
{
		...
		itSensor != liSensors.end(); ++itSensor)
	{
		...
		if( pSensorAsp ) // driver has sensor mapping, so use this to set up the sensors...
		{
			...
			if( nDriverSensorId != IVistaDriverCreationMethod::INVALID_TYPE )
			{
				...
				IVistaMeasureTranscoderFactory *pFac = pDriver->GetFactory()->GetTranscoderFactoryForSensor(sType);
				if( pFac )
								...
								const VistaPropertyList& oConfig )
{
		...
		itConnName != liConnections.end(); ++itConnName)
	{
		...
		{
			pConAsp->SetConnection( nRoleId, pCon, sDriverRole, VistaDriverConnectionAspect::FORBID_CONNECTION_REPLACEMENT );
		}
								...
								const VistaPropertyList& oConfig )
{
	...
	if( sType == "DEFAULT" )
	{
		...
		if(pOld)
		{
			pDriver->UnregisterAspect(pOld, IVistaDeviceDriver::DO_NOT_DELETE_ASPECT ); // remove, but do not delete, maybe a leak?
		}
