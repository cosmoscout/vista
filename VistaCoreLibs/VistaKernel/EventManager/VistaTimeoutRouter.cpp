...
bool VistaTimeoutRouter::CTimedCallback::Callback(
    double dTimeStamp, double dDesiredLength, double dRealLength) {
  if (m_nRepeatCount > 0)
    --m_nRepeatCount;
...
/*============================================================================*/
VistaTimeoutRouter::VistaTimeoutRouter(VistaEventManager* pEvMgr, VistaClusterMode* pClusterMode)
...
void VistaTimeoutRouter::HandleTimeout(HD_TIMER tim) {
  ...
  for (TIMLIST::iterator lit = m_liScheduledRemoval.begin(); lit != m_liScheduledRemoval.end();
       ++lit) {
    DoReleaseTim(*lit);
  }
...
bool VistaTimeoutRouter::CallUpdaters(CBLIST* pList, HD_TIMER tim) {
  m_bInCallback = true;
  for (CBLIST::const_iterator lit = (*pList).begin(); lit != (*pList).end(); ++lit) {
