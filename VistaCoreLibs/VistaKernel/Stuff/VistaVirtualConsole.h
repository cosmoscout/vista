...
class VISTAKERNELAPI VistaVirtualConsole : public IVistaSceneOverlay {
 public:
  VistaVirtualConsole(VistaDisplayManager* pDisplayManager, VistaEventManager* pEventManager,
      VistaKeyboardSystemControl* pCtrl, int iCmdToken, int cActivationKey = VISTA_KEY_ESC,
      const std::string& strViewportName = "");
  VistaVirtualConsole(VistaViewport* pViewport, VistaEventManager* pMgr,
      VistaKeyboardSystemControl* pCtrl, int iCmdToken, int cActivationKey = VISTA_KEY_ESC);
   ...
   */
  virtual bool Output(const std::string& strOutput, const VistaColor& oColor = VistaColor::WHITE);
  ...
  enum eCursorDir { E_CURSOR_RIGHT = 0, E_CURSOR_LEFT };

  enum eHistoryDir { E_HISTORY_UP = 0, E_HISTORY_DOWN };
   ...
   */
  void UpdateOnViewportChange(int iWidth, int iHeight, int iPosX, int iPosY);
 ...
 private:
  ...
  // state handling
  bool m_bEnabled, m_bInitDone;
  int  m_nActivationKey; /**< the enable / disable key for the console */

  VistaColor m_BackgroudColor, m_TextColor, m_CursorColor;
  ...
  int    m_nVpTotalLines, m_nVpTotalCols, m_nInputLines, m_nInputCursorPos, m_nOutputCursorPos;
  double m_nCharWidth, m_nCharHeight;

  void DrawString(const char* s, const int& nLength, const double& x, const double& y,
      const double& w, const double& h, const double& z);
  void DrawCharacter(const int& c, const double& x, const double& y, const double& w,
      const double& h, const double& z);

  /**
   * @return the number of lines used for printing
   */
  int DrawWrapString(const char* s, const int& nLength, const double& x, const double& y,
      const double& w, const double& h, const double& z, const int& wrap);
