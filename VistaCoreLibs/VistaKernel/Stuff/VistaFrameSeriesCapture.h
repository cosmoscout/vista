...
class VISTAKERNELAPI VistaFrameSeriesCapture : public VistaEventHandler {
 public:
   ...
   * FilePatterns can also have %F% and %S%, which will be replaced by the frame and screenshot
   * count For compatibility, you can also use $$ instead of %%
   */
  bool InitCaptureEveryFrame(const std::string& sLocation, const std::string& sFilenamePattern);
  bool InitCaptureEveryNthFrame(
