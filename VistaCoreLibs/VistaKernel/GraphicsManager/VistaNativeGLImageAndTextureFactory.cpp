...
VistaType::byte* LoadImageFromTgaStream(
    std::istream& oStream, int& nWidth, int& nHeight, int& nChannels) {
...
bool WriteTGAImageToStream(
    std::ostream& oStream, int nWidth, int nHeight, int nChannels, VistaType::byte* pData) {
...
VistaNativeOpenGLImageAndTextureCoreFactory::ImageCore*
VistaNativeOpenGLImageAndTextureCoreFactory::CreateImageCore() {
  return new VistaNativeOpenGLImageAndTextureCoreFactory::ImageCore();
}

VistaNativeOpenGLImageAndTextureCoreFactory::TextureCore*
VistaNativeOpenGLImageAndTextureCoreFactory::CreateTextureCore() {
...
bool VistaNativeOpenGLImageAndTextureCoreFactory::ImageCore::LoadFromFile(
    const std::string& sFilename) {
  std::size_t nDotPos = sFilename.rfind('.');
  if (nDotPos == std::string::npos) {
    vstr::warnp() << "VistaNativeOpenGL::LoadFromFile() -- filename \"" << sFilename
  ...
  if (VistaAspectsComparisonStuff::StringCaseInsensitiveEquals(sExtension, ".tga") == false) {
    vstr::warnp() << "VistaNativeOpenGL::LoadFromFile() -- cannot read file \"" << sFilename
                  << "\" - only TGA format is supported" << std::endl;
    return false;
  }

  std::ifstream oFile(sFilename.c_str(), std::ios::binary);
  if (oFile.is_open() == false) {
    vstr::warnp() << "VistaNativeOpenGL::LoadFromFile() -- could not open file \"" << sFilename
                  << "\"" << std::endl;
    return false;
  }
  ...
  if (pData == NULL) {
    vstr::warnp() << "VistaNativeOpenGL::LoadFromFile() -- reading data from file \"" << sFilename
                  << "\" failed" << std::endl;
    return false;
  }
...
bool VistaNativeOpenGLImageAndTextureCoreFactory::ImageCore::WriteToFile(
  ...
  default:
    vstr::warnp() << "VistaNativeOpenGL::WriteToFile() -- file \"" << sFilename
                  << "\" cannot be written - image has wrong format (requied GL_RGB or GL_RGBA)"
                  << std::endl;
    return false;
  }
  ...
  if (nDotPos == std::string::npos) {
    ...
    if (VistaAspectsComparisonStuff::StringCaseInsensitiveEquals(sExtension, ".tga") == false) {
      ...
      vstr::warnp() << "VistaNativeOpenGL::WriteToFile() -- file \"" << sFilename
                    << "\" - only TGA is supported, writing to \"" << sActualFilename
                    << "\" instead" << std::endl;
    }
  }
  VistaFileSystemFile oFile(sActualFilename);
  if (oFile.Exists() == false && oFile.CreateWithParentDirectories() == false) {
    vstr::warnp() << "VistaNativeOpenGL::WriteToFile() -- cannot create file \"" << sActualFilename
                  << "\"" << std::endl;
    return false;
  }
  ...
  if (oFileStream.is_open() == false) {
    vstr::warnp() << "VistaNativeOpenGL::WriteToFile() -- could not open file \"" << sActualFilename
                  << "\"" << std::endl;
    return false;
  }
...
bool VistaNativeOpenGLImageAndTextureCoreFactory::ImageCore::Set2DData(const int nWidth,
...
bool VistaNativeOpenGLImageAndTextureCoreFactory::ImageCore::Set3DData(const int nWidth,
...
bool VistaNativeOpenGLImageAndTextureCoreFactory::ImageCore::Rescale(
...
bool VistaNativeOpenGLImageAndTextureCoreFactory::ImageCore::Crop(const unsigned int nBottomLeftX,
...
bool VistaNativeOpenGLImageAndTextureCoreFactory::ImageCore::ReformatImage(
...
IVistaImageCore* VistaNativeOpenGLImageAndTextureCoreFactory::ImageCore::Clone() const {
  VistaNativeOpenGLImageAndTextureCoreFactory::ImageCore* pClone =
      new VistaNativeOpenGLImageAndTextureCoreFactory::ImageCore();
  if (GetIsValid()) {
...
bool VistaNativeOpenGLImageAndTextureCoreFactory::TextureCore::Set1DData(const int nWidth,
...
bool VistaNativeOpenGLImageAndTextureCoreFactory::TextureCore::Set2DData(const int nWidth,
  ...
  if (m_nNumMultisamples > 0) {
    if (pData != NULL) {
      vstr::warnp() << "[VistaGLTexture::Set2DData]: Cannot use multisampling when passing data - "
                       "using non-multisampled texture instead"
                    << std::endl;
  ...
  if (m_nNumMultisamples > 0)
    glTexImage2DMultisample(m_eTextureTarget, m_nNumMultisamples, eInternalFormat, nWidth, nHeight,
        false); // @TODO: fixedsamplelocations?
  else
    glTexImage2D(
        m_eTextureTarget, 0, eInternalFormat, nWidth, nHeight, 0, ePixelFormat, eDataFormat, pData);
...
bool VistaNativeOpenGLImageAndTextureCoreFactory::TextureCore::Set3DData(const int nWidth,
  ...
  glTexImage3D(m_eTextureTarget, 0, eInternalFormat, nWidth, nHeight, nDepth, 0, ePixelFormat,
      eDataFormat, pData);
...
bool VistaNativeOpenGLImageAndTextureCoreFactory::TextureCore::SetData(
  ...
  if (oImage.GetDimension() == 3) {
    return Set3DData(oImage.GetWidth(), oImage.GetHeight(), oImage.GetDepth(), oImage.GetData(),
        bGenMipmaps, eInternalFormat, ePixelFormat, eDataFormat);
  } else if (oImage.GetDimension() == 2) {
    return Set2DData(oImage.GetWidth(), oImage.GetHeight(), oImage.GetData(), bGenMipmaps,
        eInternalFormat, ePixelFormat, eDataFormat);
  } else if (oImage.GetDimension() == 1) {
    return Set1DData(oImage.GetWidth(), oImage.GetData(), bGenMipmaps, eInternalFormat,
        ePixelFormat, eDataFormat);
  } else {
...
void VistaNativeOpenGLImageAndTextureCoreFactory::TextureCore::SetMinFilter(
    const GLenum eMinFilterMode) {
  Bind();
...
void VistaNativeOpenGLImageAndTextureCoreFactory::TextureCore::SetMagFilter(
    const GLenum eMagFilterMode) {
  Bind();
...
void VistaNativeOpenGLImageAndTextureCoreFactory::TextureCore::SetWrapS(
    const GLenum eWrappingMode) {
  Bind();
...
void VistaNativeOpenGLImageAndTextureCoreFactory::TextureCore::SetWrapT(
    const GLenum eWrappingMode) {
  Bind();
...
void VistaNativeOpenGLImageAndTextureCoreFactory::TextureCore::SetWrapR(
    const GLenum eWrappingMode) {
  Bind();
...
void VistaNativeOpenGLImageAndTextureCoreFactory::TextureCore::SetAnisotropy(
    const float nAnisotropy) {
  Bind();
...
bool VistaNativeOpenGLImageAndTextureCoreFactory::TextureCore::Bind(
    const GLenum eTextureUnit) const {
  if (m_bIsValid == false)
    return false;
...
bool VistaNativeOpenGLImageAndTextureCoreFactory::TextureCore::Release(
    const GLenum eTextureUnit) const {
  glActiveTexture(eTextureUnit);
  glBindTexture(m_eTextureTarget, 0);
