...
/*                                                                            */
/*============================================================================*/
VistaGeometry* VistaGeometryFactory::CreatePlane(
		float nSizeX, float nSizeZ,
		int nResolutionX, int nResolutionZ,
		...
		float nMinTextureCoordZ, float nMaxTextureCoordZ)
{
	...
	CreatePlaneData(
		&index, &coords, &textureCoords, &normals, &colors, &nFaceType,
		nSizeX, nSizeZ, nResolutionX, nResolutionZ, eNormalDirection,
		oColor, eFaceType, nMinTextureCoordX, nMaxTextureCoordX,
		nMinTextureCoordZ, nMaxTextureCoordZ);

	VistaGeometry* pGeom = GetSG()->NewIndexedGeometry(
		index, coords, textureCoords, normals, colors,
...
/*                                                                            */
/*============================================================================*/
bool VistaGeometryFactory::CreatePlaneData(
...
/*                                                                            */
/*============================================================================*/
VistaGeometry* VistaGeometryFactory::CreateBox(
	float nSizeX, float nSizeY, float nSizeZ,
	...
	float nMinTextureCoordZ, float nMaxTextureCoordZ)
{
	...
	CreateBoxData(
		&index, &coords, &textureCoords, &normals, &colors, &nFaceType,
		nSizeX, nSizeY, nSizeZ, nResolutionX, nResolutionY, nResolutionZ, 
		oColor, eFaceType,
		nMinTextureCoordX, nMaxTextureCoordX,
		nMinTextureCoordY, nMaxTextureCoordY,
		nMinTextureCoordZ, nMaxTextureCoordZ);
	
	VistaGeometry* pGeo = m_pSG->NewIndexedGeometry(
		index, coords, textureCoords, normals, colors,
		oFormat, nFaceType);
...
/*                                                                            */
/*============================================================================*/
bool VistaGeometryFactory::CreateBoxData(
	vector<VistaIndexedVertex> *pIndex,
	vector<float> *pCoords,
	vector<float> *pTexCoords,
	vector<float> *pNormals,
...
/*                                                                            */
/*============================================================================*/
bool VistaGeometryFactory::CreateDiskData(
	vector<VistaIndexedVertex> *pIndex,
	vector<float> *pCoords,
	vector<float> *pTexCoords,
	vector<float> *pNormals,
...
{	
	...
	for(int j = 1; j <= resD; j++)
	{
		for(int i = 0; i < resC; i++)
		{
			...
			{
			case Vista::X:
				pTexCoords->push_back((float)((-j*stepR*std::cos(i*2*Vista::Pi/resC))/radius+1)/2);
				pTexCoords->push_back((float)((-j*stepR*std::sin(i*2*Vista::Pi/resC))/radius+1)/2);
				break;
			default:
			case Vista::Y:
				pTexCoords->push_back((float)((j*stepR*std::cos(i*2*Vista::Pi/resC))/radius+1)/2);
	...
	bool bBottom, bool bTop, bool bSides)
{
	...
	CreateConeData(&index, &coords, &textureCoords, &normals, &colors,
				   botRad, topRad, height, resC, resD, resY, color,
				   bBottom, bTop, bSides);
...
/*                                                                            */
/*============================================================================*/
bool VistaGeometryFactory::CreateConeData(
	vector<VistaIndexedVertex> *pIndex,
	vector<float> *pCoords,
	vector<float> *pTexCoords,
	vector<float> *pNormals,
	...
	if(bBottom)
	{
		for(int j = 1; j <= resD; j++)
		{
			for(int i = 0; i < resC; i++)
			{
				...
				pTexCoords->push_back((float)((-j*stepR*std::cos(i*2*Vista::Pi/resC))/botRad+1)/2);
				pTexCoords->push_back((float)((j*stepR*std::sin(i*2*Vista::Pi/resC))/botRad+1)/2);
			}
		}
	}
	...
	if(bTop)
	{
		...
		for(int j = 1; j <= resD; j++)
		{
			for(int i = 0; i < resC; i++)
			{
				...
				pTexCoords->push_back((float)((j*stepR1*std::cos(i*2*Vista::Pi/resC))/topRad+1)/2);
				pTexCoords->push_back((float)((j*stepR1*std::sin(i*2*Vista::Pi/resC))/topRad+1)/2);
			}
		}
		pCoords->push_back(0);
		pCoords->push_back(height/2.0f);
...
{	
	...
	CreateTorusData(innerRad, outerRad, resSides, resRings, color,
					&index, &coords, &textureCoords, &normals, &colors);
...
/*                                                                            */
/*============================================================================*/
bool VistaGeometryFactory::CreateTorusData(
	float innerRad, float outerRad,
	int resSides, int resRings,
	VistaColor color,
	vector<VistaIndexedVertex> *pIndex,
...
/*                                                                            */
/*============================================================================*/
bool VistaGeometryFactory::CreateSphereData(
	vector<VistaIndexedVertex> *pIndex,
...
{
	return CreateEllipsoidData(
		pIndex, pCoords, pTexCoords, pNormals, pColors,
	...
	const VistaColor & color)
{
	...
	CreateEllipsoidData(
		&index, &coords, &textureCoords, &normals, &colors,
		radius_a, radius_b, radius_c, thetaPrecision, phiPrecision, color);
...
/*                                                                            */
/*============================================================================*/
bool VistaGeometryFactory::CreateEllipsoidData(
	vector<VistaIndexedVertex> *pIndex,
	vector<VistaVector3D> *pCoords,
	vector<VistaVector3D> *pTexCoords,
	vector<VistaVector3D> *pNormals,
	...
	const VistaColor & color)
{
	...
	for (i=0; i < thetaPrecision; i++)
	{
		...
		for (j= 1; j<phiPrecision-1; j++)
		{
			...
			(*pCoords)[idx] = VistaVector3D(-radius_a * sinf(phi) * cosf(theta),
											-radius_b * cosf(phi),
											-radius_c * sinf(phi) * sinf(theta));

			(*pNormals)[idx] = VistaVector3D(-sinf(phi) * cosf(theta)/radius_a,
											 -cosf(phi)/radius_b,
	...
	for(i=0; i<=thetaPrecision; i++)
		for(j=1; j<phiPrecision-1; j++)
			(*pTexCoords)[tidx++] = VistaVector3D(float(i)/float(thetaPrecision),
												  float(j)/(float(phiPrecision) - 1.0f),
												  0.0);

	for(j=0; j<2; j++)
		for(i=0; i<thetaPrecision; i++)
			(*pTexCoords)[tidx++] = VistaVector3D((2.0f*float(i)+1.0f)/(2.0f*float(thetaPrecision)),
												  j ? 1.0f : 0.0f,
												  0.0f);
	...
	)
{
	...
	CreateTriangleData(a, b, c, resolution, color,
					   &index, &coords, &textureCoords, &normals, &colors);
...
/*                                                                            */
/*============================================================================*/
bool VistaGeometryFactory::CreateTriangleData(
	const VistaVector3D & a,
	const VistaVector3D & b,
	const VistaVector3D & c,
	int resolution,
...
VistaGeometry* VistaGeometryFactory::CreateFromPropertyList( const VistaPropertyList &oPropList )
{
	...
	else if( oCompare( sType, "DISK" ) )
	{
		...
		Vista::AXIS nNormal =
			Vista::AXIS(oPropList.GetValueOrDefault<int>( "NORMAL", Vista::Y ));
