...
class VistaRenderingAttributes {
 public:
  ...
  VistaRenderingAttributes(raShading sh = SHADING_DEFAULT, raCulling c = CULL_DEFAULT,
      raStyle st = STYLE_DEFAULT, raTextureFilter t = TEXTURE_FILTER_DEFAULT)
      : shading(sh)
      , culling(c)
      , style(st)
      , texFilter(t) {
...
class VISTAKERNELAPI VistaMaterial {
 public:
  VistaMaterial();
  VistaMaterial(const VistaColor& ambient, const VistaColor& diffuse, const VistaColor& specular,
      const VistaColor& emission, const float shininess, const float opacity,
      const std::string& name);
  VistaMaterial(const float ambient[3], const float diffuse[3], const float specular[3],
      const float emission[3], const float shininess, const float opacity, const std::string& name);
...
class VISTAKERNELAPI VistaGeometry : public VistaGeometryObservable {
 ...
 public:
  ...
  bool GetTrianglesVertexAndNormalIndices(
      std::vector<int>& vecVertexIndices, std::vector<int>& vecNormalIndices) const;
   ...
   *  @return bool : false if point outside of face or there is an error in computation, true in all
   * other cases
   */
  ...
  IVistaGeometryData* GetData() {
    return m_pData;
  }
  const IVistaGeometryData* GetData() const {
    return m_pData;
  }
