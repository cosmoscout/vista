...
VistaNode::VistaNode(VistaGroupNode* pParent, IVistaNodeBridge* pBridge, IVistaNodeData* pData,
...
bool VistaNode::GetWorldBoundingBox(VistaBoundingBox& oBox) const {
  ...
  if (GetParentWorldTransform(matToWorldTransform) == false)
    return false;
  return VistaBoundingBox::ComputeAABB(v3Min, v3Max, matToWorldTransform, oBox);
}
...
bool VistaNode::GetScale(VistaVector3D& v3Scale) const {
  ...
  if (matTransform.Decompose(v3Translation, qRot, v3Scale) == false) {
    vstr::warnp() << "[VistaNode::GetScale]: Trying to retrieve non-uniform, sheared scale!"
                  << std::endl;
    return false;
  }
  return true;
...
bool VistaNode::GetWorldScale(VistaVector3D& v3Scale) const {
  ...
  // this will fail if the matrix contains a shearing component!
  if (matTransform.Decompose(v3Translation, qRot, v3Scale) == false) {
    vstr::warnp() << "[VistaNode::GetWorldScale]: Trying to retrieve non-uniform, sheared scale!"
                  << std::endl;
    return false;
  }
  return true;
