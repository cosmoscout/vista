...
bool VistaSystem::DoInit( int argc, char **argv )
{
	...
	m_pSystemClassFactory = new VistaOpenSGSystemClassFactory(
		this, bUseOpenSGThreads );
#else
	...
	if(!m_pSystemClassFactory)
	{
		vstr::errp() << "Unable to initialize the system - "
			"no SystemClassFactory available!" << std::endl;
		m_bInitialized = false;
		return false;
	}
...
bool VistaSystem::SetupDisplayManager()
{
	...
	if( m_eFrameCaptureMode != -1 )
	{
				...
				itWin != mapWindows.end(); ++itWin )
		{
			...
			{
#ifdef WIN32
				std::replace( m_sCaptureFramesFilePattern.begin(), m_sCaptureFramesFilePattern.end(), '\\', '/' );
#endif
...
bool VistaSystem::SetupInteractionManager()
{
	...
	GetEventManager()->AddEventHandler( m_pKeyboardSystemControl, VistaSystemEvent::GetTypeId(), VistaSystemEvent::VSE_POSTAPPLICATIONLOOP, VistaEventManager::PRIO_HIGH + 5 );
...
{	
	...
	else // --> NOT m_pClusterMode->GetIsLeader
	{
				...
				itDriver != liDriverList.end(); ++itDriver )
		{
			...
			VistaShallowDriver *pDriver
					= dynamic_cast<VistaShallowDriver*>(
									(*VistaShallowDriver::GetDriverFactoryMethod()).CreateDriver() );

			vstr::outi() << "Adding driver of type [" + sType + "] with name [" + sName + "]" << std::endl;

			int def_history_size;
			oDriverSection.GetValueOrDefault<int>("HISTORY", def_history_size, 1 );

			unsigned int nPrio = oDriverSection.GetValueOrDefault<unsigned int>( "PRIORITY", 0 );
			mapCreatedDrivers[sName] = DriverInfo(pDriver, sType, sName, (*itDriver), nPrio, std::list<std::string>(), def_history_size );
		}
...
bool VistaSystem::IntroMsg (std::ostream *pStream) const
{
	...
	PrintMsg( "*                                                                            *\n", pStream );
	PrintMsg( "* Torsten W. Kuhlen et al.,           1998-2014                              *\n", pStream );
	PrintMsg( "*                                                                            *\n", pStream );
...
bool VistaSystem::ArgHelpMsg (const std::string& sAppName, std::ostream *pStream) const
{
	...
	PrintMsg("-capture_frames_filename <pattern>  : captured files are written with the according filename pattern\n", pStream);
