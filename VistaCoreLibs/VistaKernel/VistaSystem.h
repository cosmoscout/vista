...
class VISTAKERNELAPI VistaSystem : public VistaEventHandler {
 public:
  ...
  // ############################################################################
  bool IndicateApplicationProgress(int iAppProgress, const std::string& sProgressMessage);
 ...
 private:
  ...
  bool LoadDriverPlugin(const std::string& sDriverType, const std::string& sPluginName = "",
      const std::string& sTranscoderName = "");

  bool IndicateSystemProgress(const std::string& sProgressMessageText, bool bDone);

  std::string FindFileInIniSearchPath(const std::string& file, std::ostream* logger = NULL) const;
