...
bool VistaBoundingBoxAdapter::GetPosition(VistaVector3D &pTrans, const VistaReferenceFrame &oReferenceFrame) const
{
	...
	if ( bbox.Intersection( &start[0], &direction[0], true, &intersection[0], tmp[0], &tmp[0], tmp[0] ) > 0 )
	{
...
{
	AdapterVector::iterator it = std::remove( 
		m_vecScores.begin(),
		m_vecScores.end(),
		pNode
		);
...
void VistaIntentionSelect::Update( std::vector<IVistaIntentionSelectAdapter*>& vecObj )
{
	...
	for(AdapterVector::iterator it = m_vecScores.begin();
		it != m_vecScores.end(); ++it)
	{
		...
		else
		{
			...
			float fContrib = 
				CalculatePointContribution(v3Point) 
				* pPointNode->GetContributionMultiplier()
				;

			float fScore = std::max<float>(0.0f, (pPointNode->GetScore() * m_nStickyness) + (fContrib * m_nSnappiness));
	...
	// (assumed to be empty)
	for(AdapterVector::iterator cit = m_vecScores.begin();
