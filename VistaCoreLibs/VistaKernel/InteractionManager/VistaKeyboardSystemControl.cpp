...
bool VistaKeyboardSystemControl::BindAction(
	int nKeyCode,
	...
	bool bForce /*= false*/)
{
	return BindAction(nKeyCode, VISTA_KEYMOD_ANY, funcCallback, sHelpText, bManageDeletion, bCallOnKeyRepeat, bForce);
}
	...
	bool bForce /*= false*/)
{
	return BindAction(
		nKeyCode,
		nModifiers,
		new VistaSimpleCallback(funcCallback),
		sHelpText,
		bManageDeletion,
		bCallOnKeyRepeat,
		bForce
...
void VistaKeyboardSystemControl::EmitBufferedKeyActions()
{
	...
	{
		if( (*itAction) != NULL ) // actions may be NULLed if they were unregistered qhile already in the queue
