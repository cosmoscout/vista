...
bool VistaDfnSystemTriggerControlNode::GetIsValid() const {
  return (m_pCtrl && ((m_pTriggers != NULL && m_pTrigger == NULL) ||
...
bool VistaDfnSystemTriggerControlNode::DoEvalNode() {
  ...
  {
    std::vector<int> vUnswallowedTrigger, vUnswallowedMods;
    if (m_pModders) {
      m_pCtrl->InjectKeys(m_pTriggers->GetValueConstRef(), m_pModders->GetValueConstRef(),
          vUnswallowedTrigger, vUnswallowedMods);
    } else {
      std::vector<int> vTmp(m_pTriggers->GetValueConstRef().size(), 0);
      m_pCtrl->InjectKeys(
          m_pTriggers->GetValueConstRef(), vTmp, vUnswallowedTrigger, vUnswallowedMods);
    }
