...
bool VistaKernelDfnNodeCreators::RegisterNodeCreates(VistaSystem* pVistaSystem) {
  ...
  pFac->SetNodeCreator(
      "ViewerSource", new VistaDfnViewerSourceNodeCreate(pVistaSystem->GetDisplayManager()));
  pFac->SetNodeCreator(
      "ViewerSink", new VistaDfnViewerSinkNodeCreate(pVistaSystem->GetDisplayManager()));
  pFac->SetNodeCreator(
      "WindowSource", new VistaDfnWindowSourceNodeCreate(pVistaSystem->GetDisplayManager()));
  pFac->SetNodeCreator(
      "ViewportSource", new VistaDfnViewportSourceNodeCreate(pVistaSystem->GetDisplayManager()));
  pFac->SetNodeCreator("ProjectionSource",
      new VistaDfnProjectionSourceNodeCreate(pVistaSystem->GetDisplayManager()));
  pFac->SetNodeCreator(
      "3DMouseTransform", new Vista3DMouseTransformNodeCreate(pVistaSystem->GetDisplayManager()));
  pFac->SetNodeCreator("MousewheelChangeDetect", new VistaDfnMouseWheelChangeDetectNodeCreate);
  pFac->SetNodeCreator("ReferenceFrameTransform",
      new VistaDfnReferenceFrameTransformNodeCreate(pVistaSystem->GetDisplayManager()));
  pFac->SetNodeCreator("SensorFrame", new TVdfnDefaultNodeCreate<VistaDfnSensorFrameNode>);
  pFac->SetNodeCreator(
      "DeviceDebug", new VistaDfnDeviceDebugNodeCreate(pVistaSystem->GetDisplayManager()));
  pFac->SetNodeCreator("SystemTriggerControl",
      new VistaDfnSystemTriggerControlNodeCreate(pVistaSystem->GetKeyboardSystemControl()));
  pFac->SetNodeCreator("Navigation", new VistaDfnNavigationNodeCreate());
  pFac->SetNodeCreator(
      "ClusterNodeInfo", new VdfnClusterNodeInfoNodeCreate(pVistaSystem->GetClusterMode()));
  pFac->SetNodeCreator(
      "Frameclock", new VistaDfnFrameclockNodeCreate(pVistaSystem->GetClusterMode()));
  ...
  pFac->SetNodeCreator(
      "DumpHistory", new VdfnDumpHistoryNodeClusterCreate(pVistaSystem->GetClusterMode()));

  pFac->SetNodeCreator("Trackball", new VistaDfnTrackballNodeCreate());
  pFac->SetNodeCreator(
      "EventSource", new VistaDfnEventSourceNodeCreate(
                         pVistaSystem->GetEventManager(), pVistaSystem->GetInteractionManager()));
  ...
  pFac->SetNodeCreator("TextOverlay[double]",
      new VistaDfnTextOverlayNodeCreate<double>(pVistaSystem->GetDisplayManager()));
  ...
  pFac->SetNodeCreator(
      "SimpleText", new VistaDfnSimpleTextNodeCreate(pVistaSystem->GetDisplayManager()));

  pFac->SetNodeCreator(
      "CropViewport", new VistaDfnCropViewportNodeCreate(pVistaSystem->GetDisplayManager()));
  pFac->SetNodeCreator(
      "KeyCallback", new VistaDfnKeyStateNodeCreate(pVistaSystem->GetKeyboardSystemControl()));
  pFac->SetNodeCreator(
      "KeyState", new VistaDfnKeyStateNodeCreate(pVistaSystem->GetKeyboardSystemControl()));
  pFac->SetNodeCreator("ProximityWarning", new VistaDfnProximityWarningNodeCreate(pVistaSystem));
  pFac->SetNodeCreator("Fadeout", new VistaDfnFadeoutNodeCreate(pVistaSystem->GetClusterMode(),
                                      pVistaSystem->GetDisplayManager()));
  pFac->SetNodeCreator("Geometry",
      new VistaDfnGeometryNodeCreate(pVistaSystem->GetGraphicsManager()->GetSceneGraph()));
  pFac->SetNodeCreator("PointingRay", new VistaDfnPointingRayGeometryNodeCreate(
                                          pVistaSystem->GetGraphicsManager()->GetSceneGraph()));
  ...
  try {
    ...
    if (subs.GetValue("displaysystem", strDisplaySystem) &&
      ...
      if (pViewport && pSys) {
        ...
        VistaVdfn3DMouseTransformNode* pNd =
            new VistaVdfn3DMouseTransformNode(pSys, pViewport, fOriginOffset);
...
IVdfnNode* VistaDfnDeviceDebugNodeCreate::CreateNode(const VistaPropertyList& oParams) const {
  try {
    ...
    VistaDfnDeviceDebugNode* pRet =
        new VistaDfnDeviceDebugNode(m_pDisplayManager, strTagName, strPorts, sViewportName);

    if (subs.HasProperty("red") || subs.HasProperty("green") || subs.HasProperty("blue")) {
...
IVdfnNode* VdfnDumpHistoryNodeClusterCreate::CreateNode(const VistaPropertyList& oParams) const {
  try {
    ...
    std::string strFinalName = strHostName.empty() ? strFileName : strHostName + "." + strFileName;
...
VistaDfnEventSourceNodeCreate::VistaDfnEventSourceNodeCreate(
    VistaEventManager* pEvMgr, VistaInteractionManager* pInMa)
    : VdfnNodeFactory::IVdfnNodeCreator()
    , m_pEvMgr(pEvMgr)
    , m_pInMa(pInMa) {
}
...
IVdfnNode* VistaDfnEventSourceNodeCreate::CreateNode(const VistaPropertyList& oParams) const {
  return new VistaDfnEventSourceNode(
      m_pEvMgr, m_pInMa, oParams.GetValueOrDefault<std::string>("tag", ""));
}
...
IVdfnNode* VistaDfnNavigationNodeCreate::CreateNode(const VistaPropertyList& oParams) const {
  ...
  if (oPars.GetValue<int>("default_navigation_mode", iDefaultMode) == false) {
    ...
    if (oPars.GetValue<std::string>("default_navigation_mode", sDefaultMode)) {
      if (VistaAspectsComparisonStuff::StringCaseInsensitiveEquals(sDefaultMode, "unconstrained"))
        ...
        vstr::warnp() << "[VistaDfnNavigationNodeCreate]: unknown value [" << sDefaultMode
                      << "] for param \"default_navigation_mode\"" << std::endl;
      }
    }
  }

  float fDefaultLinearVelocity = oPars.GetValueOrDefault<float>("default_linear_velocity", 1.0f);
  float fDefaultAngularVelocity =
      oPars.GetValueOrDefault<float>("default_angular_velocity", Vista::Pi);
  ...
  return new VistaDfnNavigationNode(iDefaultMode, fDefaultLinearVelocity, fDefaultAngularVelocity,
      fLinearAcceleration, fLinearDeceleration, fAngularAcceleration, fAngularDeceleration);
}
  ...
  try {
    ...
    if (oSubs.GetValue("displaysystem", sDisplaySystemName)) {
      ...
      vstr::warnp() << "[VistaDfnViewerSinkNodeCreate]: More than one display system available "
                       " - please specify a \"displaysystem\" parameter!"
                    << std::endl;
    }
...
VistaDfnSystemTriggerControlNodeCreate::VistaDfnSystemTriggerControlNodeCreate(
...
IVdfnNode* VistaDfnSystemTriggerControlNodeCreate::CreateNode(
...
IVdfnNode* VistaDfnReferenceFrameTransformNodeCreate::CreateNode(
  ...
  return (new VistaDfnReferenceFrameTransformNode(pDisplaySystem->GetReferenceFrame(),
      bTransformToFrame, iNrPositionPorts, iNrOrientationPorts, iNrMatrixPorts));
}
...
IVdfnNode* VistaDfnCropViewportNodeCreate::CreateNode(const VistaPropertyList& oParams) const {
  ...
  if (oParams.GetValue("viewports", vecViewportNames)) {
    for (std::vector<std::string>::const_iterator itViewportName = vecViewportNames.begin();
      ...
      if (pViewport) {
        ...
        vstr::warnp() << "[VistaCropViewportNodeCreate]: Viewport of name [" << (*itViewportName)
                      << "] does not exist" << std::endl;
      }
    }
  } else {
    for (std::map<std::string, VistaViewport*>::const_iterator itViewport =
             m_pDisplayManager->GetViewportsConstRef().begin();
  ...
  VistaDfnCropViewportNode* pNode =
      new VistaDfnCropViewportNode(nLeft, nRight, nBottom, nTop, bUseProjExtents, oCropColor);
  ...
  if (oSubs.GetValue("filter_projection_normal", v3Normal)) {
    ...
    for (std::vector<VistaViewport*>::const_iterator itViewport = vecViewports.begin();
      ...
      (*itViewport)
          ...
          ->GetProjPlaneNormal(v3ProjectionNormal[0], v3ProjectionNormal[1], v3ProjectionNormal[2]);
...
IVdfnNode* VistaDfnKeyCallbackNodeCreate::CreateNode(const VistaPropertyList& oParams) const {
  ...
  if (oSubs.GetValue<int>("key_value", nKeyCode) == false) {
    ...
    if (oSubs.GetValue<std::string>("key", sKey)) {
      if (VistaKeyboardSystemControl::GetKeyAndModifiersValueFromString(sKey, nKeyCode, nModCode) ==
          false) {
        vstr::warnp() << "[DfnKeyCallbackNodeCreate]: invalid \"key\"-entry [" << sKey << "]"
                      << std::endl;
        return NULL;
      }
    } else {
      vstr::warnp() << "[DfnKeyCallbackNodeCreate]: neither \"key\" nor \"key_value\" specified"
                    << std::endl;
      return NULL;
    }
  }
  if (oSubs.GetValue<int>("mod_value", nModCode) == false) {
    ...
    if (oSubs.GetValue<std::string>("mod", sMod)) {
      ...
      if (nModCode == -1) {
        vstr::warnp() << "[DfnKeyCallbackNodeCreate]: invalid \"mod\"-entry [" << sMod << "]"
                      << std::endl;
        return NULL;
      }
    }
...
IVdfnNode* VistaDfnProximityWarningNodeCreate::CreateNode(const VistaPropertyList& oParams) const {
  ...
  if (oSubs.GetValue("safe_distance", nSafeDistance) == false ||
    ...
    vstr::warnp() << "[VistaDfnProximityWarningNodeCreate]: \"safe_distance\""
                     " or \"danger_distance\" entry missing"
                  << std::endl;
    return NULL;
  }
  ...
  if (sType.empty() || VistaAspectsComparisonStuff::StringCaseInsensitiveEquals(sType, "fadeout")) {
    VistaColor oColor = oSubs.GetValueOrDefault<VistaColor>("color", VistaColor::BLACK);

    VistaProximityFadeout* pFadeout = new VistaProximityFadeout(
        m_pVistaSystem->GetEventManager(), nSafeDistance, nDangerDistance);
    ...
    if (oSubs.GetValue("viewports", vecViewports)) {
      for (std::vector<std::string>::const_iterator itViewport = vecViewports.begin();
        ...
        VistaViewport* pViewport =
            m_pVistaSystem->GetDisplayManager()->GetViewportByName((*itViewport));
        if (pViewport == NULL) {
    ...
    VistaProximitySign* pSign = new VistaProximitySign(m_pVistaSystem->GetEventManager(),
        nSafeDistance, nDangerDistance, bDisableOcclusion, m_pVistaSystem->GetGraphicsManager());
    std::string         sTexture;
    if (oSubs.GetValue<std::string>("texture", sTexture)) {
      if (pSign->SetTexture(sTexture) == false) {
        vstr::warnp() << "[VistaDfnProximityWarningNodeCreate]: specified \"texture\" "
                      << " for warning type \"sign\" could not be loaded - using default texture"
                      << std::endl;
    ...
    if (m_pVistaSystem->GetDisplayManager()->GetNumberOfDisplaySystems() > 0) {
      VistaUserPlatform* pPlatform =
          m_pVistaSystem->GetPlatformFor(m_pVistaSystem->GetDisplayManager()->GetDisplaySystem(0));
    ...
    bool bDisableOcclusion = oSubs.GetValueOrDefault<bool>("disable_occlusion", true);
    VistaProximityBarrierTape* pTape =
        new VistaProximityBarrierTape(m_pVistaSystem->GetEventManager(), nSafeDistance,
            nDangerDistance, bDisableOcclusion, m_pVistaSystem->GetGraphicsManager());
    ...
    vstr::warnp() << "[VistaDfnProximityWarningNodeCreate]: \"type\"-value [" << sType
                  << "] invalid" << std::endl;
    return NULL;
  }
  ...
  VistaType::microtime nFlashPeriod =
      oSubs.GetValueOrDefault<VistaType::microtime>("flash_period", 0.5);
  bool bFlashInDangerZone = oSubs.GetValueOrDefault<bool>("flash_in_dangerzone", false);
  VistaType::microtime nFlashAfterLostTracking =
      oSubs.GetValueOrDefault<VistaType::microtime>("flash_after_lost_tracking", 0);
  VistaType::microtime nHideTimeout =
      oSubs.GetValueOrDefault<VistaType::microtime>("hide_timeout", 0);
  VistaType::microtime nHideFadeoutTime =
      oSubs.GetValueOrDefault<VistaType::microtime>("hide_fadeout_time", 2);
...
VistaDfnFadeoutNodeCreate::VistaDfnFadeoutNodeCreate(
    VistaClusterMode* pClusterMode, VistaDisplayManager* pDisplayManager)
    : VdfnNodeFactory::IVdfnNodeCreator()
    , m_pDisplayManager(pDisplayManager)
    , m_pClusterMode(pClusterMode) {
}
...
IVdfnNode* VistaDfnGeometryNodeCreate::CreateNode(const VistaPropertyList& oParams) const {
  ...
  if (oSubs.GetValue("modelfile", sFilename)) {
    ...
    if (pModelNode == NULL) {
      vstr::warnp() << "[VistaDfnGeometryNodeCreate]: could not load model file [" << sFilename
                    << "]" << std::endl;
      return NULL;
    }
  ...
  if (pNode->GetIsValid() == false) {
    ...
    if (oSubs.GetValue("offset_uniform_scale", nOffsetScaleUniform))
      pNode->GetOffsetNode()->SetScale(
          nOffsetScaleUniform, nOffsetScaleUniform, nOffsetScaleUniform);
...
VistaDfnPointingRayGeometryNodeCreate::VistaDfnPointingRayGeometryNodeCreate(
    VistaSceneGraph* pSceneGraph)
    : m_pSceneGraph(pSceneGraph) {
}

IVdfnNode* VistaDfnPointingRayGeometryNodeCreate::CreateNode(
    const VistaPropertyList& oParams) const {
  const VistaPropertyList& oSubs = oParams.GetSubListConstRef("param");
  ...
  VistaGeometry* pGeom = oFactory.CreateCone(nRadius, nRadiusTip, nLength, nNumSides, 1, 1, oColor);
  ...
  VistaTransformMatrix matTransform(
      VistaQuaternion(0.70710778f, 0, 0, -0.70710778f), VistaVector3D(0, 0, -0.5f * nLength));
