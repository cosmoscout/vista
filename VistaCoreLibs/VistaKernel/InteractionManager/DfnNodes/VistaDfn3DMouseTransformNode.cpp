...
class VistaVdfn3DMouseTransformNode::TransObserver : public IVistaObserver {
 public:
  enum { ID_PROJECTION = 0, ID_VIEWPORT, ID_DISPSYSTEM, ID_PLATFORM };
  ...
  void ReleaseObserveable(IVistaObserveable* pObserveable, int nTicket) {
    std::vector<IVistaObserveable*>::iterator it =
        std::remove(m_vecObjs.begin(), m_vecObjs.end(), pObserveable);
...
unsigned int VistaVdfn3DMouseTransformNode::CalcUpdateNeededScore() const {
  if (m_bNeedsUpdate) // use the const-version here, we need the flags later
  {
    ...
    return (
        m_nUpdateCount + IVdfnNode::CalcUpdateNeededScore()); // maybe mouse coords have changed?
}
...
void VistaVdfn3DMouseTransformNode::UpdateOnProjectionChange() {
  ...
  m_pProjection->GetProjectionProperties()->GetProjPlaneExtents(
      m_nLeft, m_nRight, m_nBottom, m_nTop);
...
void VistaVdfn3DMouseTransformNode::UpdateOnDisplaySystemChange() {
  ...
  if (!m_pSystem->GetDisplaySystemProperties()
           ->GetLocalViewer()) // the position was given in "world-coords"
                               // but we want it local to the projection
    m_v3ViewerPos =
        m_pSystem->GetReferenceFrame()->TransformPositionToFrame(m_v3ViewerPos); // transform back
}
