 ...
 *
 * @ingroup VdfnNodes
 * @inport{position, Vector3D, optional, sets translation of the geometry (for sub-root nodes, this
 * is position then)}
 * @inport{orientation, Quaternion, optional, set rotation of the geometry (for sub-root nodes this
 * is the orientation then}
...
class VISTAKERNELAPI VistaDfnGeometryNode : public IVdfnNode {
 public:
  ...
  VistaDfnGeometryNode(
      IVistaNode* pSubtreeRoot, VistaSceneGraph* pSceneGraph, bool bDeleteSubtree = true);
  VistaDfnGeometryNode(const VistaPropertyList& oGeometryConfig, VistaSceneGraph* pSceneGraph);
  ~VistaDfnGeometryNode();
