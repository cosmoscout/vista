...
int VistaInteractionContext::SerializeContext(
    IVistaSerializer& oSerializer, const VistaInteractionContext& oContext) {
  int nRet = 0;
    ...
    if (nCnt > 0) {
      ...
      for (VdfnGraph::Nodes::const_iterator cit = nodes.begin(); cit != nodes.end(); ++cit) {
        ...
        if ((*cit)->GetIsMasterSim()) {
          ...
          if (sNodeName.empty()) {
            vstr::warnp() << "[VistaInteractionContext::SerializeContext]: "
...
int VistaInteractionContext::DeSerializeContext(
    IVistaDeSerializer& oDeSerializer, VistaInteractionContext& oContext) {
...
VistaInteractionContext::VistaInteractionContext(
    VistaInteractionManager* pMgr, VistaEventManager* pEvMgr)
    : IVistaObserveable()
    , m_bEnabled(true)
    , m_bEventSource(true)
    , m_bRegistered(false)
...
void VistaInteractionContext::DumpStateAsDot() const {
  ...
  std::string           sFilename =
      strGraphName + "_" + VistaTimeUtils::ConvertToLexicographicDateString(nTime) + ".dot";
  DumpStateAsDot(sFilename);
}

void VistaInteractionContext::DumpStateAsDot(const std::string& sTargetFile) const {
  std::string strGraphName = m_pInteractionManager->GetRoleForId(GetRoleId());
  VdfnPersistence::SaveAsDot(m_pTransformGraph, sTargetFile, strGraphName, true, true, true);
}
