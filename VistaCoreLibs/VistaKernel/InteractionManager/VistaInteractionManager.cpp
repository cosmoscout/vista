...
class VistaInteractionManagerReadstateMapSource : public IVddReadstateSource {
 public:
  ...
  virtual VistaSensorReadState* GetReadState(VistaDeviceSensor& sensor) {
    if (sensor.GetParent()) {
      std::map<VistaDeviceSensor*, VistaSensorReadState*> m =
          GetReadStatesForDriver(sensor.GetParent());
      std::map<VistaDeviceSensor*, VistaSensorReadState*>::iterator it = m.find(&sensor);
      if (it == m.end()) {
        // error?
        vstr::errp() << "Could not retrieve read state for sensor [" << sensor.GetSensorName()
                     << "]";
        return 0;
      }
      return (*it).second;
...
class VistaInteractionUpdateObserver : public IVistaObserver {
 public:
  class MapObj2Msg {
   public:
    ...
    bool operator==(const MapObj2Msg& oOther) const {
      if (oOther.m_nMsg >= 0)
        return ((m_pCtx == oOther.m_pCtx) && m_nMsg == oOther.m_nMsg);
      else
  ...
  virtual ~VistaInteractionUpdateObserver() {
    for (std::vector<VistaInteractionContext*>::size_type z = 0; z < m_vecObsVec.size(); ++z) {
  ...
  virtual void ReleaseObserveable(IVistaObserveable* pObserveable, int) {
    ...
    for (std::vector<MapObj2Msg>::size_type n = 0; n < m_mpUpdates.size(); ++n) {
      std::vector<MapObj2Msg>& v = m_mpUpdates[n];

      std::vector<MapObj2Msg>::iterator it = std::remove(v.begin(), v.end(), MapObj2Msg(pObj, -1));
    ...
    std::vector<VistaInteractionContext*>::iterator vit =
        std::remove(m_vecObsVec.begin(), m_vecObsVec.end(), pObj);
  ...
  virtual bool Observes(IVistaObserveable* pObserveable) {
    ...
    const std::vector<VistaInteractionContext*>::iterator vit =
        std::remove(m_vecObsVec.begin(), m_vecObsVec.end(), pObj);
...
static bool SetupNodeWithTag(const std::string& sNodeName, IVdfnNode* pNode) {
  ...
  for (std::list<std::string>::const_iterator cit = liTokens.begin(); cit != liTokens.end();
       ++cit) {
    ...
    if (ve[0] == "enable") {
      ...
      for (std::list<std::string>::const_iterator k = liNames.begin(); k != liNames.end(); ++k) {
...
VistaInteractionManager::VistaInteractionManager(VistaEventManager* pEventManager)
  ...
  m_pEventManager->AddEventHandler(this, VistaSystemEvent::GetTypeId(),
      VistaSystemEvent::VSE_UPDATE_INTERACTION, VistaEventManager::PRIO_HIGH + 100);
  m_pEventManager->AddEventHandler(this, VistaSystemEvent::GetTypeId(),
      VistaSystemEvent::VSE_UPDATE_DELAYED_INTERACTION, VistaEventManager::PRIO_HIGH + 100);
...
bool VistaInteractionManager::Init(VistaDriverMap* pDriverMap, bool bIsLeaderInCluster) {
  m_bIsActiveProducer = bIsLeaderInCluster;
  vstr::outi() << "VistaInteractionManager() -- Created as "
               << (m_bIsActiveProducer ? "ACTIVE" : "PASSIVE") << " InteractionManager"
               << std::endl;
...
bool VistaInteractionManager::AddDeviceDriver(
    const std::string& sName, IVistaDeviceDriver* pDriver, int nPrio) {
...
bool VistaInteractionManager::ActivateDeviceDriver(IVistaDeviceDriver* pDriver, int nPrio) {
  ...
  case IVistaDeviceDriver::UPDATE_EXPLICIT_POLL: {
    ...
    std::stable_sort(m_vecInFrameUpdates.begin(), m_vecInFrameUpdates.end(), _qCompPrio());
    return true;
  }
...
bool VistaInteractionManager::DelDeviceDriver(const std::string& sName) {
  ...
  case IVistaDeviceDriver::UPDATE_CONNECTION_THREADED: {
    ...
    IVistaExplicitCallbackInterface* pI = m_pUpdater->RemConnectionUpdate(
        pConAsp->GetConnection(pConAsp->GetUpdateConnectionIndex()));
    delete pI;
    break;
  }
  case IVistaDeviceDriver::UPDATE_CUSTOM_THREADED: {
    VistaDriverThreadAspect* pThreadAspect = dynamic_cast<VistaDriverThreadAspect*>(
        pDriver->GetAspectById(VistaDriverThreadAspect::GetAspectId()));
    if (!pThreadAspect)
      return false;

    if (pThreadAspect->StopProcessing()) {
      std::vector<IVistaDeviceDriver*>::iterator it =
          std::remove(m_vecThreadedUpdates.begin(), m_vecThreadedUpdates.end(), pDriver);
...
bool VistaInteractionManager::SetPostUpdateFunctor(
    const std::string& sDriverName, IVistaPostUpdateFunctor* pFunctor) {
  ...
  if (pDriver) {
    ...
    case IVistaDeviceDriver::UPDATE_CONNECTION_THREADED: {
      ...
      if (pConAsp && pConAsp->GetConnection()) {
        DriverUpdateCallback* pCb = dynamic_cast<DriverUpdateCallback*>(
            m_pUpdater->GetConnectionUpdate(pConAsp->GetConnection()));
        if (pCb) {
...
VistaInteractionManager::IVistaPostUpdateFunctor* VistaInteractionManager::GetPostUpdateFunctor(
    const std::string& sDriverName) const {
  IVistaDeviceDriver* pDriver = GetDeviceDriver(sDriverName);
...
bool VistaInteractionManager::AddInteractionContext(
    VistaInteractionContext* pCtx, int nPriority, bool bDelayedUpdate) {
  ...
  if (GetInteractionContextByRoleId(pCtx->GetRoleId())) {
    vstr::warnp() << " [VistaInteractionMgr]: Context with role ["
                  << GetRoleForId(pCtx->GetRoleId()) << "] already registered" << std::endl;
...
bool VistaInteractionManager::LoadGraphForContext(
    VistaInteractionContext* pContext, const std::string& sNodeTag) {
  std::string sGraphFile = pContext->GetGraphSource();
  std::string sRole      = GetRoleForId(pContext->GetRoleId());
  if (sGraphFile.empty()) {
    vstr::warnp() << "Can't load context for role [" << sRole << "] - no graph file spcified"
                  << std::endl;
    return false;
  }

  vstr::outi() << "Loading DFN graph [" << sGraphFile << "] for Context with Role [" << sRole << "]"
               << std::endl;
  ...
  if (pGraph == NULL) {
    ...
    for (VdfnGraph::Nodes::iterator itNode = liNodes.begin(); itNode != liNodes.end(); ++itNode) {
...
bool VistaInteractionManager::ReloadGraphForContext(VistaInteractionContext* pContext,
    const std::string& sNodeTag, const bool bDumpGraphAsDot, const bool bWritePortsToDump) {
  ...
  if (bDumpGraphAsDot) {
    ...
    VdfnPersistence::SaveAsDot(
        pContext->GetTransformGraph(), sFileName, sGraphName, bWritePortsToDump, true);
  }
  return true;
}
...
bool VistaInteractionManager::RemoveFromUpdateList(IVistaDeviceDriver* pDev) {
  std::vector<DRIVER>::iterator it =
      std::remove(m_vecInFrameUpdates.begin(), m_vecInFrameUpdates.end(), DRIVER(pDev, 0));
...
bool VistaInteractionManager::RemInteractionContext(VistaInteractionContext* pCtx) {
  std::vector<CONTEXT>::iterator it = std::remove(
      m_vecInteractionContexts.begin(), m_vecInteractionContexts.end(), CONTEXT(pCtx, 0, false));
...
bool VistaInteractionManager::DelInteractionContext(VistaInteractionContext* pCtx) {
  ...
  if (m_bProcessingEvents == false) {
    for (std::vector<CONTEXT>::iterator cit = it; cit != m_vecInteractionContexts.end(); ++cit) {
      delete (*cit).m_pContext;
    }
...
int VistaInteractionManager::GetInteractionContextPriority(
    VistaInteractionContext* pContext) const {
...
bool VistaInteractionManager::SetInteractionContextPriority(
    VistaInteractionContext* pContext, int nPriority) {
...
int VistaInteractionManager::GetInteractionContextDelayedUpdate(
    VistaInteractionContext* pContext) const {
  for (std::vector<CONTEXT>::const_iterator itCtx = m_vecInteractionContexts.begin();
       itCtx != m_vecInteractionContexts.end(); ++itCtx) {
    if ((*itCtx).m_pContext == pContext) {
...
bool VistaInteractionManager::SetInteractionContextDelayedUpdate(
    VistaInteractionContext* pContext, bool bDelayedUpdate) {
  for (std::vector<CONTEXT>::iterator itCtx = m_vecInteractionContexts.begin();
       itCtx != m_vecInteractionContexts.end(); ++itCtx) {
    if ((*itCtx).m_pContext == pContext) {
...
bool VistaInteractionManager::RegisterEventTypes(VistaEventManager* pEventMgr) {
  ...
  for (int n = VistaInteractionEvent::VEID_CONTEXT_CHANGE; n < VistaInteractionEvent::VEID_LAST;
       ++n)
...
unsigned int VistaInteractionManager::GetRoleId(const std::string& sRole) const {
  for (ROLEMAP::const_iterator cit = m_mapRoleIdMap.begin(); cit != m_mapRoleIdMap.end(); ++cit) {
    if ((*cit).second == sRole)
      return (*cit).first;
  }
...
VistaInteractionContext* VistaInteractionManager::GetInteractionContextByRoleId(
    unsigned int nRoleId) const {
...
bool VistaInteractionManager::RegisterDriverCreationMethod(
    const std::string& sTypeName, IVistaDriverCreationMethod* pMethod, bool bForceRegistration) {
  if (!m_pDriverMap)
    return false;
...
bool VistaInteractionManager::UnregisterDriverCreationMethod(
    const std::string& sTypeName, bool bDeleteDriverCreationMethod) {
  if (!m_pDriverMap)
    return false;
...
IVistaDriverCreationMethod* VistaInteractionManager::GetDriverCreationMethod(
    const std::string& sTypeName) const {
  if (!m_pDriverMap)
    return NULL;
...
const VistaInteractionManager::READSTATEVECTOR* VistaInteractionManager::GetReadStatesForDriver(
    IVistaDeviceDriver* pDriver) const {
