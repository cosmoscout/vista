...
static void makeGaussBlobAlpha(ImagePtr &img, const float &sigma, const float &m, const bool bApplyScale = true)
{
...
void VistaOpenSGParticles::SetUseUnscaledGaussBlobTexture( const int iTexSize, const float fSigma, const float fCenterHeight ) const
{
	ImagePtr pImage = Image::create();
	beginEditCP(pImage);
...
void VistaOpenSGParticles::SetShaderPrograms(const std::string &VertexShaderProgram, const std::string &FragmentShaderProgram)
{
