...
struct VistaOpenSGGraphicsStatsOverlay::Line {
 public:
  ...
  IVistaTextEntity* GetText() const {
    return m_pText;
  }
  ...
  void UpdateText() {
    ...
    oText << m_sPrefix << std::setw(S_nEntrySize) << ((double)m_nFactor * m_pStatElem->getValue())
          << m_sPostfix;
...
VistaOpenSGGraphicsStatsOverlay::VistaOpenSGGraphicsStatsOverlay(VistaDisplayManager* pManager,
    VistaEventManager* pEventManager, const bool bRegisterAllViewports)
...
VistaOpenSGGraphicsStatsOverlay::~VistaOpenSGGraphicsStatsOverlay() {
  for (std::vector<VistaSimpleTextOverlay*>::iterator itOverlay = m_vecTextOverlays.begin();
       itOverlay != m_vecTextOverlays.end(); ++itOverlay) {
    delete (*itOverlay);
  }
  for (std::vector<Line*>::iterator itLine = m_vecLines.begin(); itLine != m_vecLines.end();
       ++itLine) {
    delete (*itLine);
  }
...
void VistaOpenSGGraphicsStatsOverlay::SetIsEnabled(bool bSet) {
  ...
  if (m_vecLines.empty()) {
    ...
    VistaOpenSGDisplayBridge* pOSGBridge =
        dynamic_cast<VistaOpenSGDisplayBridge*>(m_pDisplayManager->GetDisplayBridge());
