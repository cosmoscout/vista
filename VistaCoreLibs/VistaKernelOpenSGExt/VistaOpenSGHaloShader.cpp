...
/*There are two types of lights, which can be rendered in dependence to the over given transparency
 * value.*/
/*The brightness of the halo should be controlled by the parent class*/

/*Type 1:*/
/*If the transparency value is below 0.5 then the light is rendered with z-value 0 and with a
 * rotating star effect.*/
/*This means if an object has the value 0 then it is nearly fully transparent and if it is rendered
 * with 0.49 it is very opaque rendered with a star.*/
/*Please take notice that setting the z-value to 0 requires the parent class to turn off the light
 * manually if its non-visible (for example via ray tracing).*/
/*See LabInLab Demo car front lights.*/

/*Type 2 (SIMPLER):*/
/*If the transparency value is between 0.5 and 1 then the object is simple rendered with an
 * approximated Gaussian.*/
/*This means if an object has the value 0.5 then it is nearly fully transparent and if it is
 * rendered with 1 it is very opaque.*/
/*This is the simplest way for creating a glow effect. The z-value simply over given and not set to
 * zero, thus there could be occlusion problems.*/
...
const std::string VistaOpenSGHaloShader::s_sFragmentShader =
    ...
    "	if( gl_FrontFacing ){\n"
    "		float fAngle = asin(dot(v3WorldPos, v3WorldNormal) / (length(v3WorldPos) * "
    "length(v3WorldNormal)));\n"
    ...
    "			vec3 v3DiagVec1 = vec3( cos(fRotationAngle+0.785), "
    "sin(fRotationAngle+0.785), 0.0 );\n"
    "			vec3 v3DiagVec2 = vec3( -sin(fRotationAngle+0.785), "
    "cos(fRotationAngle+0.785), 0.0 );\n"
    ...
    "			float fHorAngle = abs(asin(dot(v3HorVec, v3WorldNormal) / "
    "(length(v3HorVec)*length(v3WorldNormal))));\n"
    "			float fVerAngle = abs(asin(dot(v3VerVec, v3WorldNormal) / "
    "(length(v3VerVec)*length(v3WorldNormal))));\n"
    "			float fDiag1Angle = abs(asin(dot(v3DiagVec1, v3WorldNormal) / "
    "(length(v3DiagVec1)*length(v3WorldNormal))));\n"
    "			float fDiag2Angle = abs(asin(dot(v3DiagVec2, v3WorldNormal) / "
    "(length(v3DiagVec2)*length(v3WorldNormal))));\n"
    ...
    "			float fDiag1Brightness = max(0.0, "
    "fMainBrightness*(fMainBrightness-tColor[3]*fDiag1Angle));\n"
    "			float fDiag2Brightness = max(0.0, "
    "fMainBrightness*(fMainBrightness-tColor[3]*fDiag2Angle));\n"
    "			tColor[3] = "
    "0.25*(fHorBrightness+fVerBrightness)+(fDiag1Brightness+fDiag2Brightness)+fMainBrightness;\n"
    "		}\n"
    "		else{\n"
...
bool VistaOpenSGHaloShader::ApplyToOSGMaterial(
