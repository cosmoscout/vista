...
namespace
{
	...
	std::map< VistaOpenSGMultiMaterialShaderCreator::ShaderConfig, std::string > S_mapFragmentShaderStrings;
}
...
const std::string& VistaOpenSGMultiMaterialShaderCreator::GetVertexShader( const ShaderConfig& oConfig )
{
	FragmentShaderMode nId = FS_NO_TEXTURE;
	if( oConfig.m_bHasBaseMap || oConfig.m_bHasBlendMap
		|| oConfig.m_bHasSpecularMap ||
...
const std::string& VistaOpenSGMultiMaterialShaderCreator::GetFragmentShader( const ShaderConfig& oConfig )
{
	...
	if( oConfig.m_bHasSpecularMap )
		sString +=	"void ProcessLight( in int iLight, in vec3 v3Normal, in vec3 v3ToPosDir, in float nSpecularityInfluence, inout vec3 v3Color )\n";
	else
		sString +=	"void ProcessLight( in int iLight, in vec3 v3Normal, in vec3 v3ToPosDir, inout vec3 v3Color )\n";
	sString +=		"{\n"
					"    vec3 v3LightDir;\n" 
					...
					"        {\n"
					"            vec3 v3SpecularityColor = gl_FrontMaterial.specular.rgb * gl_LightSource[iLight].specular.rgb;\n"
	...
	if( oConfig.m_bHasNormalMap )
	{
		sString +=	"    // process data for normal mapping \n"
					...
					"    v3Normal = normalize( ( texture2D( spNormalMap, gl_TexCoord[0].st ).xyz * 2.0 ) - 1.0 );\n"
	...
	if( oConfig.m_bHasSpecularMap )
	{
		sString +=	"    float nSpecularityInfluence = texture2D( spSpecularMap, gl_TexCoord[0].st ).r;\n"						
					"    if( OSGLight0Active )\n"
	...
	{
		if( oConfig.m_bHasGlossMap )
			sString	+=	"\n    float nReflInfluence = nReflectionFactor * texture2D( spGlossMap, gl_TexCoord[0].st ).r;\n";
		else
		...
		if( oConfig.m_bHasObjectEnvMap && oConfig.m_bHasWorldEnvMap )
		{
			sString	+=	
					...
					"    vec4 v4CubeColorObject = textureCube( spObjectEnvMap, v3ReflectedObject, nEnvMapBias );\n"
		...
		{
			sString	+=	
					"    vec3 v3Reflected = normalize( vec3( OSGInvViewMatrix * vec4( reflect( v3WorldPos, v3WorldNormal ), 0 ) ) );\n"
		...
		{
			sString	+=	
					"    vec3 v3Reflected = normalize( vec3( OSGInvWorldMatrix * OSGInvViewMatrix * vec4( reflect( v3WorldPos, v3WorldNormal ), 0 ) ) );\n"
...
bool VistaOpenSGMultiMaterialShaderCreator::ShaderConfig::operator<( const ShaderConfig& oConfig ) const
{
