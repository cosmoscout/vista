...
std::string GetImageFileFromPrefix(
    const std::string& sPrefix, const std::vector<std::string>& vecExtension) {
  std::string sFile = sPrefix;
  if (VistaFileSystemFile(sFile).Exists())
    return sFile;
  for (std::vector<std::string>::const_iterator itExtension = vecExtension.begin();
       itExtension != vecExtension.end(); ++itExtension) {
...
osg::TextureChunkPtr VistaOpenSGTextureLoader::LoadNormalMapTextureFromBumpMap(
...
bool VistaOpenSGTextureLoader::ConvertBumpMapImageToNormalMap(
    osg::ImagePtr pImage, const float nNormalMagnitude) {
  if (pImage->getPixelFormat() != osg::Image::OSG_L_PF) {
    beginEditCP(pImage);
    pImage->reformat(osg::Image::OSG_L_PF);
    endEditCP(pImage);
  }
  return osg::createNormalMapFromBump(
      pImage, osg::NullFC, osg::Vec3f(1.0f, 1.0f, 1.0f / nNormalMagnitude));
}
...
osg::TextureChunkPtr VistaOpenSGTextureLoader::RetrieveTexture(const std::string& sFilename) {
  std::map<std::string, osg::TextureChunkRefPtr>::const_iterator itEntry =
      m_mapLoadedTextures.find(sFilename);
...
osg::TextureChunkPtr VistaOpenSGTextureLoader::RetrieveNormalMapTexture(
    const std::string& sFilename) {
  std::map<std::string, osg::TextureChunkRefPtr>::const_iterator itEntry =
      m_mapLoadedNormalTextures.find(sFilename);
...
osg::TextureChunkPtr VistaOpenSGTextureLoader::RetrieveNormalMapTextureFromBumpMap(
    const std::string& sFilename, const float nNormalMagnitude, const bool bSaveConvertedImage) {
  ...
  std::map<std::string, osg::TextureChunkRefPtr>::const_iterator itEntry =
      m_mapLoadedNormalTextures.find(sNormalMapName);
  if (itEntry != m_mapLoadedNormalTextures.end()) {
    assert((*itEntry).second != osg::NullFC);
    return (*itEntry).second;
  }
...
osg::CubeTextureChunkPtr VistaOpenSGTextureLoader::RetrieveCubemapTexture(
    const std::string& sTopFile, const std::string& sBottomFile, const std::string& sLeftFile,
    const std::string& sRightFile, const std::string& sFrontFile, const std::string& sBackFile) {
  std::string sIndexString =
      GetCubemapIndexString(sTopFile, sBottomFile, sLeftFile, sRightFile, sFrontFile, sBottomFile);

  std::map<std::string, osg::CubeTextureChunkRefPtr>::const_iterator itEntry =
      m_mapCubemapTextures.find(sIndexString);
  if (itEntry != m_mapCubemapTextures.end()) {
    assert((*itEntry).second != osg::NullFC);
    return (*itEntry).second;
  }

  osg::CubeTextureChunkPtr pTex =
      LoadCubeMapTexture(sTopFile, sBottomFile, sLeftFile, sRightFile, sFrontFile, sBackFile);
...
std::string VistaOpenSGTextureLoader::GetNormalMapFilenameForBumpmap(
  ...
  std::string sNormalMapName =
      sBaseName + "_normal_" + VistaConversion::ToString(nNormalMagnitude) + sExtension;
...
std::string VistaOpenSGTextureLoader::GetCubemapIndexString(const std::string& sTopFile,
  ...
  std::string sIndex = sTopFile + "|" + sBackFile + "|" + sLeftFile + "|" + sRightFile + "|" +
                       sFrontFile + "|" + sBackFile;
  return sIndex;
}
...
void VistaOpenSGTextureLoader::SetImageSearchPathes(
    const std::vector<std::string>& vecSearchPathes) {
...
void VistaOpenSGTextureLoader::SetImageFileExtensions(
...
bool VistaOpenSGTextureLoader::GetCubemapFileLocations(const std::string& sFolder,
    std::string& sTopFile, std::string& sBottomFile, std::string& sLeftFile,
    std::string& sRightFile, std::string& sFrontFile, std::string& sBackFile) {
  ...
  return (!sTopFile.empty() && !sBottomFile.empty() && !sLeftFile.empty() && !sRightFile.empty() &&
          !sFrontFile.empty() && !sBackFile.empty());
}
