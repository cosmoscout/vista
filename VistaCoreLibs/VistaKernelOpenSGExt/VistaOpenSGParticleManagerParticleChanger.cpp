...
VistaVector3D PMPhysicalEnvironment::ApplyPhysics(float* v3VelVec, float* v3PosVec,
    VistaType::microtime dDeltaT, VistaType::microtime dParticleTime) {
  // Creating the vectors we're later working with :-)

  /*
  Question is whether we need m_v3Vel/Pos-Vectors each time. the m_v3VelVector is used for
  StreamingVel and as output, the PosVector only used once so it might be cheaper to not use it
  here. However, I am pretty confident one could use it further .....
  */
  ...
  // In case m_v3PositionVector makes sense, change to:
  // m_v3PositionVector=m_fDensity*pow((1-(0.0065*(v3PositionVector[1])/288.15)),5.255);

  /*

  We are pretty static here. I think we could have some list (or array for the beginning) that
  contains Force-Objects that calculate forces. Then each run we go through this array and add the
  returned values to our m_v3Forces vector :-)
  ...
  */
  ...
  // Static Lift
  if (m_bStaticLiftOn) {
    m_v3Forces += VistaVector3D(
        0, m_fGravity * m_fObjectVolume * (m_fDensityAtParticle - m_fObjectDensity), 0);
  }
  ...
  if (m_bDynamicLiftOn) {
    ...
    fForcesY = 0;
    fForcesY += m_v3LiftCoefficientXYZ[0] * m_fDensityAtParticle * 0.5f *
                m_v3StreamingVelocities[0] * m_v3StreamingVelocities[0] * m_v3WorkingSurfaceXYZ[0];
    fForcesY += m_v3LiftCoefficientXYZ[2] * m_fDensityAtParticle * 0.5f *
                m_v3StreamingVelocities[2] * m_v3StreamingVelocities[2] * m_v3WorkingSurfaceXYZ[2];

    if (m_v3Wind[1] > 0) {
      fForcesY += m_v3LiftCoefficientXYZ[1] * m_fDensityAtParticle * 0.5f *
                  m_v3StreamingVelocities[1] * m_v3StreamingVelocities[1] *
                  m_v3WorkingSurfaceXYZ[1];
    }
  ...
  All forces/winds here (except anything that got to deal with density) are static (even those that
  deal with density, but we try to influence them by height).

  You could (and maybe SHOULD) make them non-static to somehow have a feeling of reality. This can
  be solved by function or lambda expressions once you're running on C++0x!
  */
...
PMPhysicalEnvironment::PMPhysicalEnvironment(float fObjectMass, float fObjectVolume,
    float fObjectDenisty, float fGravity, float fDensity, const VistaVector3D& v3LiftCoefficient,
    const VistaVector3D& v3DragCoefficientXYZ, const VistaVector3D& v3WorkingSurfaceXYZ)
...
PMPhysicalEnvironment::PMPhysicalEnvironment(float fObjectMass, float fGravity, float fDensity,
    const VistaVector3D& v3DragCoefficientXYZ, const VistaVector3D& v3WorkingSurfaceXYZ)
    : m_fObjectMass(fObjectMass)
    , m_fGravity(fGravity)
    , m_fDensity(fDensity)
...
float PMPhysicalEnvironment::CalculateDrag(int iDirection) {
  return (signum<float>(m_v3StreamingVelocities[iDirection]) * m_v3DragCoefficientXYZ[iDirection] *
...
void PMPhysicalFrameChange::ChangeParticle(
...
void PMPhysicalFrameChange::CalculatePositionAndVelocities(SParticle& oParticle, double dt) {
  ...
  VistaVector3D mReturnVelocities =
      m_pPhysicEngine->ApplyPhysics(oParticle.m_a3fVelocity, oParticle.m_a3fPosition, dt, 0);
...
PMPhysicalFrameChange::PMPhysicalFrameChange(
    float fColorFactor, float fSizeFactor, float fVelocitiesFactor)
    : IParticleChanger()
...
void PMOriginSet::CalculatePositionAndVelocities(
    SParticle& oParticle, VistaType::microtime fEventTime) {
  ...
  oParticle.m_a3fPosition[0] =
      m_v3ParticleOrigin[0] + (float)m_pRand->GenerateDouble2() * m_fOriginFactor;
  oParticle.m_a3fPosition[1] =
      m_v3ParticleOrigin[1] + (float)m_pRand->GenerateDouble2() * m_fOriginFactor;
  oParticle.m_a3fPosition[2] =
      m_v3ParticleOrigin[2] + (float)m_pRand->GenerateDouble2() * m_fOriginFactor;
}

void PMOriginSet::CalculateColor(SParticle& oParticle, VistaType::microtime fEventTime) {
  oParticle.m_a4fColor[0] =
      m_v3ParticleColor[0] + (float)m_pRand->GenerateDouble2() * m_fColorFactor;
  oParticle.m_a4fColor[1] =
      m_v3ParticleColor[1] + (float)m_pRand->GenerateDouble2() * m_fColorFactor;
  oParticle.m_a4fColor[2] =
      m_v3ParticleColor[2] + (float)m_pRand->GenerateDouble2() * m_fColorFactor;
...
PMOriginSet::PMOriginSet(const VistaVector3D& v3ParticleVelocities, float fVelocitiesFactor,
    const VistaVector3D& v3ParticleOrigin, float fOriginFactor,
    const VistaVector3D& v3ParticleColor, float fColorFactor, float fAlphaFactor,
    const VistaVector3D& v3ParticleSize, float fSizeFactor)
    : IParticleChanger()
...
void PMOriginSet::ChangeParticle(
    SParticle& oParticle, VistaType::microtime dCurrentTime, VistaType::microtime dDeltaT) {
...
void PMLinearFrameChange::ChangeParticle(
    SParticle& oParticle, VistaType::microtime dCurrentTime, VistaType::microtime dDeltaT) {
  *oParticle.m_pRemainingLifeTime -= (float)dDeltaT;
