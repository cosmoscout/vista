...
public:
	...
	void UpdateTexScaleParameters()
	{
		if( m_pShaderUniforms == osg::NullFC || GetUseTextureScale() == false )
			return;
		beginEditCP( m_pShaderUniforms, osg::SHLParameterChunk::ParametersFieldMask );
		m_pShaderUniforms->setUniformParameter( "u_v4TexCoordScale", osg::Vec4f( m_fTexScaleS, m_fTexScaleT, 1.0f, 1.0f ) );
		endEditCP( m_pShaderUniforms, osg::SHLParameterChunk::ParametersFieldMask );
	}
	...
	{
		if( m_pShaderUniforms == osg::NullFC || ( m_pWorldEnvironmentMap == osg::NullFC && m_pObjectEnvironmentMap == osg::NullFC ) )
			return;
		beginEditCP( m_pShaderUniforms, osg::SHLParameterChunk::ParametersFieldMask );
	...
	{
		return ( ( m_pBaseMap != osg::NullFC )
				|| ( m_pBlendMap != osg::NullFC )
				|| ( m_pSpecularMap != osg::NullFC )
				|| ( m_pNormalMap != osg::NullFC )
				|| ( m_pGlossMap != osg::NullFC && ( m_pWorldEnvironmentMap != osg::NullFC || m_pObjectEnvironmentMap != osg::NullFC ) ) );
	}
...
bool VistaOpenSGMultiMaterial::SetBaseMap( const std::string& sImageFile )
{
	...
	{
		m_pInternals->RemoveTexture( m_pInternals->m_pBaseMap, VistaOpenSGMultiMaterialShaderCreator::S_nBaseMapTextureId );
	...
	if( m_pInternals->m_pBaseMap == osg::NullFC )
		return false;		
	return m_pInternals->AddTexture( m_pInternals->m_pBaseMap, VistaOpenSGMultiMaterialShaderCreator::S_nBaseMapTextureId );
}
...
bool VistaOpenSGMultiMaterial::SetBlendMap( const std::string& sImageFile, const float nBlendFactor )
{
...
bool VistaOpenSGMultiMaterial::SetBlendMap( const std::string& sImageFile )
{
	...
	{
		m_pInternals->RemoveTexture( m_pInternals->m_pBlendMap, VistaOpenSGMultiMaterialShaderCreator::S_nBlendMapTextureId );
	...
	if( m_pInternals->m_pBlendMap == osg::NullFC )
		return false;		
	return m_pInternals->AddTexture( m_pInternals->m_pBlendMap, VistaOpenSGMultiMaterialShaderCreator::S_nBlendMapTextureId );
}
...
bool VistaOpenSGMultiMaterial::SetSpecularMap( const std::string& sImageFile )
{
	...
	{
		m_pInternals->RemoveTexture( m_pInternals->m_pSpecularMap, VistaOpenSGMultiMaterialShaderCreator::S_nSpecularMapTextureId );
	...
	if( m_pInternals->m_pSpecularMap == osg::NullFC )
		return false;		
	return m_pInternals->AddTexture( m_pInternals->m_pSpecularMap, VistaOpenSGMultiMaterialShaderCreator::S_nSpecularMapTextureId );
}
...
bool VistaOpenSGMultiMaterial::SetWorldEnvironmentMap( const std::string& sTopImage,
										const std::string& sBottomImage,
										const std::string& sLeftImage,
										const std::string& sRightImage,
										const std::string& sFrontImage,
										const std::string& sBackImage )
{
	if( m_pInternals->m_pWorldEnvironmentMap )
	{
		m_pInternals->RemoveTexture( m_pInternals->m_pWorldEnvironmentMap, VistaOpenSGMultiMaterialShaderCreator::S_nWorldEnvMapTextureId );
	...
	m_pInternals->m_pWorldEnvironmentMap = m_pInternals->m_pTextureLoader->RetrieveCubemapTexture( sTopImage,
																							   sBottomImage,
																							   sLeftImage,
																							   sRightImage,
																							   sFrontImage,
	...
	if( m_pInternals->m_pWorldEnvironmentMap == osg::NullFC )
		return false;		
	return m_pInternals->AddTexture( m_pInternals->m_pWorldEnvironmentMap, VistaOpenSGMultiMaterialShaderCreator::S_nWorldEnvMapTextureId );
}
bool VistaOpenSGMultiMaterial::SetWorldEnvironmentMap( const std::string& sFolder )
{
	...
	m_pInternals->m_pWorldEnvironmentMap = m_pInternals->m_pTextureLoader->RetrieveCubemapTexture( sFolder );
										...
										const std::string& sBackImage )
{
	if( m_pInternals->m_pWorldEnvironmentMap )
	{
		m_pInternals->RemoveTexture( m_pInternals->m_pObjectEnvironmentMap, VistaOpenSGMultiMaterialShaderCreator::S_nObjectEnvMapTextureId );
		m_pInternals->m_pWorldEnvironmentMap = osg::NullFC;
	}
	if( sTopImage.empty() )
	{		
	...
	m_pInternals->m_pObjectEnvironmentMap = m_pInternals->m_pTextureLoader->RetrieveCubemapTexture( sTopImage,
																							   sBottomImage,
																							   sLeftImage,
																							   sRightImage,
																							   sFrontImage,
	...
	if( m_pInternals->m_pObjectEnvironmentMap == osg::NullFC )
		return false;		
	return m_pInternals->AddTexture( m_pInternals->m_pObjectEnvironmentMap, VistaOpenSGMultiMaterialShaderCreator::S_nObjectEnvMapTextureId );
}
...
bool VistaOpenSGMultiMaterial::SetObjectEnvironmentMap( const std::string& sFolder )
{
	...
	m_pInternals->m_pObjectEnvironmentMap = m_pInternals->m_pTextureLoader->RetrieveCubemapTexture( sFolder );
...
bool VistaOpenSGMultiMaterial::SetBumpMap( const std::string& sImageFile,
									const float nDepthMagnitude,
									const bool bSaveNormalMap )
{
	if( m_pInternals->m_pNormalMap )
	{
		m_pInternals->RemoveTexture( m_pInternals->m_pNormalMap, VistaOpenSGMultiMaterialShaderCreator::S_nNormalMapTextureId );
	...
	m_pInternals->m_pNormalMap = m_pInternals->m_pTextureLoader->RetrieveNormalMapTextureFromBumpMap( sImageFile, nDepthMagnitude, bSaveNormalMap );	
	m_pInternals->UpdateBlending();
	m_pInternals->UpdateShaders();
	if( m_pInternals->m_pNormalMap == osg::NullFC )
		return false;		
	return m_pInternals->AddTexture( m_pInternals->m_pNormalMap, VistaOpenSGMultiMaterialShaderCreator::S_nNormalMapTextureId );
}
...
bool VistaOpenSGMultiMaterial::SetGlossMap( const std::string& sImageFile )
{
	...
	{
		m_pInternals->RemoveTexture( m_pInternals->m_pGlossMap, VistaOpenSGMultiMaterialShaderCreator::S_nGlossMapTextureId );
	...
	if( m_pInternals->m_pGlossMap == osg::NullFC )
		return false;		
	return m_pInternals->AddTexture( m_pInternals->m_pGlossMap, VistaOpenSGMultiMaterialShaderCreator::S_nGlossMapTextureId );
}
...
void VistaOpenSGMultiMaterial::SetColor( const VistaColor& oColor,
							  const float nRelativeDiffuse /*= 1.0f*/,
							  const float nRelativeSpecular /*= 1.0f*/,
							  const float nRelativeAmbient /*= 1.0f*/,
...
void VistaOpenSGMultiMaterial::SetAmbientColor( const VistaColor& oColor )
{
	beginEditCP( m_pInternals->m_pMaterialChunk );
		m_pInternals->m_pMaterialChunk->setAmbient(	osg::Color4f(  oColor[0], oColor[1], oColor[2], m_pInternals->m_nOpacity ) );
...
bool VistaOpenSGMultiMaterial::Configure( const VistaPropertyList& oConfig )
{
	...
	if( oConfig.GetValue( "BLEND_MAP", sFilename ) )
	{
		...
		{
			vstr::warnp() << "VistaOpenSGMultiMaterial::NewMaterialFromConfig() failed to set blend map \""
						<< sFilename << "\"" << std::endl;
		}
	}
	if( oConfig.GetValue( "SPECULAR_MAP", sFilename ) )
	{
		...
		{
			vstr::warnp() << "VistaOpenSGMultiMaterial::NewMaterialFromConfig() failed to set specular map \""
						<< sFilename << "\"" << std::endl;
		}
	}
	...
	if( oConfig.GetValue( "NORMAL_MAP", sFilename ) )
	{
		...
		{
			vstr::warnp() << "VistaOpenSGMultiMaterial::NewMaterialFromConfig() failed to set normal map \""
						<< sFilename << "\"" << std::endl;
		}
	}
	...
	{		
		...
		if( vecFiles.size() == 6 )
		{
			bSuccess = SetWorldEnvironmentMap( vecFiles[0], vecFiles[1], vecFiles[2], vecFiles[3], vecFiles[4], vecFiles[5] );
		}
		...
		if( bSuccess == false )
		{
			vstr::warnp() << "VistaOpenSGMultiMaterial::NewMaterialFromConfig() failed to set world environment map" << std::endl;
		}		
	}
	...
	{		
		...
		if( vecFiles.size() == 6 )
		{
			bSuccess = SetObjectEnvironmentMap( vecFiles[0], vecFiles[1], vecFiles[2], vecFiles[3], vecFiles[4], vecFiles[5] );
		}
		else if( vecFiles.size() == 1 )
		{
		...
		if( bSuccess == false )
		{
			vstr::warnp() << "VistaOpenSGMultiMaterial::NewMaterialFromConfig() failed to set object environment map" << std::endl;
		}		
	}
	if( oConfig.GetValue( "GLOSS_MAP", sFilename ) )
	{
		...
		{
			vstr::warnp() << "VistaOpenSGMultiMaterial::NewMaterialFromConfig() failed to set gloss map \""
						<< sFilename << "\"" << std::endl;
		}
	}
	...
	if( oConfig.GetValue( "BLENDING_MODE", sValue ) )
	{
		...
		{
			vstr::warnp() <<  "ParseMaterialsFromFile( \""
							<< sFilename << "\" ) -- BLENDING_MODE value ["
	...
	for( VistaPropertyList::const_iterator it = oCopy.begin();
			it != oCopy.end(); ++it )
	{
		vstr::warnp() << "VistaOpenSGMultiMaterial::NewMaterialFromConfig() -- "
			"unknown property list entry [" << (*it).first << "]" << std::endl;
	}
...
bool VistaOpenSGMultiMaterial::ApplyTo( osg::GeometryPtr pGeometry, const bool bReplaceVertexColors )
{
	if( m_pInternals->VerifyShadersHaveBeenCreated() == false )
		return false;
...
bool VistaOpenSGMultiMaterial::ApplyToGeomNodesInSubtree( IVistaNode* pNode, const bool bReplaceVertexColors )
{
	std::vector< IVistaNode* > vecGeomNodes;
	VistaSceneGraph::GetAllSubNodesOfType( vecGeomNodes, VISTA_GEOMNODE, pNode );
	bool bSuccess = true;
	for( std::vector< IVistaNode* >::iterator itNode = vecGeomNodes.begin(); itNode != vecGeomNodes.end(); ++itNode )
	{
...
void VistaOpenSGMultiMaterial::CopyPropertiesToOtherMaterial( VistaOpenSGMultiMaterial* pOther ) const
{
	*pOther->m_pInternals = *m_pInternals;
	// we can copy everythink except for the actual material and its non-texture chunks
	pOther->m_pInternals->m_pMaterialChunk = osg::MaterialChunkPtr::dcast( deepClone( m_pInternals->m_pMaterialChunk ) );
	pOther->m_pInternals->m_pPolygonChunk = osg::PolygonChunkPtr::dcast( deepClone( m_pInternals->m_pPolygonChunk ) );
	...
	beginEditCP( pOther->m_pInternals->m_pMaterial );
		...
		if( pOther->m_pInternals->m_pBaseMap )
			pOther->m_pInternals->m_pMaterial->addChunk( pOther->m_pInternals->m_pBaseMap, VistaOpenSGMultiMaterialShaderCreator::S_nBaseMapTextureId );
		if( pOther->m_pInternals->m_pBlendMap )
			pOther->m_pInternals->m_pMaterial->addChunk( pOther->m_pInternals->m_pBlendMap, VistaOpenSGMultiMaterialShaderCreator::S_nBlendMapTextureId );
		if( pOther->m_pInternals->m_pSpecularMap )
			pOther->m_pInternals->m_pMaterial->addChunk( pOther->m_pInternals->m_pSpecularMap, VistaOpenSGMultiMaterialShaderCreator::S_nSpecularMapTextureId );
		if( pOther->m_pInternals->m_pNormalMap )
			pOther->m_pInternals->m_pMaterial->addChunk( pOther->m_pInternals->m_pNormalMap, VistaOpenSGMultiMaterialShaderCreator::S_nNormalMapTextureId );
		if( pOther->m_pInternals->m_pWorldEnvironmentMap )
			pOther->m_pInternals->m_pMaterial->addChunk( pOther->m_pInternals->m_pWorldEnvironmentMap, VistaOpenSGMultiMaterialShaderCreator::S_nWorldEnvMapTextureId );
		if( pOther->m_pInternals->m_pObjectEnvironmentMap )
			pOther->m_pInternals->m_pMaterial->addChunk( pOther->m_pInternals->m_pObjectEnvironmentMap, VistaOpenSGMultiMaterialShaderCreator::S_nObjectEnvMapTextureId );
		if( pOther->m_pInternals->m_pGlossMap )
			pOther->m_pInternals->m_pMaterial->addChunk( pOther->m_pInternals->m_pGlossMap, VistaOpenSGMultiMaterialShaderCreator::S_nGlossMapTextureId );
