...
public:
	...
	virtual bool Do() 
	{
		...
		//VistaTimer oTimer;		
		m_oTexture.Bind();
		//vstr::outi() << "Bind took " << vstr::formattime( 1000000.0f * oTimer.GetLifeTime(), 3 ) << "us" << std::endl;
	...
	virtual bool HandleKeyPress( int nKey, int nMod, bool bIsKeyRepeat = false ) 
	{
		switch( nKey )
		{
			...
			case 't':
			{
				...
				std::cout << "Reading RGB took " << vstr::formattime( 1000.0 * oTimer.GetLifeTime(), 3 ) << "ms" << std::endl;
				oTimer.ResetLifeTime();
				oImage.WriteToFile( "testshot.jpg" );
				std::cout << "Saving RGB took " << vstr::formattime( 1000.0 * oTimer.GetLifeTime(), 3 ) << "ms" << std::endl;
				oTimer.ResetLifeTime();

				//VistaImage oDepthImage = pWinTK->GetDepthImage( pWindow );
				//std::cout << "Reading Depth took " << vstr::formattime( 1000.0 * oTimer.GetLifeTime(), 3 ) << "ms" << std::endl;
				//oTimer.ResetLifeTime();
				//oDepthImage.ReformatImage( GL_RGB, GL_UNSIGNED_BYTE );
				//std::cout << "Convert Depth took " << vstr::formattime( 1000.0 * oTimer.GetLifeTime(), 3 ) << "ms" << std::endl;
				//oTimer.ResetLifeTime();
				//oDepthImage.WriteToFile( "testshot_d.png" );
				//std::cout << "Saving Depth took " << vstr::formattime( 1000.0 * oTimer.GetLifeTime(), 3 ) << "ms" << std::endl;
...
public:
	...
	, m_pWin2Text( NULL )
	{
		m_pSystem->GetEventManager()->AddEventHandler( this, VistaSystemEvent::GetTypeId(), VistaSystemEvent::VSE_POSTAPPLICATIONLOOP );
	...
	/*	VistaOpenSGGeometryData* pGeomData = static_cast< VistaOpenSGGeometryData* >( m_pShaderTestGeom->GetData() );
		osg::GeometryPtr pGeom = pGeomData->GetGeometry();
	...
	virtual void HandleEvent( VistaEvent *pEvent ) 
	{
		...
		//		std::cout << "Reading RGB took " << vstr::formattime( 1000.0 * oTimer.GetLifeTime(), 3 ) << "ms" << std::endl;
		//		oTimer.ResetLifeTime();
		//		oImage.WriteToFile( "screenshot" + VistaConversion::ToString(m_nFrameCount) + "_rtt.jpg" );
		//		std::cout << "Saving RGB took " << vstr::formattime( 1000.0 * oTimer.GetLifeTime(), 3 ) << "ms" << std::endl;
		//		oTimer.ResetLifeTime();
		//	}
		...
		//		std::cout << "Reading RGB took " << vstr::formattime( 1000.0 * oTimer.GetLifeTime(), 3 ) << "ms" << std::endl;
		//		oTimer.ResetLifeTime();
		//		oImage.WriteToFile( "screenshot" + VistaConversion::ToString(m_nFrameCount) + "_w.jpg" );
		//		std::cout << "Saving RGB took " << vstr::formattime( 1000.0 * oTimer.GetLifeTime(), 3 ) << "ms" << std::endl;
		//		oTimer.ResetLifeTime();
		//	}
		//}
		...
		//osg::Vec3f v3Rand( pRNG->GenerateFloat( 0, 1 ), pRNG->GenerateFloat( 0, 1 ), pRNG->GenerateFloat( 0, 1 ) );
		//beginEditCP( m_pOsgTestShader, osg::SHLChunk::ParametersFieldMask );
		//m_pOsgTestShader->setUniformParameter( "v3Color", v3Rand );
		//endEditCP( m_pOsgTestShader, osg::SHLChunk::ParametersFieldMask );

		//float a3fColor[] = { pRNG->GenerateFloat( 0, 1 ), pRNG->GenerateFloat( 0, 1 ), pRNG->GenerateFloat( 0, 1 ) };
		//VistaOpenSGPerMaterialShader::CShaderUniformParam oParam( "v3Color", a3fColor, 3 );
		//m_pShader->AddUniformParameter( oParam );
	}
...
{	
	...
	//pSG->GetRoot()->AddChild( VistaOpenSGModelOptimizer::LoadAutoOptimizedFile( pSG, "N:/InsideSceneData/Cars/mclaren.ac", "tmp" ) );
	pSG->GetRoot()->AddChild( pSG->LoadNode( "D:/dev/InsideSceneData/common/testcube.ac" ) );

	Handler* pHandler = new Handler( pSystem );

	//pSystem->GetKeyboardSystemControl()->SetDirectKeySink( new KeyHandler( pSystem, oTexture, oTexture2 ) );
