...
class VISTADEVICEDRIVERSAPI VistaSensorMeasure {
 public:
  ...
  VistaSensorMeasure(unsigned int nMeasureIdx, VistaType::microtime nTs, unsigned int nSize)
  ...
  size_t GetSerializedSize() const {
    return sizeof(VistaType::uint32) + sizeof(VistaType::microtime) * 4 +
           sizeof(VistaType::byte) * 2 + sizeof(VistaType::uint32) // for length encoding of vector
           + m_vecMeasures.size();
  }
};
...
class VISTADEVICEDRIVERSAPI VistaMeasureHistory {
 ...
 public:
  VistaMeasureHistory(VistaAtomics* pAtomics, IVistaDeviceDriver::AllocMemoryFunctor* pAlloc = NULL,
      IVistaDeviceDriver::ClaimMemoryFunctor* pFree = NULL);
  VistaMeasureHistory(const VistaMeasureHistory& oHistory, const bool bCopyData = true);
  ~VistaMeasureHistory();
  ...
  VistaType::uint32 m_state_cookie; /**< experimantal: reflects if this history is currently written
...
// SERIALIZING
////////////////////////////////////////////////////////////////////////////////////////
VISTADEVICEDRIVERSAPI IVistaSerializer& operator<<(
    IVistaSerializer& oSer, const VistaSensorMeasure& oMeasure);

VISTADEVICEDRIVERSAPI IVistaDeSerializer& operator>>(
    IVistaDeSerializer& oDeSer, VistaSensorMeasure& oMeasure);

VISTADEVICEDRIVERSAPI IVistaSerializer& operator<<(
    IVistaSerializer& oSer, const VistaMeasureHistory& oHist);

VISTADEVICEDRIVERSAPI IVistaDeSerializer& operator>>(
    IVistaDeSerializer& oDeSer, VistaMeasureHistory& oHist);
...
class VISTADEVICEDRIVERSAPI IVistaMeasureTranscode : public IVistaReflectionable {
 ...
 public:
  ...
  class VISTADEVICEDRIVERSAPI ITranscodeGet : public IVistaPropertyGetFunctor {
   public:
    ITranscodeGet(const std::string& sPropname, const std::string& sClassName,
     ...
     */
    virtual bool operator()(const IVistaPropertyAwareable&, VistaProperty&) const {
      return false;
    }
   ...
   */
  template <class T>
  class TTranscodeValueGet : public ITranscodeGet {
   public:
    TTranscodeValueGet(const std::string& sPropname, const std::string& sClassName,
        const std::string& sDescription = "<none>")
    ...
    virtual std::size_t GetReturnSize() const {
      return sizeof(T);
    }
  };
  ...
  class VISTADEVICEDRIVERSAPI ITranscodeIndexedGet : public ITranscodeGet {
   public:
    ITranscodeIndexedGet(const std::string& sPropname, const std::string& sClassName,
        const std::string& sDescription = "<none>")
        : ITranscodeGet(sPropname, sClassName, sDescription) {
    }
    ...
    virtual unsigned int GetNumberOfIndices() const {
      return UNKNOWN_NUMBER_OF_INDICES;
    }
  };

  template <class T>
  class TTranscodeIndexedGet : public ITranscodeIndexedGet {
   public:
    TTranscodeIndexedGet(const std::string& sPropname, const std::string& sClassName,
        const std::string& sDescription = "<none>")
    ...
    virtual size_t GetReturnSize() const {
      return sizeof(T);
    }
  };
  ...
  class TTranscodeMemberGet : public IVistaMeasureTranscode::TTranscodeValueGet<TGetValueType> {
   public:
    ...
    TTranscodeMemberGet(const std::string& sPropName, const std::string& sTranscodeName,
        const std::string& sHelpText, MemberVariable pmMemberVariable)
        : IVistaMeasureTranscode::TTranscodeValueGet<TGetValueType>(
    ...
    bool GetValue(const VistaSensorMeasure* pMeasure, TGetValueType& oValue) const {
      ...
      oValue                       = VistaConversion::ConvertType<TGetValueType, TMemberType>(
          pRealMeasure->*m_pmMemberVariable);
      return true;
    }
...
class VISTADEVICEDRIVERSAPI IVistaMeasureTranscoderFactory {
 public:
  ...
  virtual std::string             GetTranscoderName() const {
    return "IVistaMeasureTranscoderFactory";
  }
...
class VISTADEVICEDRIVERSAPI VistaDeviceSensor {
 ...
 private:
  ...
  unsigned int            m_nSwap1FailCount, m_nSwap2FailCount;
