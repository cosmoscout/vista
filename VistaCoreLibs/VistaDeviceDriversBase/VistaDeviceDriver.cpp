...
{
	UnregisterAspect(m_pHistoryAspect, IVistaDeviceDriver::DO_NOT_DELETE_ASPECT, IVistaDeviceDriver::FORCE_UNREGISTRATION );
...
VistaSensorMeasure *IVistaDeviceDriver::MeasureStart(unsigned int nSensorIdx, VistaType::microtime dTs, eMeasureStartBehavior getCurrent)
{
...
VistaSensorMeasure *IVistaDeviceDriver::MeasureStart( VistaDeviceSensor &Sen, VistaType::microtime dTs, eMeasureStartBehavior getCurrent)
{
...
{
	unsigned int nUpdateRateOfSensorInHz = GetFactory()->GetUpdateEstimatorFor( GetFactory()->GetTypeFor( pSensor->GetTypeHint() ) );
	...
	unsigned int nMeasureSize = GetFactory()->GetMeasureSizeFor( GetFactory()->GetTypeFor( pSensor->GetTypeHint() ) );
		                 ...
		                 VistaType::microtime nMaxHistoryAccessTimeInMsec )
{
	...
	unsigned int nNum = (unsigned int)(ceilf((float)nMaxHistoryAccessTimeInMsec/float(1000.0f/float(nUpdateRateOfSensorInHz))));
	if( 2*nNum > m_pHistoryAspect->GetUserReadHistorySize( pSensor ) )
	{
		unsigned int measureSize = GetFactory()->GetMeasureSizeFor( GetFactory()->GetTypeFor( pSensor->GetTypeHint() ) );
...
void IVistaDeviceDriver::RegisterSensorInfo( unsigned int nSensorIndex,
...
bool IVistaDeviceDriver::UnregisterAspect(IVistaDeviceDriverAspect *pDriverAspect,
		eDeleteAspectBehavior bDelete,
		eUnregistrationBehavior bForce )
{
