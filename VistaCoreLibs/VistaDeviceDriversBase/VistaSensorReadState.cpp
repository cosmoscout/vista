...
VistaSensorReadState::VistaSensorReadState( VistaDeviceSensor* pSensor,
											...
											bool bUseOwnHistoryForDeSerialization )
...
VistaType::uint32 VistaSensorReadState::GetNewAvailableMeasureCount( VistaType::uint32& nLastRead ) const
{
...
bool VistaSensorReadState::SetToLatestSample()
{
	if( m_bOwnHistory )
	{
		vstr::warnp() << "VistaSensorReadState::SetToLatestSample() called for read state with remotely received history - "
...
int VistaSensorReadState::Serialize( IVistaSerializer& oSerializer ) const
{
	...
	nWritten +=  oSerializer.WriteInt32( (VistaType::uint32)m_pHistory->GetReadBuffer().GetCurrentVal() );

	if( m_bUseIncrementalSerialization )
	{
		int nLastSerializeCount = m_mapLastSerializeMeasureCounts[&oSerializer]; // initializes to default if not yet presents

		VistaType::uint32 nNewMeasures = m_nCurrentMeasureCount - nLastSerializeCount;
		// never transmit more measures than the data size
		nNewMeasures = std::min( nNewMeasures, (VistaType::uint32)m_pHistory->GetReadBuffer().GetBufferSize() );
		nWritten += oSerializer.WriteInt32( nNewMeasures );
		
		VistaMeasureHistory::Buffer::const_iterator itMeasure = m_pHistory->GetReadBuffer().index( m_nSnapshotReadHead );
	...
	{
		const VistaMeasureHistory::Buffer::container_type& oContainer = m_pHistory->GetReadBuffer().GetRawAccess();
...
int VistaSensorReadState::DeSerialize( IVistaDeSerializer& oDeSerializer )
{
	...
	if( m_pHistory->GetWriteBuffer().GetBufferSize() != ( nClientReadSize + nDriverWriteSize ) )
	{
		if( m_bOwnHistory == false )
		{
			...
			vstr::warnp() << "[VistaSensorReadState]: Deserializing received history with different size than local history"
						" - history will be resized, but this shouldn't happen too often (once per readstate)" << std::endl;
		}
	...
	if( m_bUseIncrementalSerialization )
	{
		...
		VistaMeasureHistory::Buffer::iterator itMeasure	= m_pHistory->GetWriteBuffer().index( m_nSnapshotReadHead );
