...
bool LoadCreationMethodFromPlugin(const std::string& strPathToPlugin, VistaDriverPlugin* pStoreTo) {
...
bool LoadTranscoderFromPlugin(const std::string& strPathToPlugin, VistaDriverPlugin* pStoreTo) {
  // make a test for existence?
  ...
  if (pStoreTo->m_TranscoderDll) {
    ...
    VistaDLL::DLLSYMBOL sym =
        VistaDLL::FindSymbol(pStoreTo->m_TranscoderDll, "CreateTranscoderFactoryFactory");
    if (sym) {
      CrMethod m              = (CrMethod)sym;
...
bool DisposeTranscoderFromPlugin(VistaDriverPlugin* pPlug) {
  ...
  if (pPlug->m_TranscoderDll) {
    if (pPlug->m_pTranscoder) {
      VistaDLL::DLLSYMBOL sym =
          VistaDLL::FindSymbol(pPlug->m_TranscoderDll, "OnUnloadTranscoderFactoryFactory");
      if (sym) {
...
std::string GetDefaultPluginPath() {
  ...
  if (strPlugPath.empty())
    strPlugPath = VistaEnvironment::GetEnv("VISTA_ROOT") +
                  VistaFileSystemDirectory::GetOSSpecificSeparator() + std::string("lib") +
