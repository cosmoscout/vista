...
namespace
{
	...
	public:
		   ...
		   , m_nMsecsSleep(0)
		{
			...
			double dRate = (1.0/(2.0*double(nMax)))*1000.0; // create a loop-time to match a half of the minimum latency
...
VistaDriverManager::~VistaDriverManager()
{
	...
	for(VistaDriverMap::iterator it = m_Drivers.begin();
		it != m_Drivers.end(); ++it)
	{
		...
		if( pConAsp )
		{
			IVistaExplicitCallbackInterface *pI
							= m_pConnUpdater->RemConnectionUpdate(
									pConAsp->GetConnection(
											pConAsp->GetUpdateConnectionIndex()));
			delete pI;
		}
	}
...
bool VistaDriverManager::InitPlugin( const std::string &driver_name, const std::string &search_dir_root )
{
	...
	vstr::outi() << "VistaDriverManager::InitPlugin( " << driver_name << ") -- " << "plug path resolved to: " << strPlugPath << std::endl;
	...
	std::string plugin = VddUtil::GetPlugPrefix() + driver_name + VddUtil::GetPlugInfix() + VddUtil::GetPlugPostfix();

	// try to load transcoder for this device
	dsc.m_TranscoderSourceName = strPlugPath + VistaFileSystemDirectory::GetOSSpecificSeparator() + VddUtil::GetTranscoderLibName( driver_name );

	dsc.m_DriverCreationSourceName = strPlugPath + VistaFileSystemDirectory::GetOSSpecificSeparator() + plugin;
...
bool VistaDriverManager::LoadPlugin( PlugDesc &dsc ) const
{
	...
	{
		vstr::errp() << "Could not load transcoder by name: [" << dsc.m_TranscoderSourceName << "]" << std::endl;
		return false;
	}
	...
	if( VddUtil::LoadCreationMethodFromPlugin( dsc.m_DriverCreationSourceName, &dsc.m_plg ) )
	{
		...
		vstr::outi() << "Loaded factory from ["
				  ...
				  << dsc.m_plg.m_pMethod
				  << " with name ["
				  << dsc.m_plg.m_strDriverClassName
				  << "]"
				  << std::endl;
#endif
		return true;
...
bool VistaDriverManager::StartAsyncDriverDispatch( eLauchOption launch_option )
{
				...
				it != m_Drivers.end(); ++it)
	{
		...
		if( pConAsp )
		{
			...
			{
				vstr::errp() << "No connection given for connection based aspect driver [" 
							<< (*it).first << "]" << std::endl;
			}
		}
		else
		{
			...
			else
			{
				...
				{
					vstr::erri() << "Driver [" << (*it).first << "] does not offer a thread aspect and user asked not to launch one." << std::endl;
				}
			}
		}
	}
