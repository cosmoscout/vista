...
class VISTADEVICEDRIVERSAPI IVistaDeviceDriver {
 ...
 public:
   ...
   * @param nMaxHistoryAccessTimeInMsec this is the maximum time ANY reader on the
                                  history is accessing the history. For
                                                                  example if you know that you want
   to render with 30Hz on your graphical output and all readers live in the rendering thread,
   then 33.3 is the value to set here.
   * @param nMaxSlotsToRead define the maximum number of slots to read within the time frame
                      specified by nMaxHistoryAccessTimeInMsec for ALL readers of the
                                          history. If you are doing latest-sampling, a value of 1 is
   ok, it must not be 0.
   * @see IVistaDriverCreationMethod::GetUpdateEstimatorFor()
   */
  bool SetupSensorHistory(VistaDeviceSensor* pSensor, unsigned int nMaxSlotsToRead,
      VistaType::microtime nMaxHistoryAccessTimeInMsec);

  bool RequestHistorySize(VistaDeviceSensor* pSensor, unsigned int nMaxSlotsToRead,
      VistaType::microtime nMaxHistoryAccessTimeInMsec);
  ...
  class SensorInfo {
   public:
    ...
    SensorInfo(const std::string& sensor_name, const std::string& type_hint,
        size_t sensor_history_size, VistaType::microtime access_time_duration_in_ms)
    ...
    bool IsValid() const {
      return m_sensorHistorySize != UNKNOWN_SIZE && m_MaxAccessTimeInMs != UNKNOWN_ACCESS_DURATION;
    }

    std::string          m_sensorName, m_sensorTypeHint;
    size_t               m_sensorHistorySize;
    VistaType::microtime m_MaxAccessTimeInMs;
  };

  void RegisterSensorInfo(unsigned int nSensorIndex, const SensorInfo& sensor_info);
  ...
  enum eDeleteAspectBehavior { DELETE_ASPECT, DO_NOT_DELETE_ASPECT };

  enum eUnregistrationBehavior { FORCE_UNREGISTRATION, DO_NOT_FORCE_UNREGISTRATION };
  /**
   ...
   * @return iff the driver tries to attach to a stream only without trying to re-initialize the
   * device
   */
 ...
 protected:
  ...
  // ###################################################################
 public:
  enum eMeasureStartBehavior { RETURN_NULL, RETURN_CURRENT_SLOT };
  /**
   ...
   *         was not found.)
   */
  VistaSensorMeasure* MeasureStart(unsigned int nSensorIdx, VistaType::microtime dTs,
      eMeasureStartBehavior getCurrent = RETURN_NULL);
  VistaSensorMeasure* MeasureStart(
      VistaDeviceSensor&, VistaType::microtime dTs, eMeasureStartBehavior getCurrent = RETURN_NULL);
...
* @param pFac a pointer to the transcoder factory to use for this sensor type
 ...
 private:
  ...
  struct _sL {
    ...
    _sL(IVistaMeasureTranscoderFactory* pFac, unsigned int nMeasureSize,
    ...
    unsigned int                    m_nMeasureSize, m_nUpdateEstimator;
  };
