...
void VistaDriverConnectionAspect::SetConnection(unsigned int nIndex,
												 ...
												 ReplaceBehavior bCanBeReplaced,
												 ForceAlreadySetBehavior bForce,
												 ConnectionCollectBehavior bCollectConnection)
{
	...
	if(m_vecConnections[nIndex] == NULL)
		...
		m_vecConnections[nIndex] = new ConnectionHlp(this, pConnection, NULL, NULL, bCanBeReplaced, bCollectConnection);
	else
	{
		...
		if(m_vecConnections[nIndex]->m_pConnection)
		{
			...
			// below
			if(m_vecConnections[nIndex]->m_bCanBeReplaced == ALLOW_CONNECTION_REPLACEMENT || (bForce == REPLACE_ALSO_WHEN_ALREADY_SET) )
