...
bool VistaDriverMeasureHistoryAspect::RegisterSensor(VistaDeviceSensor* pSen,
    IVistaDeviceDriver::AllocMemoryFunctor* amf, IVistaDeviceDriver::ClaimMemoryFunctor* cmf,
    bool bReRegister) {
...
void VistaDriverMeasureHistoryAspect::MeasureStart(
    VistaDeviceSensor* pSen, VistaType::microtime dTs) {
  ...
  // create an entry
  _sL l(&pSen->GetDriverMeasures(), (*cit).second.m_amf, (*cit).second.m_cmf,
      (*cit).second.m_bInitialized);
...
bool VistaDriverMeasureHistoryAspect::SetHistorySize(VistaDeviceSensor* pSen,
    unsigned int nUserSize, unsigned int nDriverSize, unsigned int nSlotLength) {
  HISTORY::iterator it = m_mapHistories.find(pSen);
  if (it != m_mapHistories.end()) {
...
unsigned int VistaDriverMeasureHistoryAspect::GetDriverWriteHistorySize(
    VistaDeviceSensor* pSen) const {
  HISTORY::const_iterator it = m_mapHistories.find(pSen);
  if (it != m_mapHistories.end()) {
...
unsigned int VistaDriverMeasureHistoryAspect::GetUserReadHistorySize(
    VistaDeviceSensor* pSen) const {
  HISTORY::const_iterator it = m_mapHistories.find(pSen);
  if (it != m_mapHistories.end()) {
