...
VistaDriverMap::~VistaDriverMap() {
  for (CRMAP::iterator it1 = m_mapCreationMethods.begin(); it1 != m_mapCreationMethods.end();
       ++it1) {
    (*it1).second->OnUnload();
    delete (*it1).second;
  }
...
bool VistaDriverMap::AddDeviceDriver(const std::string& sName, IVistaDeviceDriver* pDriver) {
...
bool VistaDriverMap::RegisterDriverCreationMethod(
    const std::string& sTypeName, IVistaDriverCreationMethod* pMethod, bool bForce) {
...
bool VistaDriverMap::UnregisterDriverCreationMethod(
    const std::string& sTypeName, bool bDeleteMethod) {
...
IVistaDriverCreationMethod* VistaDriverMap::GetDriverCreationMethod(
...
void VistaDriverMap::Purge(bool bDelete) {
  if (bDelete) {
    ...
    for (CRMAP::iterator it0 = m_mapCreationMethods.begin(); it0 != m_mapCreationMethods.end();
         ++it0)
      delete (*it0).second;
  }
