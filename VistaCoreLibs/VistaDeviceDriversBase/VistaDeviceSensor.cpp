 ...
 public:
  ...
  VistaType::uint32 AtomicSet(SWAPADDRESS v1, VistaType::uint32 v2) {
    ...
    *((VistaType::uint32*)v1) = v2; // store new one, we assume that this works using
    ...
    (defined(__i386__) || defined(__x86_64__) || defined(__LP32__) || defined(__LP64__))
    return intel_gnuc_atomicswap(
        (VistaType::uint32*)v1, (VistaType::uint32)v2); // atomic_xchg(v1, v2);
#else
    VistaSemaphoreLock l(m_lock);
    VistaType::uint32  vOld   = *((VistaType::uint32*)v1); // assign old value
    *((VistaType::uint32*)v1) = v2; // store new one, we assume that this works using
...
#elif defined(__GNUC__) &&                                                                         \
    (defined(__i386__) || defined(__x86_64__) || defined(__LP32__) || defined(__LP64__))
    return intel_gnuc_atomicswap64(
        (VistaType::uint64*)v1, (VistaType::uint64)v2); // atomic_xchg(v1, v2);
#else
    VistaSemaphoreLock l(m_lock);
    VistaType::uint64  vOld   = *((VistaType::uint64*)v1); // assign old value
    *((VistaType::uint64*)v1) = v2; // store new one, we assume that this works using
  ...
  static VistaType::uint32 intel_gnuc_atomicswap(
      volatile VistaType::uint32* mem, VistaType::uint32 val) {
    VistaType::uint32 prev = val;
    asm volatile("lock; xchgl %0, %1" : "=r"(prev) : "m"(*(mem)), "0"(prev) : "memory");
    return prev;
  }

  static VistaType::uint64 intel_gnuc_atomicswap64(
      volatile VistaType::uint64* mem, VistaType::uint64 val) {
    VistaType::uint64 prev = val;
    asm volatile("lock; xchgq %0, %1" : "=r"(prev) : "m"(*(mem)), "0"(prev) : "memory");
    return prev;
  }
#endif
...
const VistaSensorMeasure* VistaMeasureHistory::GetPast(
    unsigned int nNum, const VistaType::uint32 nReadHead) const {
  if (nReadHead == ~0u) {
...
VistaMeasureHistory::VistaMeasureHistory(VistaAtomics* pAtomics,
    IVistaDeviceDriver::AllocMemoryFunctor* pAlloc, IVistaDeviceDriver::ClaimMemoryFunctor* pFree)
    : m_state_cookie(0)
    , m_rbHistory(1024)
...
bool VistaMeasureHistory::Resize(
    unsigned int nClientReadSize, unsigned int nDriverWriteSize, unsigned int nMeasureSize) {
  if (nMeasureSize == 0)
...
bool IVistaMeasureTranscoderFactory::OnUnload() {
#if defined(DEBUG)
  vstr::outi() << "IVistaMeasureTranscoderFactory::OnUnload(" << typeid(*this).name() << ")"
               << std::endl;
#endif
  ...
  std::list<IVistaPropertyGetFunctor*> liGetterByClass =
      pReg->GetGetterByClass(GetTranscoderName());
  std::list<IVistaPropertySetFunctor*> liSetterByClass =
      pReg->GetSetterByClass(GetTranscoderName());
...
int DefaultBinaryDumpSerializeAdapter::SerializeMeasureHeader(
    IVistaSerializer& output, const VistaSensorMeasure& output_measure) {
  int nret = 0;
...
int DefaultBinaryDumpSerializeAdapter::DeSerializeMeasureHeader(
    IVistaDeSerializer& input, VistaSensorMeasure& input_measure) {
  int nret = 0;
...
IVistaMeasureTranscode::ITranscodeGet* IVistaMeasureTranscode::GetMeasureProperty(
    const std::string& strPropName) const {
  VistaPropertyFunctorRegistry* pReg = VistaPropertyFunctorRegistry::GetSingleton();
  ...
  if (m_MpList.empty()) {
    ...
    m_MpList.remove_if(VistaAspectsComparisonStuff::ObjEqualsCompare<std::string>(
        SuperClass::GetReflectionableType()));
  }

  // use the cached version for the get-functor
  return dynamic_cast<ITranscodeGet*>(
      pReg->GetGetFunctor(strPropName, GetReflectionableType(), m_MpList));
}
...
class VistaMeasureIndexTranscode
    : public IVistaMeasureTranscode::TTranscodeValueGet<VistaType::uint32> {
 public:
  VistaMeasureIndexTranscode()
      : IVistaMeasureTranscode::TTranscodeValueGet<unsigned int>(
            "MEASUREINDEX", sSReflectionTypeVSD, "absolute index of measurement") {
  }
  ...
  virtual bool GetValue(const VistaSensorMeasure* pMeasure, VistaType::uint32& dIndex) const {
    dIndex = pMeasure->m_nMeasureIdx;
    return true;
  }
};

class VistaMeasureTimeStampTranscode
    : public IVistaMeasureTranscode::TTranscodeValueGet<VistaType::microtime> {
 public:
  VistaMeasureTimeStampTranscode()
      : IVistaMeasureTranscode::TTranscodeValueGet<VistaType::microtime>(
            "TIMESTAMP", sSReflectionTypeVSD, "timestamp of raw measurement") {
  }
  ...
  virtual bool GetValue(const VistaSensorMeasure* pMeasure, VistaType::microtime& dStamp) const {
    dStamp = pMeasure->m_nMeasureTs;
    return true;
  }
};

class VistaMeasureTimeStampSecTranscode
    : public IVistaMeasureTranscode::TTranscodeValueGet<double> {
 public:
  ...
  virtual bool GetValue(const VistaSensorMeasure* pMeasure, double& dStamp) const {
...
class VistaMeasureDeliveryTranscode : public IVistaMeasureTranscode::TTranscodeValueGet<double> {
 public:
  VistaMeasureDeliveryTranscode()
      : IVistaMeasureTranscode::TTranscodeValueGet<double>(
            "DELIVERY_TIMESTAMP", sSReflectionTypeVSD, "the timestamp of delivery to the history") {
  }
  virtual double GetValue(const VistaSensorMeasure* pMeasure) const {
...
class VistaMeasureSwapTimeTranscode : public IVistaMeasureTranscode::TTranscodeValueGet<double> {
 public:
  VistaMeasureSwapTimeTranscode()
      : IVistaMeasureTranscode::TTranscodeValueGet<double>(
            "SWAPTIME", sSReflectionTypeVSD, "time in between locks (lock latency)") {
  }
...
IVistaPropertyGetFunctor* SaGetter[] = {new VistaMeasureIndexTranscode,
    new VistaMeasureTimeStampTranscode, new VistaMeasureSwapTimeTranscode,
    new VistaMeasureTimeStampSecTranscode, NULL};
} // namespace
...
bool VistaDeviceSensor::AdvanceMeasure() {
  ...
  bool bIs64bit =
      (sizeof(TVistaRingBuffer<VistaSensorMeasure>::size_type) == sizeof(VistaType::uint64));
  if (bIs64bit) {
    while (m_pAtomics->AtomicSet64((VistaAtomics::SWAPADDRESS)&cur, next) == cur)
      ++m_nSwap1FailCount;
  } else {
    while (m_pAtomics->AtomicSet((VistaAtomics::SWAPADDRESS)&cur, (VistaType::uint32)next) ==
           (VistaType::uint32)cur)
      ++m_nSwap1FailCount;
  }
  ...
  VistaMeasureHistory::Buffer::iterator it(
      oBuffer.index(m_rbSensorHistory.GetCurrentReadHead())); // of the current slot being written
...
unsigned int VistaDeviceSensor::GetNewMeasureCount(unsigned int& lastRead) const {
  ...
  unsigned int nNewMeasureCount =
      std::min<unsigned int>(nMIndex - lastRead, m_rbSensorHistory.m_nClientReadSize);
...
IVistaSerializer& operator<<(IVistaSerializer& oSer, const VistaSensorMeasure& oMeasure) {
  oSer << oMeasure.m_nEndianess << oMeasure.m_nBusWidth << oMeasure.m_nMeasureIdx
       << oMeasure.m_nMeasureTs << oMeasure.m_nSwapTime << oMeasure.m_nDeliverTs
...
IVistaDeSerializer& operator>>(IVistaDeSerializer& oDeSer, VistaSensorMeasure& oMeasure) {
  oDeSer >> oMeasure.m_nEndianess >> oMeasure.m_nBusWidth;
  ...
  oDeSer >> oMeasure.m_nMeasureIdx >> oMeasure.m_nMeasureTs >> oMeasure.m_nSwapTime >>
      oMeasure.m_nDeliverTs >> oMeasure.m_nAverageDeliverTime;
...
IVistaSerializer& operator<<(IVistaSerializer& oSer, const VistaMeasureHistory& oHist) {
  ...
  oSer << oHist.m_nClientReadSize << oHist.m_nDriverWriteSize
  ...
  const TVistaRingBuffer<VistaSensorMeasure>::container_type& vCont =
      oHist.GetReadBuffer().GetRawAccess();
  ...
  for (TVistaRingBuffer<VistaSensorMeasure>::container_type::const_iterator cit = begin; cit != end;
       ++cit) {
    oSer << *cit;
  }
  return oSer;
...
IVistaDeSerializer& operator>>(IVistaDeSerializer& oDeSer, VistaMeasureHistory& oHist) {
  ...
  oDeSer >> oHist.m_nClientReadSize >> oHist.m_nDriverWriteSize;

  VistaType::uint32 nFirst, nNext, nCurrent, nSize;
  nFirst = nNext = nCurrent = nSize = 0;
  oDeSer >> nSize >> nFirst >> nNext >> nCurrent;
