...
{
	if( !m_pVAO || !m_pParticleDataTexture 
		|| m_iParticleCount==0
		|| m_aParticleDataTextureSize[0]<=0 
		|| m_aParticleDataTextureSize[1]<=0 )
		return;
	...
	VistaTexture* pRadiiTexture = 
		( m_pParticleRadiiTexture ) ?
		( m_pParticleRadiiTexture ) :
	( m_pDefaultParticleRadiiTexture );
	...
	case VistaParticleRenderingProperties::DM_BUMPED_BILLBOARDS:
		RenderBillboards( 
			m_pLitBBShaders[m_pProperties->GetLightingMode()], 
			m_pSphereNormalsTexture );
		break;

	case VistaParticleRenderingProperties::DM_BUMPED_BILLBOARDS_DEPTH_REPLACE:
		RenderBillboards( 
			m_pDepthShaders[m_pProperties->GetLightingMode()], 
			m_pSphereNormalsTexture );
		break;
	}
...
void VistaParticleRenderingCore::Observe( 
...
void VistaParticleRenderingCore::ObserveableDelete( 
	IVistaObserveable* pObserveable, int nTicket )
{ }
void VistaParticleRenderingCore::ReleaseObserveable( 
	IVistaObserveable* pObserveable, int nTicket )
{ }
...
{
	if( m_pPointShader || m_pSmokeShader || m_pBillboardShader || 
		m_pLitBBShaders[0] || m_pLitBBShaders[1] || m_pLitBBShaders[2] ||
		m_pDepthShaders[0] || m_pDepthShaders[1] || m_pDepthShaders[2]  )
		return false;
	...
	if( strPointVert.empty() || strPointFrag.empty() || 
		...
		strBBVert.empty()    || strBBFrag.empty()    ||
		strLitVert.empty()   || strLitFrag.empty()   || 
		strDepthVert.empty() || strDepthFrag.empty() ||
		strHaloVert.empty()  || strHaloFrag.empty()  ||
		strLighting[0].empty() || strLighting[1].empty() || strLighting[2].empty() )
	{
...
{	
	GLenum eMinFilter = m_pProperties->GetGenerateMipmaps() ? 
	...
	if( m_pSphereIlluminationTexture )
	{
		float* pData = VistaOGLUtils::CreateSphereIlluminationTextureData( 
			iResolution, iResolution );

		m_pSphereIlluminationTexture->Bind();
		glTexImage2D( m_pSphereIlluminationTexture->GetTarget(),
			0, GL_LUMINANCE_ALPHA, iResolution, iResolution, 
			0, GL_LUMINANCE_ALPHA, GL_FLOAT, pData );
	...
	{
		float* pData = VistaOGLUtils::CreateGaussianBlendingTextureData( 
			iResolution, iResolution );

		m_pGaussianBlendingTexture->Bind();
		glTexImage2D( m_pGaussianBlendingTexture->GetTarget(),
			0, GL_ALPHA, iResolution, iResolution, 
			0, GL_ALPHA, GL_FLOAT, pData );
		if( m_pProperties->GetGenerateMipmaps() )
	...
	{
		float* pData = VistaOGLUtils::CreateSphereNormalsTextureData(
			iResolution, iResolution, true );

		m_pSphereNormalsTexture->Bind();
		glTexImage2D( m_pSphereNormalsTexture->GetTarget(),
			0, GL_RGBA, iResolution, iResolution, 
			0, GL_RGBA, GL_FLOAT, pData );
		if( m_pProperties->GetGenerateMipmaps() )
...
void VistaParticleRenderingCore::UpdateUniformVariables( VistaGLSLShader* pShader )
{
	...
	if( iUniformLocation >= 0 )
	{
		glUniform2f( iUniformLocation, m_aLookupRange[0],
			 1.0f/(m_aLookupRange[1]-m_aLookupRange[0]) );
	}
...
void VistaParticleRenderingCore::RenderPoints()
{
	...
	//VistaTexture* pLookupTexture = ( m_pLookUpTexture )?( m_pLookUpTexture ):( m_pDefaultLookUpTexture );
...
void VistaParticleRenderingCore::RenderTransparentBillboards()
{
	...
	if( m_pProperties->GetBlendingMode() ==
