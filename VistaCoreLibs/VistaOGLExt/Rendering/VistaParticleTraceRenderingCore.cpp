...
void VistaParticleTraceRenderingCore::Draw()
{
	...
	case VistaParticleRenderingProperties::DM_BUMPED_BILLBOARDS:
		RenderBodies( 
			m_pBumpedShaders[ m_pProperties->GetLightingMode() ], 
			m_pTubeNormalsTexture );
		RenderCaps( 
			m_pBumpedShaders[ m_pProperties->GetLightingMode() ],
			m_pCapNormalsTexture );
		break;
...
void VistaParticleTraceRenderingCore::Observe( 
...
VistaGLSLShader* VistaParticleTraceRenderingCore::CreateShader( 
	const std::string& strVertexShaderName, 
	const std::string& strFragmentShaderName, 
	const std::string& strLingtingShaderName )
{
		...
		(!strLingtingShaderName.empty() && strLighting.empty() ) )
	{
		...
		if( strVert.empty() ) vstr::erri() << "can't find " << strVertexShaderName << endl;
		if( strFrag.empty() ) vstr::erri() << "can't find " << strFragmentShaderName << endl;

		if( !strLingtingShaderName.empty() && strLighting.empty() )
			if( strFrag.empty() ) vstr::erri() << "can't find " << strLingtingShaderName << endl;
...
{	
	...
	if( m_pBlendingTexture )
	{
		...
		m_pBlendingTexture->Bind();
		glTexImage2D( m_pBlendingTexture->GetTarget(),
	...
	if( m_pIlluminationTexture )
	{
		...
		m_pIlluminationTexture->Bind();
		glTexImage2D( m_pIlluminationTexture->GetTarget(),
	...
	{
		float* pData = VistaOGLUtils::CreateTubeNormalsTextureData(
			iResolution, iResolution, true );

		m_pTubeNormalsTexture->Bind();
		glTexImage2D( m_pTubeNormalsTexture->GetTarget(),
	...
	if( m_pCapNormalsTexture )
	{
		...
		m_pCapNormalsTexture->Bind();
		glTexImage2D( m_pCapNormalsTexture->GetTarget(),
			0, GL_RGBA, iResolution, iResolution, 
			0, GL_RGBA, GL_FLOAT, pData );
		if( m_pProperties->GetGenerateMipmaps() )
...
void VistaParticleTraceRenderingCore::RenderBodies( VistaGLSLShader* pShader, VistaTexture* pTexture)
{
	if( !pShader || !pTexture )
		return;
...
void VistaParticleTraceRenderingCore::RenderCapsIntoDeapthTexture()
{
	...
	if( aViewport[2] > m_aDepthTextureSize[0] || aViewport[3] > m_aDepthTextureSize[1] )
	{
		...
		m_pDepthTexture->Bind();
		glTexImage2D( 
			m_pDepthTexture->GetTarget(),
			0, GL_DEPTH_COMPONENT, 
			m_aDepthTextureSize[0], 
			m_aDepthTextureSize[1], 
			0, GL_DEPTH_COMPONENT, 
			GL_FLOAT, NULL );
		m_pDepthTexture->Unbind();
	}
