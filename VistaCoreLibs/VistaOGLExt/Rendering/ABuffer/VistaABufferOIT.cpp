...
public:
	...
	virtual bool Do()
	{
		if(m_bPrerender) { m_pOIT->ClearABuffer();  }
		else			 { m_pOIT->DisplayABuffer(); }
...
/* PUBLIC INTERFACE	                                                          */
/******************************************************************************/
bool VistaABufferOIT::Init(
	EABufferImplementation eABufferImp,
	unsigned int uiFragmentesPerPixel,
...
void VistaABufferOIT::ClearABuffer()
{
	...
	for( size_t n=0; n < m_vecShader.size(); ++n)
	{
		...
		{
			glProgramUniform4iv( 
				m_vecShader[n]->GetProgram(), iLoc, 1, m_aViewport );
		}
	}
...
{
	if( m_pDisplayShader==NULL ) return;
...
/* PRIVATE INTERFACE	                                                      */
/******************************************************************************/
bool VistaABufferOIT::InitABuffer(
	unsigned int uiFragmentesPerPixel,
...
bool VistaABufferOIT::InitShader()
{
		...
		strShader[4].empty() )
	{
		...
		if( strShader[1].empty() ) vstr::erri() << "Can't find " << g_strShader_Default_Vert  << std::endl;
		if( strShader[2].empty() ) vstr::erri() << "Can't find " << g_strShader_Default_Frag  << std::endl;
		if( strShader[3].empty() ) vstr::erri() << "Can't find " << g_strShader_StoreFrag_Aux << std::endl;
		if( strShader[4].empty() ) vstr::erri() << "Can't find " << g_strShader_Lighting_Aux  << std::endl;
		return false;
	}
