...
{
	if( m_pClearShader == NULL ) return false;
...
bool IVistaABuffer::AddDataField( unsigned int nSizeInByte, const std::string& strName )
{
	if( !m_bIsInitialized )
	{
		vstr::errp() << "[IVistaABuffer] - ABuffer must be initialized before DataFields can be added."  << endl;
		return false;
	}
	...
	ResizeBuffer( oDataField.m_uiBuffer, oDataField.m_addrBuffer, 
	              oDataField.m_nSizeInByte * m_uiBufferSize );
...
void IVistaABuffer::ResizeDataFileds()
{
	...
	{
		ResizeBuffer(	
			m_vecDataFileds[n].m_uiBuffer,
			m_vecDataFileds[n].m_addrBuffer, 
			m_vecDataFileds[n].m_nSizeInByte * m_uiBufferSize );
	}
}

void IVistaABuffer::ResizeBuffer(	GLuint uiBufferID, 
									...
									unsigned int iSize )
{
	...
	glGetBufferParameterui64vNV(	GL_TEXTURE_BUFFER, 
