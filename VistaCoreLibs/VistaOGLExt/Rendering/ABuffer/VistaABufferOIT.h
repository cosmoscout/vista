...
public: 
	 ...
	 */
	bool Init( EABufferImplementation eABufferImp, unsigned int uiFragmentesPerPixel, unsigned int uiPageSize = 4 );
	...
	/**
	 * This method should be called to clear the A-Buffer ( delete all Fragments that are stored in the A-Buffer ).
	 * Must be called before rendering the Scene.
	 * Make sure GL_VIEWPORT is set correct when calling this method,
	 * because this method will resize the A-Buffer, to match the ViewPort size.
	 */
	void ClearABuffer();
