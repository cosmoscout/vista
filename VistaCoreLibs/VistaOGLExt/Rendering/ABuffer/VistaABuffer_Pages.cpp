...
VistaABufferPages::~VistaABufferPages() {
  if (m_uiPagePointerBuffer)
    glDeleteBuffers(1, &m_uiPagePointerBuffer);
  if (m_uiNextPointerBuffer)
    glDeleteBuffers(1, &m_uiNextPointerBuffer);
  if (m_uiSemaphoreBuffer)
    glDeleteBuffers(1, &m_uiSemaphoreBuffer);
  if (m_uiFragCountBuffer)
    glDeleteBuffers(1, &m_uiFragCountBuffer);
  if (m_uiAtomicPageCounter)
    glDeleteBuffers(1, &m_uiAtomicPageCounter);
}
...
bool VistaABufferPages::SetABufferSize(unsigned int uiWidth, unsigned int uiHeight) {
  ...
  IVistaABuffer::ResizeBuffer(
      m_uiNextPointerBuffer, m_addrNextPointerBuffer, sizeof(unsigned int) * m_nNumPages);
...
bool VistaABufferPages::SetFragmenstPerPixel(unsigned int uiFragmenstPerPixel) {
  if (uiFragmenstPerPixel % m_nPageSize != 0) {
    vstr::errp() << "[VistaABufferPages] uiFragmenstPerPixel must be a multiple of nPageSize "
                 << endl;
    return false;
  }
...
bool VistaABufferPages::InitBuffer() {
  ...
  if (!m_uiPagePointerBuffer)
    glGenBuffers(1, &m_uiPagePointerBuffer);
  if (!m_uiNextPointerBuffer)
    glGenBuffers(1, &m_uiNextPointerBuffer);
  if (!m_uiSemaphoreBuffer)
    glGenBuffers(1, &m_uiSemaphoreBuffer);
  if (!m_uiFragCountBuffer)
  ...
  IVistaABuffer::ResizeBuffer(
      m_uiSemaphoreBuffer, m_addrSemaphoreBuffer, sizeof(unsigned int) * m_uiWidth * m_uiHeight);
...
void VistaABufferPages::CreateAtomicCounter() {
  if (!m_uiAtomicPageCounter)
    glGenBuffers(1, &m_uiAtomicPageCounter);
