...
{
	if(iVolumeId < 0 || iVolumeId >= m_vecVolumeTextures.size() || iChannelIdx < 0 || iChannelIdx > 3 )
	{
		return false;
	}
...
bool VistaVolumeRaycasterCore::SetVolumeTexture( VistaTexture* pVolumeTexture, int iVolumeId /* = 0 */ )
{
...
bool VistaVolumeRaycasterCore::SetLookupTexture( VistaTexture* pLookupTexture, int iLookupId /* = 0 */ )
{
...
bool VistaVolumeRaycasterCore::SetLookupRange( float* pLookupRange, int iRangeId /* = 0 */, unsigned int uiNumComponents /* = 2 */, unsigned int uiNumVectors /* = 1 */ )
{
	...
	std::memcpy(
		m_vecLookupRanges[iRangeId].m_pRangeValues,
		pLookupRange,
...
bool VistaVolumeRaycasterCore::GetLookupRange( float* pLookupRange, int iRangeId /* = 0 */, unsigned int uiNumComponents /* = 2 */, unsigned int uiNumVectors /* = 1 */ ) const
{
	if( iRangeId < 0 || iRangeId >= static_cast<int>( m_vecLookupRanges.size() ) )
		return false;
...
bool VistaVolumeRaycasterCore::BuildShaders( const bool bPreserveUserUniforms )
{
	...
	if(    strGeneralVS.empty() || strRayGenFS.empty() || strOutsideFS.empty()
		|| strInsideFS.empty() || strLoopFS.empty() )
	{
		vstr::errp() << "[VistaVolumeRaycasterCore] One of the shader files \
						could not be opened or is empty." << std::endl;
		return false;
	}
...
void VistaVolumeRaycasterCore::RefreshUniformsForShader( VistaGLSLShader* pShader )
{
	...
	{
		pShader->SetUniform(
			pShader->GetUniformLocation( "u_gradient_sampler" ),
			m_vecVolumeTextures[m_iGradientVolumeId].m_iTextureUnit );
	}
	pShader->SetUniform(
		pShader->GetUniformLocation( "u_gradient_channel" ),
		m_iGradientChannelIdx );

	pShader->SetUniform(
		pShader->GetUniformLocation( "u_step_size" ),
		m_fStepSize );
	pShader->SetUniform(
		pShader->GetUniformLocation( "u_alpha_saturation_limit" ),
		m_fAlphaSaturationLimit );
	pShader->SetUniform(
		pShader->GetUniformLocation( "u_alpha_compensation" ),
		m_fAlphaCompensationFactor );

	if( m_bPerformRayJittering )
	{
		pShader->SetUniform(
			pShader->GetUniformLocation( "u_jitter_texture_size" ),
	...
	{
		pShader->SetUniform(
			pShader->GetUniformLocation( m_vecVolumeTextures[i].m_strSamplerName ),
	...
	{
		pShader->SetUniform(
			pShader->GetUniformLocation( m_vecLookupTextures[i].m_strSamplerName ),
			m_vecLookupTextures[i].m_iTextureUnit );
	}

	for( size_t i=0; i<m_vecLookupRanges.size(); ++i )
	{
		if(    m_vecLookupRanges[i].m_uiNumComponents > 0
			&& m_vecLookupRanges[i].m_uiNumVectors > 0 )
		{
			pShader->SetUniform(
				pShader->GetUniformLocation( m_vecLookupRanges[i].m_strUniformName ),
				m_vecLookupRanges[i].m_uiNumComponents,
				m_vecLookupRanges[i].m_uiNumVectors,
...
void VistaVolumeRaycasterCore::UpdateDepthTexture()
{
	if( m_bPerformDepthTest )
	{
		...
		glCopyTexSubImage2D( GL_TEXTURE_RECTANGLE, 0, 0, 0, 0, 0, m_aViewportSize[0], m_aViewportSize[1] );
...
void VistaVolumeRaycasterCore::PerformRaycasting()
{
	...
	for( size_t i=0; i<m_vecLookupTextures.size(); ++i )
	{
		...
		{
			m_vecLookupTextures[i].m_pTexture->Bind(
	...
	else
	{
		...
		printf("Upload %f %f %f \n",aTexCoordsForViewer[0],aTexCoordsForViewer[1],aTexCoordsForViewer[2]);
		m_pRaycastInsideShader->SetUniform(m_pRaycastInsideShader->GetUniformLocation( "tex_coords_for_viewer" ), 3, 1, aTexCoordsForViewer );
...
bool VistaVolumeRaycasterCore::IsInsideVolume( float aTexCoordsForViewer[3] )
{
	...
	const bool bIsInVolume =
		   std::abs( v3LocalViewPosition[0] ) <= 0.5f * m_aDataExtents[0]
		&& std::abs( v3LocalViewPosition[1] ) <= 0.5f * m_aDataExtents[1]
		&& std::abs( v3LocalViewPosition[2] ) <= 0.5f * m_aDataExtents[2];

	if( bIsInVolume )
	{
		aTexCoordsForViewer[0] = ( v3LocalViewPosition[0] + 0.5f * m_aDataExtents[0] ) / m_aDataExtents[0];
		aTexCoordsForViewer[1] = ( v3LocalViewPosition[1] + 0.5f * m_aDataExtents[1] ) / m_aDataExtents[1];
		aTexCoordsForViewer[2] = ( v3LocalViewPosition[2] + 0.5f * m_aDataExtents[2] ) / m_aDataExtents[2];
	}
...
bool VistaVolumeRaycasterCore::CheckForViewportResize()
{
	...
	if( m_aViewportSize[0] != aViewport[2]+aViewport[0] || m_aViewportSize[1] != aViewport[3]+aViewport[1] )
	{
...
{
	m_pDepthTexture->Bind();
	glTexImage2D( GL_TEXTURE_RECTANGLE, 0, GL_DEPTH_COMPONENT32, m_aViewportSize[0], m_aViewportSize[1], 0, GL_DEPTH_COMPONENT, GL_FLOAT, NULL );
	...
	m_pEntryPoints->Bind();
	glTexImage2D( GL_TEXTURE_RECTANGLE, 0, GL_RGB32F, m_aViewportSize[0], m_aViewportSize[1], 0, GL_RGB, GL_FLOAT, NULL );
	glTexParameteri( GL_TEXTURE_RECTANGLE, GL_TEXTURE_WRAP_S, GL_CLAMP_TO_EDGE );
	glTexParameteri( GL_TEXTURE_RECTANGLE, GL_TEXTURE_WRAP_T, GL_CLAMP_TO_EDGE );
	glTexParameteri( GL_TEXTURE_RECTANGLE, GL_TEXTURE_MIN_FILTER, GL_NEAREST );
...
void VistaVolumeRaycasterCore::GenerateProxyGeometry()
{
	...
	float aProxyGeometry[::g_nNumVertexEntries * 6] =
	{
		// TexCoords (x, y, z)		VertexPos (x, y, z)
		// Back plane
	...
	m_pProxyGeometrySetup->SpecifyAttributeArrayFloat( 1, 3, GL_FLOAT,
		GL_FALSE, 6 * sizeof( aProxyGeometry[0] ), 0, m_pProxyGeometry );
	// Vertex positions
	m_pProxyGeometrySetup->EnableAttributeArray( 0 );
	m_pProxyGeometrySetup->SpecifyAttributeArrayFloat( 0, 3, GL_FLOAT,
		GL_FALSE, 6 * sizeof( aProxyGeometry[0] ), 3 * sizeof( aProxyGeometry[0] ),
		m_pProxyGeometry );
}
...
bool VistaVolumeRaycasterCore::IsUniformDeclared( const std::string& strUniformName ) const
{
	...
	std::vector<Texture>::const_iterator itTexure = std::find(
		m_vecVolumeTextures.begin(),
		m_vecVolumeTextures.end(),
		strUniformName );
	...
	itTexure = std::find(
		m_vecLookupTextures.begin(),
		m_vecLookupTextures.end(),
		strUniformName );
	...
	std::vector<UniformVariable>::const_iterator itUniform = std::find(
		m_vecLookupRanges.begin(),
		m_vecLookupRanges.end(),
		strUniformName );
...
int VistaVolumeRaycasterCore::DeclareTexture(const std::string& strSamplerName, std::vector<Texture>& vecTarget )
{
...
unsigned int VistaVolumeRaycasterCore::GetNumberOfTextures() const
{
	return static_cast<unsigned int>(
		m_vecVolumeTextures.size() + m_vecLookupTextures.size() );
}
...
void VistaVolumeRaycasterCore::UpdateJitterTexture()
{
	...
	glTexImage2D( GL_TEXTURE_RECTANGLE, 0, GL_RG16F, m_iJitterTextureSize,
			m_iJitterTextureSize, 0, GL_RG, GL_FLOAT, pData );
	glTexParameteri( GL_TEXTURE_RECTANGLE, GL_TEXTURE_WRAP_S, GL_CLAMP_TO_EDGE );
	glTexParameteri( GL_TEXTURE_RECTANGLE, GL_TEXTURE_WRAP_T, GL_CLAMP_TO_EDGE );
	glTexParameteri( GL_TEXTURE_RECTANGLE, GL_TEXTURE_MIN_FILTER, GL_NEAREST );
	glTexParameteri( GL_TEXTURE_RECTANGLE, GL_TEXTURE_MAG_FILTER, GL_NEAREST );	
