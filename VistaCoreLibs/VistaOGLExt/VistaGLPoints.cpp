...
void InitPointTypes() {
  VistaGLSLShader* pHalo  = CreateShader(g_strVert, g_strGeom, g_strHalo);
  VistaGLSLShader* pPhong = CreateShader(g_strVert, g_strGeom, g_strGlyph, g_strPhong);
  VistaGLSLShader* pGooch = CreateShader(g_strVert, g_strGeom, g_strGlyph, g_strGooch);
...
void VistaGLPoints::SetDirection(const VistaVector3D& v3Dir, bool bInViewCoord) {
  if (g_oCurrentAttributes.v3Dir == v3Dir && g_oCurrentAttributes.bDirInViewCoord == bInViewCoord)
    return;
...
bool VistaGLPoints::Enable(int iType /*= POINT_WITH_HALO*/) {
  ...
  if (-1 != iUniLoc)
    glUniform3f(iUniLoc, g_oCurrentAttributes.v3Dir[0], g_oCurrentAttributes.v3Dir[1],
...
bool VistaGLPoints::PopAttrib() {
  ...
  SetDirection(g_oAttributeStack.top().v3Dir, g_oAttributeStack.top().bDirInViewCoord);
