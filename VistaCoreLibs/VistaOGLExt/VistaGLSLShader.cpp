...
bool VistaGLSLShader::InitFromFiles(const std::string& strVertexShaderFile,
...
bool VistaGLSLShader::InitFromStrings(const std::string& strVertexShaderString,
...
bool VistaGLSLShader::InitVertexShaderFromString(const std::string& strVertexShaderString) {
...
bool VistaGLSLShader::InitGeometryShaderFromString(const std::string& strGeometryShaderString) {
...
bool VistaGLSLShader::InitFragmentShaderFromString(const std::string& strFragmentShaderString) {
...
bool VistaGLSLShader::InitComputeShaderFromString(const std::string& strComputeShaderString) {
  return InitShaderFromString(GL_COMPUTE_SHADER, strComputeShaderString);
}

bool VistaGLSLShader::InitShaderFromFile(const unsigned int uiShaderTyp,
    const std::string& strShaderFile, const std::string& strPrefix /* = "" */) {
#ifdef DEBUG
  vstr::debugi() << "[VistaGLSLShader] Initializing shader from file '" << strShaderFile << "'..."
                 << std::endl;
#endif
...
bool VistaGLSLShader::InitShaderFromString(
    const unsigned int uiShaderTyp, const std::string& strShaderString) {
...
#ifdef DEBUG
    vstr::debugi() << "[VistaGLSLShader] Created program " << m_uiProgram << " ..." << std::endl;
#endif
  }
  ...
  case GL_COMPUTE_SHADER:
    ...
    break;
  default:
    vstr::errp() << "[VistaGLSLShader] Unknown shader type " << uiShaderTyp << " ..." << std::endl;
    return false;
  }
...
bool VistaGLSLShader::SetGeometryShaderVertexOutputCount(int iCount) {
  ...
  if (iCount > iMaxOutputVertices) {
    ...
    vstr::IndentObject oIndent;
    vstr::erri() << "Maximum supported vertex output: " << iMaxOutputVertices << std::endl;
    return false;
  }
...
bool VistaGLSLShader::Link() {
  ...
  if (iLinkSuccess == GL_FALSE) {
    ...
    for (size_t i = 0; i < m_vecVertexShader.size(); ++i) {
      ...
      vstr::erri() << "[VistaGLSLShader] Vertex shader #" << i << " info log: " << std::endl;
...
void VistaGLSLShader::MapUniforms() {
  ...
  for (int i = 0; i < iNumUniforms; ++i) {
    glGetActiveUniform(
        m_uiProgram, static_cast<GLuint>(i), iMaxNameLength, &iLength, &iSize, &iType, aName);
...
void VistaGLSLShader::MapUniformBlocks() {
  ...
  for (int i = 0; i < iNumUniformBlocks; ++i) {
    glGetActiveUniformBlockName(
        m_uiProgram, static_cast<GLuint>(i), iMaxNameLength, &iLength, aName);
...
GLuint VistaGLSLShader::GetVertexShader(const std::size_t nIdx) const {
  return (nIdx >= m_vecVertexShader.size() ? 0 : m_vecVertexShader[nIdx]);
}

GLuint VistaGLSLShader::GetTessControlShader(const std::size_t nIdx) const {
  return (nIdx >= m_vecTessControlShader.size() ? 0 : m_vecTessControlShader[nIdx]);
}

GLuint VistaGLSLShader::GetTessEvalShader(const std::size_t nIdx) const {
  return (nIdx >= m_vecTessEvalShader.size() ? 0 : m_vecTessEvalShader[nIdx]);
}

GLuint VistaGLSLShader::GetGeometryShader(const std::size_t nIdx) const {
  return (nIdx >= m_vecGeometryShader.size() ? 0 : m_vecGeometryShader[nIdx]);
}

GLuint VistaGLSLShader::GetFragmentShader(const std::size_t nIdx) const {
  return (nIdx >= m_vecFragmentShader.size() ? 0 : m_vecFragmentShader[nIdx]);
}

GLuint VistaGLSLShader::GetComputeShader(const std::size_t nIdx) const {
  return (nIdx >= m_vecComputeShader.size() ? 0 : m_vecComputeShader[nIdx]);
}

int VistaGLSLShader::GetUniformLocation(const std::string& strParameterName) const {
  std::map<std::string, GLint>::const_iterator cit = m_Uniforms.find(strParameterName);
...
void VistaGLSLShader::SetUniform(int iLocation, float fUniform1, float fUniform2) {
  glUniform2f(iLocation, fUniform1, fUniform2);
}

void VistaGLSLShader::SetUniform(int iLocation, float fUniform1, float fUniform2, float fUniform3) {
  glUniform3f(iLocation, fUniform1, fUniform2, fUniform3);
}

void VistaGLSLShader::SetUniform(
...
void VistaGLSLShader::SetUniform(
    int iLocation, int iCount, const float* fUniform, bool bTranspose) {
  glUniformMatrix4fv(iLocation, iCount, bTranspose, fUniform);
}

void VistaGLSLShader::SetUniform(int iLocation, int iComponents, int iCount, const float* pData) {
  switch (iComponents) {
  case 1:
...
void VistaGLSLShader::SetUniform(int iLocation, int iComponents, int iCount, const int* pData) {
  switch (iComponents) {
  case 1:
...
GLuint VistaGLSLShader::GetUniformBlockIndex(const std::string& strBlockName) const {
...
bool VistaGLSLShader::SetUniformBlockBinding(GLuint uiBlockIndex, GLuint uiBindPoint) {
  glUniformBlockBinding(m_uiProgram, uiBlockIndex, uiBindPoint);
  return !VistaOGLUtils::CheckForOGLError(__FILE__, __LINE__);
}

bool VistaGLSLShader::SetUniformBlockBinding(const std::string& strBlockName, GLuint uiBindPoint) {
  const GLuint uiIndex = GetUniformBlockIndex(strBlockName);
  return SetUniformBlockBinding(uiIndex, uiBindPoint);
}

bool VistaGLSLShader::GetUniformBlockBinding(GLuint uiBlockIndex, GLuint& uiBindPoint) {
  GLint iResult = -1;
  glGetActiveUniformBlockiv(m_uiProgram, uiBlockIndex, GL_UNIFORM_BLOCK_BINDING, &iResult);
  uiBindPoint = static_cast<GLuint>(iResult);
  return !VistaOGLUtils::CheckForOGLError(__FILE__, __LINE__);
}

bool VistaGLSLShader::GetUniformBlockBinding(const std::string& strBlockName, GLuint& uiBindPoint) {
  const GLuint uiIndex = GetUniformBlockIndex(strBlockName);
