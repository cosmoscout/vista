...
bool VistaTexture::UploadTexture(const int &iWidth, const int &iHeight,
	void* pData, bool bGenMipmaps /* = true */,
	GLenum eInputPixelFormat /* = GL_RGBA */,
	...
	if(GetTarget() == GL_TEXTURE_2D)
	{
		if(!bGenMipmaps)                       
		{
			glTexImage2D(GL_TEXTURE_2D, 0, GL_RGBA8,
				iWidth, iHeight, 0, eInputPixelFormat, eDataFormat, pData);
		}
		else
		{
			gluBuild2DMipmaps(GL_TEXTURE_2D, GL_RGBA8,
				iWidth, iHeight, eInputPixelFormat, eDataFormat, pData);
		}
	}
	else if(GetTarget() == GL_TEXTURE_1D)
	{
		if(!bGenMipmaps)
		{
			glTexImage1D(GL_TEXTURE_1D, 0, GL_RGBA8,
				iWidth, 0, eInputPixelFormat, eDataFormat, pData);
		}
		else
		{
			gluBuild1DMipmaps(GL_TEXTURE_1D, GL_RGBA8, iWidth,
				eInputPixelFormat, eDataFormat, pData);
		}
	}
	else
	{
