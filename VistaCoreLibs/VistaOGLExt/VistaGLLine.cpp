...
namespace VistaGLLine
{
	void UpdateTexture()
	{
		...
		if(!g_pTexture)
		{
			...
			g_pTexture->UploadTexture( iWidth, 0, &aData, true, GL_ALPHA,
									   GL_UNSIGNED_BYTE );
		...
		{
			g_pTexture->Bind();
			glTexSubImage1D( g_pTexture->GetTarget(), 0, 0, iWidth, GL_ALPHA,
	...
	void InitShader(int iShader)
	{
		...
		case SHADER_TRUNCATED_CONES:
			pShader = CreateShader(g_strTruncatedCone_VS, g_strTruncatedCone_GS, g_strTruncatedCone_FS, g_strLighting_Ext);
			break;
		}
		if( !pShader ) return;
...
void VistaGLLine::SetLineWidth(float fLineWith)
{
	...
	if(g_iCourentBoundShader >= 0)
	{
		...
		if( -1 != iUniLoc ) glUniform1f( iUniLoc, g_fLineWith );
	}
}
...
void VistaGLLine::SetEnableLighting(bool b)
{
	...
	if (g_iCourentBoundShader >= 0)
	{
		...
		if (-1 != iUniLoc) glUniform1i(iUniLoc, g_bComputeLighting);
	}
}
...
void VistaGLLine::SetArrowheadRadius(float fConeRadius)
{
	...
	if(g_iCourentBoundShader >= 0)
	{
		...
		if( -1 != iUniLoc ) glUniform1f( iUniLoc, g_fConeRadius );
	}
}
...
void VistaGLLine::SetArrowheadLength(float fConeHeight)
{
	...
	if(g_iCourentBoundShader >= 0)
	{
		...
		if( -1 != iUniLoc ) glUniform1f( iUniLoc, g_fConeHeight );
	}
}
...
/*                                                                            */
/*============================================================================*/
int VistaGLLine::AddLineShader( 
	VistaGLSLShader* pShader, bool bNeedAdjacencyInformations )
{
	...
	g_vecShader.push_back(
		pair<VistaGLSLShader*, bool>(pShader,bNeedAdjacencyInformations) );
