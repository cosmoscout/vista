...
VistaNetworkInfo::VistaNetworkInfo(const std::string& name, const VistaIPAddress& ipAddr,
    const VistaIPAddress& ipBroadcast, const VistaIPAddress& ipNetmask, int mtu)
    : m_sName(name)
    , m_iMtu(mtu)
...
bool VistaNetworkInfo::EnumVistaNetworkInfo(vector<VistaNetworkInfo>& devs) {
  ...
  if (sock.OpenSocket()) {
    ...
    for (int i = 0; i < count; ++i) {
      ...
      m_ipAddr.SetRawINAddress(
          &((sockaddr_in&)ifc.ifc_req[i].ifr_addr).sin_addr, sizeof(struct in_addr));
      ...
      if (ioctl(fd, SIOCGIFBRDADDR, &devifreq) != -1)
        brdaddr.SetRawINAddress(
            &((sockaddr_in&)devifreq.ifr_broadaddr).sin_addr, sizeof(struct in_addr));

      if (ioctl(fd, SIOCGIFNETMASK, &devifreq) != -1)
        maskaddr.SetRawINAddress(
            &((sockaddr_in&)devifreq.ifr_addr).sin_addr, sizeof(struct in_addr));
...
#if defined(LINUX)
      devs.push_back(
          VistaNetworkInfo(ifc.ifc_req[i].ifr_ifrn.ifrn_name, m_ipAddr, brdaddr, maskaddr, mtu));
#else
    ...
    for (int i = 0; i < ifcount; ++i) {
      ...
      brdaddr.SetRawINAddress(
          &((sockaddr_in&)iflist[i].iiBroadcastAddress).sin_addr, sizeof(struct in_addr));
      maskaddr.SetRawINAddress(
          &((sockaddr_in&)iflist[i].iiNetmask).sin_addr, sizeof(struct in_addr));
