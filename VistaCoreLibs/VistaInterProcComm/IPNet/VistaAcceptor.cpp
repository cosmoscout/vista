...
void IVistaAcceptor::DefinedThreadWork() {
  ...
  vstr::outi() << "[ViMsgP]: Created Message Port [" << m_sInterfaceName << ", " << string(buffer)
               << "]" << std::endl;
  ...
  WSAEventSelect(SOCKET((*m_pServer).GetSocketID()), sockEvent.GetEventWaitHandle(),
  ...
  if ((iRet != WAIT_ABANDONED) && (iRet != WAIT_FAILED)) {
    if (iRet == 0) {
      ...
      WSAEventSelect(SOCKET((*m_pServer).GetSocketID()), sockEvent.GetEventWaitHandle(), 0);
  ...
  printf("VistaAcceptWork::DefinedThreadWork() -- WAITING FOR CONNECT (OR ABORT)\n");
  int iRet = TEMP_FAILURE_RETRY(
      select((iSckId > iSyncId ? iSckId + 1 : iSyncId + 1), &myRdSocks, NULL, &myExSocks, NULL));
  ...
  default: {
    vstr::outi() << "[VMsgP]: Received some unknown signal (Error?: " << eSock << ")" << std::endl;
  } break;
  }
