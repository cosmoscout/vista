...
bool IVistaSocket::PrintErrorMessage(const std::string& sMethodName) {
  ...
  if (nErrorCode && (nErrorCode != WSAEWOULDBLOCK)) {
    ...
    int nMessageSize =
        FormatMessage(FORMAT_MESSAGE_FROM_SYSTEM | FORMAT_MESSAGE_IGNORE_INSERTS, NULL, nErrorCode,
            MAKELANGID(LANG_NEUTRAL, SUBLANG_DEFAULT), (LPTSTR)&sErrorMessage[0], 1024, NULL);
...
bool IVistaSocket::GetSockOpt(
    int iCodeLevel, int iOptionName, void* pOptionData, int& iDataLength) const {
#if !defined(HPUX)
  if (getsockopt(SOCKET(m_iSocketID), iCodeLevel, iOptionName, (char*)pOptionData,
          (socklen_t*)&iDataLength) < 0)
#else
...
#else
  if (TEMP_FAILURE_RETRY(setsockopt(
          SOCKET(m_iSocketID), iCodeLevel, iOptionName, (char*)pOptionData, iDataLength)) < 0)
#endif
  {
    return false;
  }
  ...
  if (GetIsBlocking()) {
    ...
    if (ioctlsocket(SOCKET(m_iSocketID), FIONBIO, &isNonBlocking) == SOCKET_ERROR) {
      ...
      if (WSAGetLastError() == WSAEINVAL) {
        ...
        vstr::errp() << "IVistaSocket::VerifyBlocking() -- WSA error (" << iErr << ")" << std::endl;
      }
    }
  }
  return m_bIsBlocking;
...
bool IVistaSocket::BindToAddress(const VistaSocketAddress& rMyAddress) {
  int nRet = bind(
      SOCKET(m_iSocketID), (sockaddr*)rMyAddress.GetINAddress(), rMyAddress.GetINAddressLength());
  if (nRet != 0) {
...
#ifdef WIN32
  int nRet = connect(
      SOCKET(this->m_iSocketID), (sockaddr*)rAddress.GetINAddress(), rAddress.GetINAddressLength());
#else
  int nRet    = TEMP_FAILURE_RETRY(connect(SOCKET(this->m_iSocketID),
      (sockaddr*)rAddress.GetINAddress(), rAddress.GetINAddressLength()));
#endif
  if (nRet == -1) {
    // m_iSocketID = -1;
    if (PrintErrorMessage("ConnectToAddress()") == false) {
      vstr::warnp() << "IVistaSocket::ConnectToAddress(" << rAddress.GetIPAddress().GetHostNameC()
...
unsigned long IVistaSocket::WaitForIncomingData(int iTimeout) {
  ...
  WSAEventSelect(SOCKET(GetSocketID()), (*m_pWaitEvent).GetEventSignalHandle(), FD_READ | FD_CLOSE);
  DWORD iRet = WaitForSingleObject(
      (*m_pWaitEvent).GetEventSignalHandle(), (iTimeout > 0 ? iTimeout : INFINITE));
  // remove event association
  WSAEventSelect(SOCKET(GetSocketID()), (*m_pWaitEvent).GetEventSignalHandle(), 0);
...
int IVistaSocket::ReceiveRaw(void* pvBuffer, const int iLength, int iTimeout, int iFlags) {
  ...
  if (iTimeout) {
    ...
    if (iReadLength == 0)
      return (iReadLength != iLength
                  ? -1
                  : iLength); // iff we could not read the desired value, we return -1
  }
  int iRet = 0;
  do {
#ifdef WIN32
...
void IVistaSocket::SetPermitBroadcast(int iOn) { // 0 disables broadcast else enables
  int iSize = sizeof(iOn);
...
void IVistaSocket::SetDontRoute(
    int iOn) { // A nonzero value enables SO_DONTROUTE. A value of 0 disables SO_DONTROUTE.
  int iSize = sizeof(iOn);
...
void IVistaSocket::SetKeepConnectionsAlive(
    int iOn) { // A nonzero value enables keepalives. A value of 0 disables keepalives.
  int iSize = sizeof(iOn);
...
int IVistaSocket::GetLingerOnClose()
    const { // When the l_onoff field is nonzero, SO_LINGER is enabled. When it is 0, SO_LINGER is
            // disabled. If SO_LINGER is being enabled, the l_linger field specifies the timeout
            // period, in seconds
  linger ling;
...
void IVistaSocket::EnableEventSelect(VistaThreadEvent* pEvent, long lEvents) {
  if (GetIsOpen()) {
    WSAEventSelect(SOCKET(GetSocketID()), (*pEvent).GetEventSignalHandle(), lEvents);
  }
}

void IVistaSocket::DisableEventSelect(VistaThreadEvent* pEvent) {
  if (GetIsOpen()) {
    WSAEventSelect(SOCKET(GetSocketID()), (*pEvent).GetEventSignalHandle(), 0);
  }
}
