...
VistaWin32ProcessEventImp::VistaWin32ProcessEventImp(const std::string& sEventName)
  ...
  if (m_pEventHandle == NULL) {
    vstr::warnp() << "[VistaWin32ProcessEventImp]: Could not create event [" << sEventName
...
VistaWin32ProcessEventImp::VistaWin32ProcessEventImp(
  ...
  if (m_pEventHandle == NULL) {
    vstr::warnp() << "[VistaWin32ProcessEventImp]: Could not open event [" << sEventName
                  << "] in allowed time frame - Error: " << GetErrorText() << std::endl;
  }
}
...
bool VistaWin32ProcessEventImp::WaitForEvent(int nBlockTime) {
  ...
  case WAIT_FAILED:
    vstr::errp() << "[InterProcEmittingBarrier]: Waiting for event failed - " << GetErrorText()
                 << std::endl;
