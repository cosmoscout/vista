...
static void * PosixEntryPoint ( void *that )
{
	...
	{
		vstr::err() << "PThreads-VistaThreadImp -- [" << (pThread ? pThread->GetThreadName() : "invalid-thread-handle") << "] -- execution terminated, given exception: " << x.what() << std::endl;
	}
	catch( ... )
	{
		vstr::err() << "PThreads-VistaThreadImp -- [" << (pThread ? pThread->GetThreadName() : "invalid-thread-handle") << "] -- execution terminated due to unkown exception (ellipse catch).";
	}
	return 0;
}
