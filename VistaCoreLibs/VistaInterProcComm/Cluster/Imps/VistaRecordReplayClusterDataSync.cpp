...
VistaRecordClusterLeaderDataSync::VistaRecordClusterLeaderDataSync(const std::string& sFilename,
    IVistaClusterDataSync* pOriginalSync, const bool bManageOriginalSyncDeletion,
    const bool bVerbose)
  ...
  if (m_pRecordFile->GetIsOpen() == false)
    VISTA_THROW(
        ("VistaRecordClusterLeaderDataSync - invalid file [" + sFilename + "]").c_str(), -1);
}
...
bool VistaRecordClusterLeaderDataSync::SyncData(
...
VistaReplayClusterFollowerDataSync::VistaReplayClusterFollowerDataSync(
    const std::string& sFilename, const bool bVerbose)
  ...
  if (m_pReplayFile->Open() == false)
    VISTA_THROW(
        ("VistaReplayClusterLeaderDataSync - invalid file [" + sFilename + "]").c_str(), -1);
}

VistaReplayClusterFollowerDataSync::VistaReplayClusterFollowerDataSync(
    VistaConnectionFile* pReplayFile, const bool bVerbose)
    : IVistaClusterDataSync(bVerbose, false)
...
bool VistaReplayClusterFollowerDataSync::SyncData(
    VistaType::byte* pDataBuffer, const int iBufferSize, int& iDataSize) {
  // bool bRes;
  // m_pReplayFile->ReadBool( bRes );
  // if( bRes )
