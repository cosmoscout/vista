...
VistaBroadcastClusterLeaderDataSync::VistaBroadcastClusterLeaderDataSync(const bool bVerbose)
...
VistaBroadcastClusterLeaderDataSync::VistaBroadcastClusterLeaderDataSync(
    VistaUDPSocket* pBroadcastSocket, const bool bManageDeletion, const bool bVerbose)
    : VistaClusterBytebufferLeaderDataSyncBase(bVerbose) {
...
bool VistaBroadcastClusterLeaderDataSync::AddBroadcast(
...
bool VistaBroadcastClusterLeaderDataSync::WaitForConnection(
...
VistaBroadcastClusterFollowerDataSync::VistaBroadcastClusterFollowerDataSync(
    const std::string& sBroadcastAddress, const int nBroadcastPort, const bool bSwap,
    const bool bVerbose)
...
VistaBroadcastClusterFollowerDataSync::VistaBroadcastClusterFollowerDataSync(
    VistaUDPSocket* pBroadcastSocket, const bool bSwap, const bool bManageDeletion,
    const bool bVerbose)
    : VistaClusterBytebufferFollowerDataSyncBase(bSwap, bVerbose)
    , m_bManageDeletion(true)
...
bool VistaBroadcastClusterFollowerDataSync::DoReceiveMessage() {
  ...
  try {
    ...
    while (nReadExtBytes < nMessageSize[1]) {
      ...
      nRet          = m_pBCReceiverSocket->ReceiveRaw(
          &m_vecMessageBuffer[nReadBytes + nReadExtBytes], nReadSize);
      if (nRet <= 0) {
        if (GetIsVerbose()) {
          vstr::warnp() << "[VistaBroadcastClusterFollowerSync]: "
                        << "Error receiving message from leader" << std::endl;
...
bool VistaBroadcastClusterFollowerDataSync::WaitForConnection(VistaConnectionIP* pConnection) {
  ...
  if (m_pBCReceiverSocket->ReceiveRaw(&nReadValue, sizeof(VistaType::sint32)) !=
