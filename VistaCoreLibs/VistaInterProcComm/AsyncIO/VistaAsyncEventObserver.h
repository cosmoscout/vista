  ...
  class VISTAINTERPROCCOMMAPI IUpdateTranslator {
   public:
    ...
    virtual bool operator()(
        IVistaObserveable* pObj, int iMsg, int iTicket, bool& bPrependSize, IVistaSerializer&) = 0;
...
class VISTAINTERPROCCOMMAPI VistaAEObserverTask : public IVistaThreadedTask {
 ...
 public:
  enum eChannel { E_FAILURE = -1, E_METAEVENT = 0, E_NOTIFY = 2, E_MODEL, E_SERVER, E_CLIENT };
 ...
 private:
  ...
  struct _objHlp {
    _objHlp(IVistaObserveable* pObj, int iMsg, int iTicket)
        : m_pObj(pObj)
        , m_iMsg(iMsg)
        , m_iTicket(iTicket) {
    }
    ...
    int                m_iMsg, m_iTicket;

    bool operator==(const _objHlp& other) const {
      return ((other.m_pObj == this->m_pObj) && (other.m_iMsg == this->m_iMsg) &&
  ...
  VistaThreadEvent *m_pNotificationEvent, *m_pModelEvent;
