...
VistaAEObserverTask::VistaAEObserverTask(const std::string& sHost, int iPort)
  ...
  m_pClientEvent = NULL;
#endif
  (*m_pMp).AddMultiplexPoint(
      m_pNotificationEvent->GetEventWaitHandle(), E_NOTIFY, VistaIOMultiplexer::MP_IOIN);

  (*m_pMp).AddMultiplexPoint(
      m_pModelEvent->GetEventWaitHandle(), E_MODEL, VistaIOMultiplexer::MP_IOIN);
...
bool VistaAEObserverTask::SetupServerSocket() {
  ...
  if ((*m_pServer).OpenSocket()) {
    vstr::outi() << "CVfvObserverTask::SetupServerSocket(" << m_sHostName << ", " << m_iHostPort
                 << ")" << std::endl;
...
#ifdef WIN32
        EnableEventSelect(
            (*m_pServer).GetSocketID(), m_pServerEvent, FD_READ | FD_CLOSE | FD_ACCEPT);
        (*m_pMp).AddMultiplexPoint(
            (*m_pServerEvent).GetEventSignalHandle(), E_SERVER, VistaIOMultiplexer::MP_IOIN);
#else
        (*m_pMp).AddMultiplexPoint(
            (*m_pServer).GetSocketID(), E_SERVER, VistaIOMultiplexer::MP_IOIN);
        //(*m_pMp).AddMultiplexPoint((*m_pServer).GetSocketID(),E_SERVER,
...
void VistaAEObserverTask::EnableEventSelect(
    HANDLE iSocketDesc, VistaThreadEvent* pEvent, long iMask) {
#ifdef WIN32
  WSAEventSelect(SOCKET(iSocketDesc), (*pEvent).GetEventSignalHandle(), iMask);
#endif
}

void VistaAEObserverTask::DisableEventSelect(HANDLE iSocketDesc, VistaThreadEvent* pEvent) {
#ifdef WIN32
  WSAEventSelect(SOCKET(iSocketDesc), (*pEvent).GetEventSignalHandle(), 0);
#endif
}

enum eServerEvType { ETS_UNKNOWN = 0, ETS_CLOSE, ETS_READPOSSIBLE, ETS_ACCEPT };

enum eClientEvType { ETC_UNKNOWN = 0, ETC_CLOSE, ETC_MESSAGE };
...
VistaAEObserverTask::eClientEvType VistaAEObserverTask::DetermineClientEvent() {
  ...
  if (WSAEnumNetworkEvents(SOCKET((*m_pClient).GetSocketID()),
          (*m_pClientEvent).GetEventSignalHandle(), &NetworkEvents) == 0) {
    // ok, this worked
...
VistaAEObserverTask::eServerEvType VistaAEObserverTask::DetermineServerEvent() {
  ...
  if (WSAEnumNetworkEvents(SOCKET((*m_pServer).GetSocketID()),
          (*m_pServerEvent).GetEventSignalHandle(), &NetworkEvents) == 0) {
    // ok, this worked
    if (NetworkEvents.lNetworkEvents & FD_CLOSE) {
      // close
...
bool VistaAEObserverTask::SetupClient(VistaTCPSocket* pSock) {
  ...
  (*m_pMp).AddMultiplexPoint(
      (*m_pClientEvent).GetEventSignalHandle(), E_CLIENT, VistaIOMultiplexer::MP_IOIN);
#else
...
void VistaAEObserverTask::HandleModelChange() {
  if (m_pTranslator && !m_ChgSet.empty()) {
    ...
    for (std::list<_objHlp>::iterator it = li.begin(); it != li.end(); ++it) {
...
bool VistaAEObserverTask::RegisterUpdateTranslator(
    VistaAsyncEventObserver::IUpdateTranslator* pTranslator) {
...
/*============================================================================*/
VistaAsyncEventObserver::VistaAsyncEventObserver(const std::string& sHostName, int iHostPort) {
...
VistaAsyncEventObserver::~VistaAsyncEventObserver() {
  ...
  for (std::set<IVistaObserveable*>::iterator it = m_setProConts.begin(); it != m_setProConts.end();
       ++it) {
...
bool VistaAsyncEventObserver::ObserveableDeleteRequest(
...
void VistaAsyncEventObserver::ReleaseObserveable(IVistaObserveable* pObserveable, int nTicket) {
  ...
  std::set<IVistaObserveable*>::iterator it =
      std::find(m_setProConts.begin(), m_setProConts.end(), pObserveable);
