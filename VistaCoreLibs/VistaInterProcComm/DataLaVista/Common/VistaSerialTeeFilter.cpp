...
IDLVistaDataPacket* DLVistaSerialTeeFilter::ReturnPacket() {
  ...
  for (int i = 0; i < GetNumberOfOutbounds(); ++i) {
    ...
    if (pPacket) {
      ...
      if (nIdx == (GetNumberOfOutbounds() - 1)) {
        // printf("DLVistaSerialTeeFilter::UpdatePacketIndex(%X) -- got finished packet from
        // (%d)\n", pPacket, nIdx);
      ...
      {
        // printf("DLVistaSerialTeeFilter::UpdatePacketIndex(%X) -- return packet on non-finished
        // packet\n", pPacket);
  ...
  // printf("DLVistaSerialTeeFilter::UpdatePacketIndex() -- returning NULL (no packet
  // available).\n");
  return NULL;
}
...
#ifdef DEBUG
    printf("DLVistaSerialTeeFilter::FilterPacketL(%lx) -- TRYING TO REFILTER A PACKET WITH INDEX "
           "(%d)\n",
        long(pPacket), iIdx);
#endif
    throw;
...
bool DLVistaSerialTeeFilter::RecycleDataPacket(
  ...
  if (iIdx == (GetNumberOfOutbounds() - 1)) {
    ...
    // printf("DLVistaSerialTeeFilter::RecycleDataPacket(%X) -- returning packet via recycle
    // (%d)\n", pPacket, iIdx);
    if (GrabMutex(m_pMapMutex, true)) {
  ...
  if (GrabMutex(m_pMapMutex, true)) {
    // printf("DLVistaSerialTeeFilter::RecycleDataPacket(%X) -- forwaring packet to new outbound
    // [%d]\n", pPacket, iIdx+1);
