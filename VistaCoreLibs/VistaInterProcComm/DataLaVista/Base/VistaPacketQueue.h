...
class VISTAINTERPROCCOMMAPI DLVistaPacketQueue {
 ...
 public:
   ...
   * @return a fresh packet from this queue that can be filled with information, or NULL if none is
   * available
   */
  ...
  /**
   * Checks whether the pPacket does belong to this queue, if so, it does invalidate its timestamp
   * (sets it to (-1,-1)), unlocks it and requeues the packet to be used again in the next
   * iterations.
  ...
  /**
   * Returns the number of available packets in this queue (which can be less or equal to
   * GetPacketCount()
  ...
  /**
   * Tries to allocate iNewSize packets for this queue. All old packets will be deleted before the
   * action.
  ...
  /**
   * A queue can only store one type of packets. This method will return the typeindicator that is
   * given back by the packets themselves. You may only call this if GetPacketCount() > 0
