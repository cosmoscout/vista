...
class VISTAINTERPROCCOMMAPI DLVistaDataSource : public IDLVistaDataProducer {
 public:
   ...
   *  <li>If there is a packet, stamp it with a given time-stamp, else increase drop-count, return
   * NULL <li>Fill packet with content (sub-class defined, via FillPacket()) <li>increase package
   * count <li>give back the produced and filled packet
   * </ul>
   ...
   * @param pPacket iff a packet is given, it is passed to the outbound component via
   * AcceptDataPacket(pPacket), if no packet is given (default) the packet cerated by
   * ProducePacket() is passed (if any could be produced)
   ...
   * @param bBlock  indicates whether the call to this method shall block until the packet is fully
   * recycled or not
  ...
  /**
   * Sets the queue size for the outbound buffer packets to the given argument. Note that this
   * method will automatically call DLVistaPacketQueue::resize() and does set this method's result
   ...
   * @return the valid type-id for outgoing packets (-1 iff this component does not produce packets,
   * e.g. is a consumer)
   */
 ...
 protected:
  ...
  /**
   * Constructor. Initializes the buffer-packet-queue to zero packets, gets an instance of the
   * global RTC and sets the counting variables to zero
   */
  DLVistaDataSource();

  /**
   * In case this producer does run out of packets, the production method does call FillUp() in
   * order to gain ALL packages that are waiting for recycling at the outbound queue. This strategy
   * tries to minimize the times a queue gets empty. If a producer runs out of packets, the consumer
   * is much slower than the producer.
 ...
 private:
  ...
  int m_iQueueSize, /**< defines the number of buffer packets to create @see m_pPacketQueue() **/
      m_iDropCount, /**< counts the number of times that data had to be dropped because there were
                       no outgoing packets left **/
