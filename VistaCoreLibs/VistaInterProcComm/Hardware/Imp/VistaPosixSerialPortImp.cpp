...
VistaPosixSerialPortImp::VistaPosixSerialPortImp() {
  ...
  m_iFileHandle =
      -1; // -1 is invalid and the standard "indicate-error"-return value, so we use this as default
...
bool VistaPosixSerialPortImp::SetHardwareState() {
  ...
  m_sTermConf.c_cflag |=
      (CREAD | CLOCAL); // enable receiver, and do not change ownership of file-handle
...
bool VistaPosixSerialPortImp::SetBlockingMode(
...
unsigned long VistaPosixSerialPortImp::WaitForIncomingData(int timeout) {
  ...
  int iRet = TEMP_FAILURE_RETRY(select(
      m_iFileHandle + 1, &fdset, NULL, NULL, ((tv.tv_sec != 0 || tv.tv_usec != 0) ? &tv : NULL)));
  if (iRet == 0) {
    // printf("timeout?\n");
    return ~0;
  }
