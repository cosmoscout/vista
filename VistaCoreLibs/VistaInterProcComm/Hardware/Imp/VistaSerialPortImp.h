...
class VISTAINTERPROCCOMMAPI VistaSerialPortImp {
 public:
   ...
   * to GET the current hardware-state. Is should be safe to call this on an already opened port,
   * but it will return false then. <br> <ol> <li>Open Port</li> <li>Set configuration</li>
   *  <li>Send/Receive data</li>
   *  <li>Close Port</li>
   * </ol><br> is a valid sequence. Note that you can change the configuration at any place in
   * between Open and Close (but be sure to know what you are doing ;)
   *
   * @see GetHardwareState()
   * @see Close()
   * @return true iff the device could be opened, false else (including the case where it is called
   * on an already state "open" port)
   */
   ...
   * port is opened.
   * @see OpenSerialPort()
   * @return true iff the once opened port could be closed, false else (e.g. the part was closed
   * already)
   */
   ...
   * of SetBlockingMode() will be used. A call to this method will fail on a closed port and return
   * -1.
   * @see SetBlockingMode()
   ...
   * @param iTimeout the number of millisecs to wait for data until the call returns. 0 indicates
   * "do not wait"
   ...
   * @return the marking for the index as specified in VistaPortIndex(), a value of CSP_NA does
   * indicate an uninitialized state
   */
  ...
  virtual bool SetSoftwareFlowOnOffChars(char cXonChar, char cXoffChar);
  ...
  virtual bool SetBlockingMode(unsigned long inReadInterval, unsigned long inReadMultiplyer,
 ...
 protected:
   ...
   * @return a marking as described in VistaPort, CSP_NA iff this name is not in the list of
   * port-names
   */
