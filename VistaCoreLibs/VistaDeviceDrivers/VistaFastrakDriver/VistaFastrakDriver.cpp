...
class IVistaFastrakCommandSet {
 public:
   ...
   */
  virtual bool CmdSetAlignmentReferenceFrame(int iStation, float fOx = 0.0f, float fOy = 0.0f,
      float fOz = 0.0f, float fXx = 1.0f, float fXy = 0.0f, float fXz = 0.0f, float fYx = 0.0f,
      float fYy = 1.0f, float fYz = 0.0f) = 0;

  virtual bool CmdSetBoresight(
      int iStation, float fAzimuth, float fElevation, float fRoll, bool bResetOrigin)   = 0;
  ...
  enum eMode { MD_NONE = -1, MD_ASCII = 0, MD_BINARY };
...
class CommonCommandSet : public IVistaFastrakCommandSet {
 public:
  CommonCommandSet(VistaDriverConnectionAspect* pCon, IVistaDriverReferenceFrameAspect* pRefFrame)
  ...
  virtual int CmdGetActiveStations() {
    return m_lActiveStations;
  }

  virtual int CmdGetTotalNumberOfStations() {
    return 0;
  }

  virtual bool CmdEnableUnit(unsigned int iUnit) {
    return false;
  }
  virtual bool CmdDisableUnit(unsigned int iUnit) {
    return false;
  }

  virtual bool CmdEnableBinaryMode() {
    return false;
  }
  ...
  virtual bool CmdStylusButtonMode(unsigned int iMode, int iUnit) {
    return false;
  }
  ...
  virtual bool CmdSetAlignmentReferenceFrame(int iStation, float fOx = 0.0f, float fOy = 0.0f,
      float fOz = 0.0f, float fXx = 1.0f, float fXy = 0.0f, float fXz = 0.0f, float fYx = 0.0f,
      float fYy = 1.0f, float fYz = 0.0f) {
    if (CmdResetReferenceFrame(iStation)) {
      char data[59];
      int  nCount =
          sprintf(data, "A%d,%+3.2f,%+3.2f,%+3.2f,%+3.2f,%+3.2f,%+3.2f,%+3.2f,%+3.2f,%+3.2f%c",
  ...
  virtual bool CmdSetAlignmentReferenceFrameForAll(float fOx = 0.0f, float fOy = 0.0f,
      float fOz = 0.0f, float fXx = 1.0f, float fXy = 0.0f, float fXz = 0.0f, float fYx = 0.0f,
      float fYy = 1.0f, float fYz = 0.0f) {
    for (int n = 0; n < m_lStations; ++n) {
      CmdSetAlignmentReferenceFrame(n + 1, fOx, fOy, fOz, fXx, fXy, fXz, fYx, fYy, fYz);
    }
    return true;
  }

  virtual bool CmdSetBoresight(
      int iStation, float fAzimuth, float fElevation, float fRoll, bool bResetOrigin) {
    char data[33];
    int  nCount = sprintf(data, "B%d,%3.2f,%3.2f,%3.2f,%d%c", iStation, fAzimuth, fElevation, fRoll,
        bResetOrigin ? 1 : 0, 0x0d);
    return m_pConnection->SendCommand(0, data, nCount, 100);
  }

  virtual bool CmdSetSourceMountingFrame(float fAzimuth, float fElevation, float fRoll) {
    char data[25];
    int  nCount = sprintf(data, "G%3.2f,%3.2f,%3.2f,%c", fAzimuth, fElevation, fRoll, 0x0d);
    return m_pConnection->SendCommand(0, data, nCount, 100);
  }

  virtual bool CmdSetCompensation(int iMap) {
    return false;
  }

  virtual float GetDataFromBinary(unsigned char* source) {
    return false;
  }
  virtual float GetDataFromAscii(unsigned char* source, int length) {
    return false;
  }
  ...
  virtual bool CmdInitSequence() {
    ...
    if (pCon->Send(data, 3) == 3) {
      ...
      if (pCon->Send(data, 2) == 2) {
        ...
        vstr::outi() << "[FastrackDriver]: this device has " << m_lStations << " stations"
                     << std::endl;
        ...
        vstr::outi() << "[FastrackDriver]:this device has " << m_lActiveStations
                     << " active stations" << std::endl;
        ...
        for (unsigned int k = 0; k < sizeof(long) * 8; ++k) {
          ...
          if (nMask & (1 << k)) {
            ...
            if (m_pRefFrame->GetSensorAlignment(mt, k + 1)) {
              ...
              /** @todo fix setup */
              // x-axis has to be given in relation to the original coord-system of the tracker
              // device
              CmdSetAlignmentReferenceFrame(
                  k + 1, origin[0], origin[1], origin[2], -1, 0, 0, 0, 0, -1);
            }
          }
        }
...
class LibertyCommandSet : public CommonCommandSet {
 public:
  LibertyCommandSet(VistaDriverConnectionAspect* pCon, IVistaDriverReferenceFrameAspect* pRefFrame)
      : CommonCommandSet(pCon, pRefFrame) {
  }

  static std::string GetCommandSetId() {
    return "LIBERTY";
  }
  virtual eMode CmdGetCurrentMode() const {
    return MD_NONE;
  }
...
class PatriotCommandSet : public LibertyCommandSet {
 public:
  PatriotCommandSet(VistaDriverConnectionAspect* pCon, IVistaDriverReferenceFrameAspect* pRefFrame)
  ...
  bool SetProtocol(const _cVersionTag& oTag) {
    if (oTag.m_strProtocolName == "FASTRAK") {
      ...
      VistaDriverConnectionAspect* pConAsp = dynamic_cast<VistaDriverConnectionAspect*>(
          m_pDriver->GetAspectById(VistaDriverConnectionAspect::GetAspectId()));

      IVistaDriverReferenceFrameAspect* pRefFrame = dynamic_cast<IVistaDriverReferenceFrameAspect*>(
          m_pDriver->GetAspectById(IVistaDriverReferenceFrameAspect ::GetAspectId()));
...
class VistaFastrackReferenceFrameAspect : public IVistaDriverReferenceFrameAspect {
 public:
  ...
  virtual bool SetSensorAlignment(const VistaTransformMatrix& mat, unsigned int nSensorId) {
  ...
  virtual bool GetSensorAlignment(VistaTransformMatrix& mat, unsigned int nSensorId) {
...
VistaFastrakDriver::~VistaFastrakDriver() {
  VistaFastrakEnableAspect* enabled =
      GetAspectAs<VistaFastrakEnableAspect>(VistaDriverEnableAspect::GetAspectId());
...
bool VistaFastrakDriver::DoSensorUpdate(VistaType::microtime dTs) {
  ...
  pCon->SetIsBlocking(true);
  int   n = 0;
  float fPx, fPy, fPz, fOx, fOy, fOz, fOw;
  int   nSensor, nButtonFlag;

  int nTimeStamp, nFrameCount, nStylus, nDistortionLevel;
  ...
  while (n < 3 * GetCommandSet()->CmdGetActiveStations()) {
    ...
    int nScanned = 0;
    if ((nScanned = sscanf(sData.c_str(), "%d %f %f %f %f %f %f %f %d %d %d %d %d", &nSensor, &fPx,
             &fPy, &fPz, &fOw, &fOx, &fOy, &fOz, &nButtonFlag, &nTimeStamp, &nFrameCount, &nStylus,
             &nDistortionLevel)) == 13) {
      ...
      if (nStylus == 0) // simple body
      {
        ...
        VistaFastrakMeasures::sFastrakButtonSample* pMeasure =
            pM->getWrite<VistaFastrakMeasures::sFastrakButtonSample>();
