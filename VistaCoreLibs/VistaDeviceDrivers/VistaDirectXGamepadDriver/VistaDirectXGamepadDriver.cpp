...
VistaDirectXGamepad::VistaDirectXGamepad(IVistaDriverCreationMethod* crm)
  ...
  DirectInput8Create(
      GetModuleHandle(NULL), DIRECTINPUT_VERSION, IID_IDirectInput8, (VOID**)&m_pDI, NULL);
...
bool VistaDirectXGamepad::UpdateDirectXInput(VistaType::microtime dTs) {
  ...
  if (hr != S_OK) {
    ...
    if (memcmp(&m_oldState, &newjs, sizeof(DIJOYSTATE2)) != 0) {
      ...
      VistaDirectXGamepadSensorMeasures::VistaDirectXGamepadSensorMeasure* p =
...
static BOOL CALLBACK EnumJoysticksCallback(const DIDEVICEINSTANCE* pdidInstance, VOID* pContext) {
  ...
  hr = pEnumContext->m_pDI->CreateDevice(
      pdidInstance->guidInstance, &pEnumContext->m_pJoystick, NULL);
...
static BOOL CALLBACK FindJoysticksCallback(const DIDEVICEINSTANCE* pdidInstance, VOID* pContext) {
  DI_ENUM_CONTEXT* pEnumContext = (DI_ENUM_CONTEXT*)pContext;
  HRESULT          hr;
...
struct _sEnumJoystickContext {
  _sEnumJoystickContext(
      LPDIRECTINPUTDEVICE8 pJoystick, int nAxesRangeMin = -1000, int nAxesRangeMax = +1000)
      : m_nBtNum(0)
  ...
  int m_nBtNum, m_nAxesRangeMin, m_nAxesRangeMax, m_nForceFeedbackAxes;
...
bool VistaDirectXGamepad::InitDirectXInput() {
  ...
  if (m_strJoyName.empty()) {
    ...
    hr = m_pDI->EnumDevices(
        DI8DEVCLASS_GAMECTRL, FindJoysticksCallback, &enumContext, DIEDFL_ATTACHEDONLY);
  }
  ...
  if (hr == S_OK && enumContext.m_pJoystick) {
    ...
    std::cout << "VistaDirectXGamepad::InitDirextX() -- found [" << DeviceInfo.tszInstanceName
              << "] controller\n";
    ...
    hr = m_pJoystick->EnumObjects(EnumObjectsCallback, (VOID*)&s, DIDFT_AXIS | DIDFT_BUTTON);
    if (hr != S_OK)
    ...
    std::cerr << "VistaDirectXGamepad::InitDirectXInput() "
              << "-- FAILED to enumerate a joystick" << std::endl;
...
bool VistaDirectXGamepad::GetRanges(
    int& xMin, int& yMin, int& zMin, int& xMax, int& yMax, int& zMax) const {
  if (m_pJoystick == NULL)
    return false;
...
bool VistaDirectXGamepad::UnregisterAspect(IVistaDeviceDriverAspect* pAspect,
    eDeleteAspectBehavior bDelete, eUnregistrationBehavior bForce) {
  if (m_pWindowAspect &&
      (pAspect->GetAspectId() == VistaDriverAbstractWindowAspect::GetAspectId())) {
    std::list<VistaDriverAbstractWindowAspect::IWindowHandle*> liWds =
        m_pWindowAspect->GetWindowList();
    for (std::list<VistaDriverAbstractWindowAspect::IWindowHandle*>::iterator it = liWds.begin();
         it != liWds.end(); ++it)
...
VistaDirectXGamepad::VistaDirectXForceFeedbackAspect::~VistaDirectXForceFeedbackAspect() {
  ...
  for (std::vector<Effect*>::iterator it = m_vecEffects.begin(); it != m_vecEffects.end(); ++it) {
    (*it)->m_pEffect->Unload();
    delete (*it);
  }
}
...
bool VistaDirectXGamepad::VistaDirectXForceFeedbackAspect::DeleteEffect(Effect* pEffect) {
  ...
  std::vector<Effect*>::iterator it =
      std::remove(m_vecEffects.begin(), m_vecEffects.end(), pEffect);
...
bool VistaDirectXGamepad::VistaDirectXForceFeedbackAspect::SetForce(
    const VistaVector3D& v3Force, const VistaVector3D&) {
  if (!m_pCurrentEffect)
    return false;
...
CreateProxy::CreateProxy(IVistaTranscoderFactoryFactory* fac)
...
unsigned int CreateProxy::RegisterSensorType(const std::string& strName, unsigned int nMeasureSize,
    unsigned int nUpdateEstimator, IVistaMeasureTranscoderFactory* pFac,
    const std::string& strTranscoderTypeString) {
  return m_pOrig->RegisterSensorType(strName, nMeasureSize, nUpdateEstimator, pFac);
}
...
IVistaMeasureTranscoderFactory* CreateProxy::GetTranscoderFactoryForSensor(
