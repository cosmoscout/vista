...
namespace {
	...
	void PrintColorNodeConfig(ColorNode::Configuration conf) {
		...
		vstr::debugi() << "Frame Format:\t\t"
					   << FrameFormatToString(conf.frameFormat) << std::endl;
	...
	void PrintColorNodeParams(ColorNode node) {
		vstr::debugi() << std::boolalpha << "WhiteBalanceAuto: "
					   << node.getWhiteBalanceAuto() << std::endl;
	}
	...
	void PrintDepthNodeParams(DepthNode node) {
		vstr::debugi() << std::boolalpha << "DepthDenoising: "
					   << node.getEnableDenoising() << std::endl;
	}
...
// DRIVER CREATE
// #############################################################################
VistaDepthSenseDriverCreationMethod::
...
// PARAMETER CREATE
// #############################################################################
typedef TParameterCreate<
	VistaDepthSenseDriver,
	VistaDepthSenseDriver::DepthSenseParameters> paramCreator;

template<class T>
bool propChange( VistaDepthSenseDriver *driver,
...
namespace {
	IVistaPropertyGetFunctor *SaParameterGetter[] =	{
		...
		VistaDepthSenseDriver::DepthSenseParameters,
		...
		VistaProperty::PROPT_BOOL>(
			"DEPTH_DENOISING",
			SsReflectionName,
			&VistaDepthSenseDriver::DepthSenseParameters::GetDepthDenoising,
			"Depth frame denoising" ),
		NULL
	};
	...
	IVistaPropertySetFunctor *SaParameterSetter[] =	{
		...
		VistaDepthSenseDriver::DepthSenseParameters>(
			"DEPTH_DENOISING",
			SsReflectionName,
			&VistaDepthSenseDriver::DepthSenseParameters::SetDepthDenoising,
			"Depth frame denoising" ),
		NULL
	};
}
...
VistaDepthSenseDriver::DepthSenseParameters::DepthSenseParameters(
	VistaDepthSenseDriver* pDriver )
	: VistaDriverGenericParameterAspect::IParameterContainer(),
	  m_parent(pDriver),
...
bool VistaDepthSenseDriver::DepthSenseParameters::SetWhiteBalanceAuto(
	bool bWhiteBalanceAuto) {
...
bool VistaDepthSenseDriver::DepthSenseParameters::SetDepthDenoising(
	bool bDepthDenoising) {
...
public:
	...
	void RegisterColorNode(ColorNode node) {
		vstr::debugi() << g_sLogPrefix << "Registering with color node"
					  << std::endl;

		VistaDriverGenericParameterAspect *pParamAspect = m_pDriver->GetAspect<VistaDriverGenericParameterAspect>();
		...
		node.newSampleReceivedEvent().connect(
	...
	void RegisterDepthNode(DepthNode node) {
		vstr::debugi() << g_sLogPrefix << "Registering with depth node"
					   << std::endl;
	...
	void NewColorSample(ColorNode obj,
		...
		VistaSensorMeasure *pM = m_pDriver->MeasureStart( 0, mtStamp, IVistaDeviceDriver::RETURN_CURRENT_SLOT );
	...
	void NewDepthSample(DepthNode obj,
						DepthNode::NewSampleReceivedData data) {
		VistaType::microtime mtStamp = VistaTimer::GetStandardTimer().GetMicroTime();
		
		VistaSensorMeasure *pMDepth = m_pDriver->MeasureStart( 1, mtStamp, IVistaDeviceDriver::RETURN_CURRENT_SLOT );
		VistaSensorMeasure *pMUVMap = m_pDriver->MeasureStart( 2, mtStamp, IVistaDeviceDriver::RETURN_CURRENT_SLOT );
...
protected:
	void ThreadBody()
	{
		vstr::debugi() << g_sLogPrefix << "Starting DepthSense event loop..."
					   << std::endl;
		m_pDriver->m_pPrivate->oContext.run();
		vstr::debugi() << g_sLogPrefix << "DepthSense event loop finished"
					   << std::endl;
	}
private: 
	...
	m_pPrivate(new DepthSensePrivate)
{
	...
	pSensor->SetMeasureTranscode(
		crm->GetTranscoderFactoryForSensor("COLOR")->CreateTranscoder() );
	...
	pSensor->SetMeasureTranscode(
		crm->GetTranscoderFactoryForSensor("DEPTH")->CreateTranscoder() );
	...
	pSensor = new VistaDeviceSensor;
	pSensor->SetMeasureTranscode(
		crm->GetTranscoderFactoryForSensor("UVMAP")->CreateTranscoder() );
...
VistaDepthSenseDriver::~VistaDepthSenseDriver()
{
	...
	GetFactory()->GetTranscoderFactoryForSensor("COLOR")->
		DestroyTranscoder( pSensor->GetMeasureTranscode() );
	RemDeviceSensor(pSensor);
	delete pSensor;

	pSensor = GetSensorByName("DEPTH");
	GetFactory()->GetTranscoderFactoryForSensor("DEPTH")->
		DestroyTranscoder( pSensor->GetMeasureTranscode() );
	RemDeviceSensor(pSensor);
	delete pSensor;

	pSensor = GetSensorByName("UVMAP");
	GetFactory()->GetTranscoderFactoryForSensor("UVMAP")->
		DestroyTranscoder( pSensor->GetMeasureTranscode() );
	RemDeviceSensor(pSensor);
	delete pSensor;
...
bool VistaDepthSenseDriver::DoConnect()
{
    ...
    if (vecDevices.size() >= 1) {
		...
		vstr::outi() << g_sLogPrefix << "Found "
					 << vecNodes.size() << " nodes on first device" 
					 << std::endl;
	}
	else {
		vstr::erri() << g_sLogPrefix 
					 << "No DepthSense devices found. Connect failed!"
					 << std::endl;
		return false;
	}

	for(std::vector<Node>::const_iterator nodeIter = vecNodes.begin(); 
		nodeIter != vecNodes.end(); nodeIter++) {
		Node node = *nodeIter;
	...
	if(!m_pPrivate->oColorNode.isSet() ||
	   !m_pPrivate->oDepthNode.isSet()) {
		vstr::erri() << g_sLogPrefix
					 << "Could not find Color and Depth nodes. Connect failed!"
					 << std::endl;
		return false;
	}
