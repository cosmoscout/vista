...
namespace {
	...
	std::string Get3DCDevicTypeString( int dev_type )
	{
		switch( dev_type )
		{
		case SI_SPACEBALL_2003: return "SPACEBALL_2003";
		case SI_SPACEBALL_3003: return "SPACEBALL_3003";
		case SI_SPACE_CONTROLLER: return "SPACE_CONTROLLER";
		case SI_SPACEEXPLORER: return "SPACEEXPLORER";
		case SI_SPACENAVIGATOR_FOR_NOTEBOOKS: return "SPACENAVIGATOR_FOR_NOTEBOOKS";
		case SI_SPACENAVIGATOR: return "SPACENAVIGATOR";
		case SI_SPACEBALL_2003A: return "SPACEBALL_2003A";
		case SI_SPACEBALL_2003B: return "SPACEBALL_2003B";
		case SI_SPACEBALL_2003C: return "SPACEBALL_2003C";
		case SI_SPACEBALL_3003A: return "SPACEBALL_3003A";
		case SI_SPACEBALL_3003B: return "SPACEBALL_3003B";
		case SI_SPACEBALL_3003C: return "SPACEBALL_3003C";
		case SI_SPACEBALL_4000:  return "SPACEBALL_4000";
		case SI_SPACEMOUSE_CLASSIC: return "SPACEMOUSE_CLASSIC";
		case SI_SPACEMOUSE_PLUS: return "SPACEMOUSE_PLUS";
		case SI_SPACEMOUSE_XT: return "SPACEMOUSE_XT";
		case SI_CYBERMAN: return "CYBERMAN";
		case SI_CADMAN: return "CADMAN";
		case SI_SPACEMOUSE_CLASSIC_PROMO: return "SPACEMOUSE_CLASSIC_PROMO";
		case SI_SERIAL_CADMAN: return "SERIAL_CADMAN";
		case SI_SPACEBALL_5000: return "SPACEBALL_5000";
		case SI_TEST_NO_DEVICE: return "TEST_NO_DEVICE";
		case SI_3DX_KEYBOARD_BLACK: return "3DC_KEYBOARD_BLACK";
		case SI_3DX_KEYBOARD_WHITE: return "3DX_KEYBOARD_WHITE";
		case SI_TRAVELER: return "TRAVELER";
		case SI_TRAVELER1: return "TRAVELER1";
		case SI_SPACEBALL_5000A: return "SPACEBALL_5000A";
		case SI_SPACEDRAGON: return "SPACEDRAGON";
		case SI_SPACEPILOT: return "SPACEPILOT";
		case SI_MB: return "MB";
		case SI_SPACEPILOT_PRO: return "SPACEPILOT_PRO";
		case SI_SPACEMOUSE_PRO: return "SPACEMOUSE_PRO";
		case SI_SPACEMOUSE_TOUCH: return "SPACEMOUSE_TOUCH";
		case SI_SPACEMOUSE_WIRELESS: return "SPACEMOUSE_WIRELESS";
		case SI_SPACEMOUSE_PRO_WIRELESS: return "SPACEMOUSE_PRO_WIRELESS";
		case SI_CADMOUSE: return "CADMOUSE";
		case SI_SCOUT: return "SCOUT";
	...
	//! have to have the static SDK_Initializer, to make sure that init and terminate are only called on load-time and
	//! on unload time respectively. This way we can have as many 3dc devices attached / active as we want in the same
	//! process / application.
	struct SDK_Initializer
	{
		...
		, m_class_atom(NULL)
		{
			...
			{
				vstr::errp() << "Error calling SiInitialize() -- " << SpwErrorString(err_code) << " -- 3DC devices will probably not work." << std::endl;
			}
...
struct Vista3DCSpaceNavigator::_sPrivate
{
	...
	public:
		...
		protected:
			...
			{
				m_parent->m_message_window = CreateWindowEx( 0, WindowClassName, "3DCContextWindow", 0, 0,0,0,0, HWND_MESSAGE, 0,0,0 );
				SiOpenWinInit( &m_parent->m_si_open_data,  m_parent->m_message_window );
				if( ( m_parent->m_si_device_handle = SiOpen( "ViSTA", SI_ANY_DEVICE, SI_NO_MASK, SI_EVENT, &m_parent->m_si_open_data ) ) == SI_NO_HANDLE )
				{
				...
				SpwRetVal err_code;
				if( (err_code=SiSetUiMode( m_parent->m_si_device_handle, SI_UI_NO_CONTROLS )) != SPW_NO_ERROR ) 
				{
					vstr::errp() << "Could not disable UI controls: " << SpwErrorString( err_code ) << std::endl;
				}
			}
		...
		protected:
			//! blocking call to GetMessage() to receive update messages from, send WM_USER to break loop (or close of window)
			virtual void DefinedThreadWork()
			{
				MSG            msg;
				BOOL           msg_handled;
				...
				while( GetMessage( &msg, m_parent->m_message_window, 0, 0 ) >= 0 )
				{
					...
					if( SiGetEvent( m_parent->m_si_device_handle, SI_AVERAGE_EVENTS, &m_parent->m_si_event_data, &m_parent->m_si_event ) == SI_IS_EVENT )
					{
						if( m_prepare )
							m_prepare->PrePoll();
		...
		virtual bool Activate()
		{
			if( m_pool.StartPoolWork() ) 
			{
				...
				if( m_pool.WaitForAllJobsDone() )
				{
					...
					if( (err_code = SiGetDeviceName( m_parent->m_si_device_handle, &dev_name ) ) != SPW_NO_ERROR )
					{
...
bool Vista3DCSpaceNavigator::DoSensorUpdate(VistaType::microtime dTs)
{
	...
	case SI_MOTION_EVENT:
		{
			...
			m_pPrivate->m_current_state.m_nLength = VistaVector3D( m_pPrivate->m_current_state.m_nPositionX, 
				m_pPrivate->m_current_state.m_nPositionY,
				m_pPrivate->m_current_state.m_nPositionZ ).GetLength();
	...
	case SI_ZERO_EVENT:
		{
			m_pPrivate->m_current_state.m_nPositionX 
				= m_pPrivate->m_current_state.m_nPositionY 
				= m_pPrivate->m_current_state.m_nPositionZ 
				= m_pPrivate->m_current_state.m_nLength

				= m_pPrivate->m_current_state.m_nRotationX 
				= m_pPrivate->m_current_state.m_nRotationY 
				= m_pPrivate->m_current_state.m_nRotationZ 
				= m_pPrivate->m_current_state.m_nRotationAngle
				= 0;
			break;
		}
