	...
	m_pMeasure(new VistaHIDCommonShare::sHIDMeasure)
{
	...
	m_pConnection->SetConnection(0, NULL, "MAIN", VistaDriverConnectionAspect::ALLOW_CONNECTION_REPLACEMENT, VistaDriverConnectionAspect::REPLACE_ONLY_WHEN_NOT_SET);
...
{
	VistaHIDDriverEnableAspect *enabler = GetAspectAs< VistaHIDDriverEnableAspect >( VistaDriverEnableAspect::GetAspectId() );
...
bool VistaHIDDriver::InitDriver(int nVendor, int nDevId)
{
	...
	while( ( entry = readdir(devdir) ) )
	{
		...
		if( strncmp( entry->d_name, "event", 5 ) == 0 )
		{
			...
			m_pConnection->SetConnection( 0, pFileCon, "MAIN", VistaDriverConnectionAspect::ALLOW_CONNECTION_REPLACEMENT,
