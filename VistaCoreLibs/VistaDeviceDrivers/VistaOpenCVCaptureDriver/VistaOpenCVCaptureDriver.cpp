...
namespace
{
	...
	IVistaPropertySetFunctor *SaParameterSetter[] =
	{
		...
		"FORMAT",
		SsReflectionName,
		&VistaOpenCVCaptureDriver::Parameters::SetFormat,
...
, m_dealloc( new DeallocateBufferPointer )
{
	...
	RegisterAspect( new VistaOpenCVCaptureEnableAspect( this, &VistaOpenCVCaptureDriver::PhysicalEnable ) );
...
VistaOpenCVCaptureDriver::~VistaOpenCVCaptureDriver()
{
	...
	VistaOpenCVCaptureEnableAspect *enabler = GetAspectAs< VistaOpenCVCaptureEnableAspect >( VistaDriverEnableAspect::GetAspectId() );
...
bool VistaOpenCVCaptureDriver::DoConnect()
{
	...
	if( !pSensor )
	{
		...
		pSensor->SetMeasureTranscode( GetFactory()->GetTranscoderFactoryForSensor("VIDEO")->CreateTranscoder() );
	...
	{
		pSensor = new VistaDeviceSensor;
		pSensor->SetMeasureTranscode( GetFactory()->GetTranscoderFactoryForSensor("TYPE")->CreateTranscoder() );
	...
	VistaSensorMeasure *pM = MeasureStart(1, GetUpdateTimer().GetMicroTime(), IVistaDeviceDriver::RETURN_CURRENT_SLOT );
	if( pM )
	{
		Parameters *p = m_pParams->GetParameter<Parameters>();
...
void VistaOpenCVCaptureDriver::OnReconnectCamera(bool bForce)
{
	...
	{
		vstr::err() << "Could not set frame dimensions. OpenCV reported ("
			<< p->GetWidth() << ", " << p->GetHeight() << ") as dimensions";
		delete m_pCapture;
		m_pCapture = 0;
		return;
	}
...
void VistaOpenCVCaptureDriver::OnUpdateMeasureSize()
{
	...
	// for a 'good' access of the history (meaning: the temporal aspects are OK, its just the size per measure
