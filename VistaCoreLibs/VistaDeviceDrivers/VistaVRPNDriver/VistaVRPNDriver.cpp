...
: IVistaDriverCreationMethod(fac)
{
	RegisterSensorType( "ANALOG",
									 ...
									 1000, fac->CreateFactoryForType("ANALOG") );
	RegisterSensorType( "BUTTON",
									 sizeof(_vrpn_BUTTONCB),
									 1000, fac->CreateFactoryForType("BUTTON") );
	RegisterSensorType( "TRACKER",
									 sizeof(_vrpn_TRACKERCB),
									 1000, fac->CreateFactoryForType("TRACKER") );
}
...
namespace
{
			                           ...
			                           const T &data )
	{
		...
		// update start
		VistaSensorMeasure *pM = pDriver->MeasureStart(*pSensor, nUpdateTime, IVistaDeviceDriver::RETURN_CURRENT_SLOT );
