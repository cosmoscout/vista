...
void Usage(const char* progname) {
  std::cout
      << progname
      << " <dfn-xml-file> <driver-name> <vrpn-sensor-address> <ANALOG|BUTTON|TRACKER> <#samples|-1>"
      << std::endl;
  exit(0);
}

bool SetupSensor(IVistaDeviceDriver* pDriver, IVistaDriverCreationMethod* pM,
    const std::string& strSensorAddress, const std::string& strSensorType) {
  VistaDeviceSensor* pTrackerSensor = new VistaDeviceSensor;

  pTrackerSensor->SetMeasureTranscode(
      pM->GetTranscoderFactoryForSensor(strSensorType)->CreateTranscoder());
...
int main(int argc, char** argv) {
  ...
  VdfnNodeFactory::GetSingleton()->SetNodeCreator(
      "quitter", new TVdfnDefaultNodeCreate<DetermineQuit>);
  ...
  if (pSm == NULL) {
    std::cerr << "could not create driver [" << pcDriverName << "] of type VRPN" << std::endl;
    return -2;
  }

  if (pM == NULL) {
    std::cerr << "could not create driver  creation method of type VRPN-GENERIC" << std::endl;
    return -3;
  }
  ...
  if (pSm && pSm->Connect()) {
    VistaDriverInfoAspect* pInfo = dynamic_cast<VistaDriverInfoAspect*>(
