...
static int   udp_send(
      void* sock, void* buffer, int len, unsigned int ipaddr, unsigned short port, int tout_us);
...
DTrack::DTrack(
    int udpport, const char* remote_host, int remote_port, int udpbufsize, int udptimeout_us) {
...
bool DTrack::cmd_send(int cmd, int val) {
  ...
  if (udp_send(d_udpsock, cmdstr, (int)strlen(cmdstr) + 1, d_remote_ip, d_remote_port,
          d_udptimeout_us)) {
    set_udperror();
    return false;
  }
