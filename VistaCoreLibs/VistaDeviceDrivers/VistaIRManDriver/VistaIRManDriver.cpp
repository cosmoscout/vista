...
VistaIRManDriver::~VistaIRManDriver()
{
	...
	UnregisterAspect( m_pConnectionAspect, IVistaDeviceDriver::DO_NOT_DELETE_ASPECT, IVistaDeviceDriver::DO_NOT_FORCE_UNREGISTRATION );
...
bool VistaIRManDriver::DoConnect()
{
	...
	m_pConnectionAspect->SendCommand(0, "I", 1, 1 ); // according to spec: have at least 500microsecs between I and R
	m_pConnectionAspect->SendCommand(0, "R", 1, 1000 ); // post-wait for a second... its serial port communication ;)
	...
	if( sResponse.size() < 2 )
	{
		...
		vstr::err() << "IRMAN DID NOT RESPOND CORRECTLY [expected at least 2 bytes response... but got less]" << std::endl;
		pConnection->Close();
		return false;
	}
