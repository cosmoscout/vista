...
REFL_IMPLEMENT_FULL(VistaLeapMotionDriver::Parameters,
...
{
public:
	LeapListener( VistaLeapMotionDriver* pDriver ) : m_pDriver( pDriver ) {}
	...
	virtual void onFrame( const Leap::Controller& ) { m_pDriver->Update(); }
...
namespace
{
	...
	{
	public:
		GesturePropGetter( const std::string &prop_name, const std::string &class_name, const std::string &desc )
		...
		{
			const VistaLeapMotionDriver::Parameters *params = dynamic_cast< const VistaLeapMotionDriver::Parameters * >( &o );
			if( params == 0 )
				return false;
	...
	{
	public:
		GesturePropSetter( const std::string &prop_name, const std::string &class_name, const std::string &desc )
			: IVistaPropertySetFunctor( prop_name, class_name, desc )
		{}

		virtual bool operator()(IVistaPropertyAwareable &o, const VistaProperty &p)
		{
			VistaLeapMotionDriver::Parameters *params = dynamic_cast< VistaLeapMotionDriver::Parameters * >( &o );
			if( params == 0 )
				return false;

			return params->SetGestureProp( GetNameForNameable(), VistaConversion::ConvertType< float, std::string >( p.GetValue() ) );
		}
	};
	...
	{
	public:
		PolicyPropSetter( const std::string &prop_name, const std::string &class_name, const std::string &desc )
			: IVistaPropertySetFunctor( prop_name, class_name, desc )
		{}
		...
		virtual bool operator()(IVistaPropertyAwareable &o, const VistaProperty &p)
		{
			...
			std::list<std::string> prop_values = VistaConversion::FromString< std::list<std::string> >( p.GetValue() );

			VistaLeapMotionDriver::Parameters::PolicyFlags policy=VistaLeapMotionDriver::Parameters::POLICY_DEFAULT;
			if( std::find( prop_values.begin(), prop_values.end(), "POLICY_BACKGROUND_FRAMES" ) != prop_values.end() ) 
			{
			...
			{
				policy = VistaLeapMotionDriver::Parameters::PolicyFlags( int(policy) | int(VistaLeapMotionDriver::Parameters::POLICY_IMAGES) );
			}

			if( std::find( prop_values.begin(), prop_values.end(), "POLICY_OPTIMIZE_HMD" ) != prop_values.end() ) 
			{
				policy = VistaLeapMotionDriver::Parameters::PolicyFlags( int(policy) | int(VistaLeapMotionDriver::Parameters::POLICY_OPTIMIZE_HMD) );
			}
	...
	IVistaPropertyGetFunctor *SaParameterGetter[] =
	{
		...
		new GesturePropGetter( "Gesture.Circle.MinArc", SsReflectionName, "defaults to 1.5 * pi radians" ),
		...
		new GesturePropGetter( "Gesture.KeyTap.MinDownVelocity", SsReflectionName, "defaults to 50 mm/s" ),
		...
		new GesturePropGetter( "Gesture.ScreenTap.MinForwardVelocity", SsReflectionName, "defaults to 50 mm" ),
		new GesturePropGetter( "Gesture.ScreenTap.HistorySeconds", SsReflectionName, "defaults to 0.1 mm" ),
		new GesturePropGetter( "Gesture.ScreenTap.MinDistance", SsReflectionName, "defaults to 5.0 mm" ),

		new TVistaPropertyGet<VistaType::microtime,
		VistaLeapMotionDriver::Parameters,
		...
		new PolicyGetter( "POLICY", SsReflectionName, "Returns the string list of currently set policy flags" ),
	...
	IVistaPropertySetFunctor *SaParameterSetter[] =
	{
		...
		new GesturePropSetter( "Gesture.Circle.MinArc", SsReflectionName, "defaults to 1.5 * pi radians" ),
		...
		new GesturePropSetter( "Gesture.KeyTap.MinDownVelocity", SsReflectionName, "defaults to 50 mm/s" ),
		...
		new GesturePropSetter( "Gesture.ScreenTap.MinForwardVelocity", SsReflectionName, "defaults to 50 mm" ),
		new GesturePropSetter( "Gesture.ScreenTap.HistorySeconds", SsReflectionName, "defaults to 0.1 mm" ),
		...
		new PolicyPropSetter( "POLICY", SsReflectionName, "Specify policy for leap controller as string list {POLICY_BACKGROUND_FRAMES, POLICY_IMAGES, POLICY_OPTIMIZE_HMD, POLICY_DEFAULT}" ),
...
VistaLeapMotionCreationMethod::VistaLeapMotionCreationMethod( IVistaTranscoderFactoryFactory* pMetaFac )
: IVistaDriverCreationMethod( pMetaFac )
{
	RegisterSensorType( "HAND", sizeof( VistaLeapMotionMeasures::HandMeasure ),
						120, pMetaFac->CreateFactoryForType( "HAND" ) );
	RegisterSensorType( "FINGERS", sizeof( VistaLeapMotionMeasures::FingersMeasure ),
						120, pMetaFac->CreateFactoryForType( "FINGERS" ) );
	RegisterSensorType( "TOOLS", sizeof( VistaLeapMotionMeasures::ToolsMeasure ),
						120, pMetaFac->CreateFactoryForType( "TOOLS" ) );
	RegisterSensorType( "KEYTAP", sizeof( VistaLeapMotionMeasures::GestureMeasure ),
						120, pMetaFac->CreateFactoryForType( "GESTURE" ) );
	RegisterSensorType( "SCREENTAP", sizeof( VistaLeapMotionMeasures::GestureMeasure ),
						120, pMetaFac->CreateFactoryForType( "GESTURE" ) );
	RegisterSensorType( "CIRCLE", sizeof( VistaLeapMotionMeasures::GestureMeasure ),
						120, pMetaFac->CreateFactoryForType( "GESTURE" ) );
	RegisterSensorType( "SWIPE", sizeof( VistaLeapMotionMeasures::GestureMeasure ),
						120, pMetaFac->CreateFactoryForType( "GESTURE" ) );
	RegisterSensorType( "GESTURE", sizeof( VistaLeapMotionMeasures::GestureMeasure ),
						120, pMetaFac->CreateFactoryForType( "GESTURE" ) );
	RegisterSensorType( "IMAGE", sizeof( VistaLeapMotionMeasures::ImageMeasure ),
						120, pMetaFac->CreateFactoryForType( "IMAGE" ) );
}
...
namespace
{
	...
	void LeapToVistaVector( VistaVector3D& v3VistaVector, const Leap::Vector& oLeapVector, const bool bIsPosition = true )
	{
	...
	void FillPointable( VistaLeapMotionMeasures::Pointable& oMeasure, const Leap::Pointable& oPointable, VistaType::uint64 time_stamp )
	{
	...
	void FillFinger( VistaLeapMotionMeasures::Finger& oMeasure, const Leap::Finger& oFinger, VistaType::uint64 time_stamp )
	{
		FillPointable( oMeasure, oFinger, time_stamp );

		oMeasure.m_bExtended = oFinger.isExtended();
		oMeasure.m_nFingerType = oFinger.type();
		LeapToVistaVector( oMeasure.m_v3MetacarpalBoneStartPoint, oFinger.bone( Leap::Bone::TYPE_METACARPAL ).prevJoint() );
		LeapToVistaVector( oMeasure.m_v3ProximalBoneStartPoint, oFinger.bone( Leap::Bone::TYPE_PROXIMAL ).prevJoint() );
		LeapToVistaVector( oMeasure.m_v3IntermediateBoneStartPoint, oFinger.bone( Leap::Bone::TYPE_INTERMEDIATE ).prevJoint() );
		LeapToVistaVector( oMeasure.m_v3DistalBoneStartPoint, oFinger.bone( Leap::Bone::TYPE_DISTAL ).prevJoint() );
	}

	void FillHand( VistaLeapMotionMeasures::HandMeasure& oMeasure, const Leap::Hand& oHand, VistaType::uint64 time_stamp )
	{
		oMeasure.m_driver_timestamp = time_stamp;
		...
		for( Leap::FingerList::const_iterator itFinger = liFingers.begin(); itFinger != liFingers.end(); ++itFinger )
		{
		...
		int nIndex = 0;
		for( Leap::ToolList::const_iterator itTool = liTools.begin(); itTool != liTools.end(); ++itTool, ++nIndex )
		{
	...
	void FillGesture( VistaLeapMotionMeasures::GestureMeasure& oMeasure, const Leap::Gesture& oGesture, VistaType::uint64 time_stamp )
	{
		oMeasure.m_driver_timestamp = time_stamp;
...
, m_nLastFrameId( ~0u )
{
	...
	m_pConfigAspect = new VistaDriverGenericParameterAspect( new TParameterCreate< VistaLeapMotionDriver, VistaLeapMotionDriver::Parameters >(this) );
...
VistaLeapMotionDriver::~VistaLeapMotionDriver()
{
	...
	VistaLeapMotionEnableAspect *enabler = GetAspectAs< VistaLeapMotionEnableAspect >( VistaDriverEnableAspect::GetAspectId() );
...
void VistaLeapMotionDriver::ProcessFrame( const Leap::Frame& oFrame, VistaType::microtime dTs )
{
	...
	for( unsigned int nHand = 0; nHand < nNumHands; ++nHand )
	{
		...
		VistaSensorMeasure* pMeasure = MeasureStart( nSensorId, dTs, IVistaDeviceDriver::RETURN_CURRENT_SLOT );
	...
	{
		VistaSensorMeasure* pMeasure = MeasureStart( nFingersSensorId, dTs, IVistaDeviceDriver::RETURN_CURRENT_SLOT );		
		VistaLeapMotionMeasures::FingersMeasure* pData = pMeasure->getWrite< VistaLeapMotionMeasures::FingersMeasure >();
	...
	{
		VistaSensorMeasure* pMeasure = MeasureStart( nToolsSensorId, dTs, IVistaDeviceDriver::RETURN_CURRENT_SLOT );		
		
		VistaLeapMotionMeasures::ToolsMeasure* pData = pMeasure->getWrite< VistaLeapMotionMeasures::ToolsMeasure >();
	...
	if( GetParameters()->GetPolicyFlags() & VistaLeapMotionDriver::Parameters::POLICY_IMAGES ) 
	{
		...
		for( Leap::ImageList::const_iterator cit = begin; cit != end; ++cit ) 
		{
			...
			{
				VistaSensorMeasure *m = MeasureStart( sensor_id, dTs, IVistaDeviceDriver::RETURN_CURRENT_SLOT );
				VistaLeapMotionMeasures::ImageMeasure *image = m->getWrite< VistaLeapMotionMeasures::ImageMeasure >();
...
void VistaLeapMotionDriver::ProcessGesture( const int nSensorTypeId, const Leap::Gesture& oGesture, const VistaType::microtime nTime, VistaType::uint64 driver_time_stamp )
{
	unsigned int nSensorId = m_pSensors->GetSensorId( nSensorTypeId, 0 );
	if( nSensorId == ~0u )
		return;
	
	VistaSensorMeasure* pMeasure = MeasureStart( nSensorId, nTime,  IVistaDeviceDriver::RETURN_CURRENT_SLOT );
	if( pMeasure == 0 )
		return;

	FillGesture( *pMeasure->getWrite< VistaLeapMotionMeasures::GestureMeasure >(), oGesture, driver_time_stamp );
	MeasureStop(nSensorId);
}
...
bool VistaLeapMotionDriver::PhysicalEnable( bool bEnable )
{
	...
	if( bEnable ) 
	{
		if( m_bListenerIsRegistered == false ) 
		{
			...
			{
				vstr::warnp() << "VistaLeapMotionDriver::PhysicalEnable( true ) -- failed to add listener callback to leap controller" << std::endl;
				m_bListenerIsRegistered = false;
			}
			else
	...
	else
	{
		if( m_bListenerIsRegistered == true ) 
		{
			...
			{
				vstr::warnp() << "VistaLeapMotionDriver::PhysicalEnable( false ) -- failed to remove listener callback from leap controller" << std::endl;
				m_bListenerIsRegistered = true;
			}
			else 
			{
...
bool VistaLeapMotionDriver::DoConnect()
{
	...
	m_pLeapController->setPolicyFlags( (Leap::Controller::PolicyFlag) GetParameters()->GetPolicyFlags() );
...
VistaLeapMotionDriver::Parameters::PolicyFlags VistaLeapMotionDriver::Parameters::GetPolicyFlags() const
{
...
VistaLeapMotionDriver::Parameters::GestureFlags VistaLeapMotionDriver::Parameters::GetGestureFlags() const
{
...
{
	return compAssignAndNotify< VistaType::microtime >( timout_in_s, m_connection_timeout, *this, MSG_CONNECTION_TIMEOUT_CHG );
}
...
{
	return compAssignAndNotify< bool >( connection_state, m_listener_connection_state, *this, MSG_CONNECTION_STATE_CHG );
}
