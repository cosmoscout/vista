  ...
  m_bAttachOnly(false)
{
	...
	// prepare connections
	m_pConnection->SetConnection(DATACON, NULL, "DATACONNECTION", VistaDriverConnectionAspect::ALLOW_CONNECTION_REPLACEMENT);
	m_pConnection->SetConnection(CTRLCON, NULL, "CONTROLCONNECTION", VistaDriverConnectionAspect::ALLOW_CONNECTION_REPLACEMENT);
...
{
	VistaDTrackEnableAspect *enabled = GetAspectAs< VistaDTrackEnableAspect >( VistaDriverEnableAspect::GetAspectId() );
		...
		if( it != m_mapDecoder.end() )
		{
			...
			else
			{
				...
				for(int n=0; n < nNumValues; ++n)
				{
					...
					if((*(*it).second).ReadSingleBlock(m_pLine, vTmp) == (int)( vTmp.size()/sizeof(double) ) )
					{
						// vTmp[0]: id
						// vTmp[1]: qu
						...
						if(pSen)
						{
							...
							VistaDTrackMeasures::sGenericMeasure* pGenericMeasure = pMeasure->getWrite< VistaDTrackMeasures::sGenericMeasure >();
