...
VistaColor::VistaColor(int iColor /*= WHITE */, EFormat eFormat /*= RGB*/) {
  SetValues(iColor, eFormat);
}

VistaColor::VistaColor(int i1, int i2, int i3, EFormat eFormat /*= RGBA*/) {
...
VistaColor::VistaColor(double d1, double d2, double d3, EFormat eFormat /*= RGB*/) {
  SetValues(d1, d2, d3, eFormat);
}

VistaColor::VistaColor(int i1, int i2, int i3, int i4, EFormat eFormat /*= RGBA*/) {
  SetValues(i1, i2, i3, i4, eFormat);
}

VistaColor::VistaColor(float f1, float f2, float f3, float f4, EFormat eFormat /*= RGBA*/) {
  SetValues(f1, f2, f3, f4, eFormat);
}

VistaColor::VistaColor(double d1, double d2, double d3, double d4, EFormat eFormat /*= RGBA*/) {
...
VistaColor::VistaColor(const double adValues[], EFormat eFormat /*= RGB*/) {
...
void VistaColor::SetValues(int i1, int i2, int i3, EFormat eFormat /*= RGB */) {
  float a3fValues[3] = {i1 / 255.f, i2 / 255.f, i3 / 255.f};
...
void VistaColor::SetValues(double d1, double d2, double d3, EFormat eFormat
    /*= RGB */) {
  float a3fValues[3] = {static_cast<float>(d1), static_cast<float>(d2), static_cast<float>(d3)};
  SetValues(a3fValues, eFormat);
}

void VistaColor::SetValues(int i1, int i2, int i3, int i4, EFormat eFormat /*= RGBA*/) {
  float a4fValues[4] = {i1 / 255.f, i2 / 255.f, i3 / 255.f, i4 / 255.f};
  SetValues(a4fValues, eFormat);
}

void VistaColor::SetValues(float f1, float f2, float f3, float f4, EFormat eFormat /*= RGBA*/) {
  float a4fValues[4] = {f1, f2, f3, f4};
  SetValues(a4fValues, eFormat);
}

void VistaColor::SetValues(double d1, double d2, double d3, double d4, EFormat eFormat /*= RGBA*/) {
  float a4fValues[4] = {static_cast<float>(d1), static_cast<float>(d2), static_cast<float>(d3),
      static_cast<float>(d4)};
  SetValues(a4fValues, eFormat);
}

void VistaColor::SetValues(const float afValues[], EFormat eFormat /*= FORMAT_RGB*/) {
  switch (eFormat) {
  case RGB:
...
void VistaColor::SetValues(const double adValues[], EFormat eFormat /*= FORMAT_RGB*/) {
  switch (eFormat) {
  case RGB:
  case HSV:
  case HSL:
  case LAB: {
    float a3fValues[3] = {static_cast<float>(adValues[0]), static_cast<float>(adValues[1]),
  ...
  case HSLA:
  case LABA: {
    float a4fValues[4] = {static_cast<float>(adValues[0]), static_cast<float>(adValues[1]),
        static_cast<float>(adValues[2]), static_cast<float>(adValues[3])};
    SetValues(a4fValues, eFormat);
  } break;
  default:
    VISTA_THROW("Unknown color mode", 1);
...
void VistaColor::SetValues(int iColor, EFormat eFormat /*= FORMAT_RGB*/) {
  float a4fDecomp[4] = {((iColor & 0xFF000000) >> 24) / 255.0f,
      ((iColor & 0x00FF0000) >> 16) / 255.0f, ((iColor & 0x0000FF00) >> 8) / 255.0f,
...
void VistaColor::SetValues(const int aiValues[], EFormat eFormat /*= FORMAT_RGB*/) {
  switch (eFormat) {
  case RGB:
  case HSV:
  case HSL:
...
int VistaColor::GetValues(EFormat eFormat /*= FORMAT_RGB*/) const {
  ...
  case LAB: {
    ...
    return (static_cast<int>(a3fValues[0] * 255.f) << 16) +
           (static_cast<int>(a3fValues[1] * 255.f) << 8) + static_cast<int>(a3fValues[2] * 255.f);
  }
  case RGBA:
  case HSVA:
  case HSLA:
  case LABA: {
    ...
    return (static_cast<int>(a4fValues[0] * 255.f) << 24) +
           ...
           (static_cast<int>(a4fValues[2] * 255.f) << 8) + static_cast<int>(a4fValues[3] * 255.f);
  }
  default:
    VISTA_THROW("Unknown color mode", 1);
  }
...
void VistaColor::GetValues(int aiValues[], EFormat eFormat /*= FORMAT_RGB*/) const {
  switch (eFormat) {
  case RGB:
  case HSV:
  case HSL:
...
void VistaColor::GetValues(float afValues[], EFormat eFormat /*= FORMAT_RGB*/) const {
  switch (eFormat) {
  case RGB:
...
void VistaColor::GetValues(double adValues[], EFormat eFormat /*= FORMAT_RGB*/) const {
  switch (eFormat) {
  case RGB:
  case HSV:
  case HSL:
...
float VistaColor::GetHSLSaturation() const {
  float fMin =
      std::min(std::min(m_a4fValues[0], m_a4fValues[1]), m_a4fValues[2]); // Min. value of RGB
  float fMax =
      std::max(std::max(m_a4fValues[0], m_a4fValues[1]), m_a4fValues[2]); // Max. value of RGB
...
VistaColor VistaColor::Mix(const VistaColor& colEnd, float fFraction /*= .f5*/,
    EFormat eFormat /* = VistaColor::RGBA */) const {
  float fOwn = 1.0f - fFraction;
  switch (eFormat) {
  case RGBA:
    return VistaColor(fOwn * m_a4fValues[0] + fFraction * colEnd.m_a4fValues[0],
        fOwn * m_a4fValues[1] + fFraction * colEnd.m_a4fValues[1],
        fOwn * m_a4fValues[2] + fFraction * colEnd.m_a4fValues[2],
        fOwn * m_a4fValues[3] + fFraction * colEnd.m_a4fValues[3], RGBA);
  case RGB:
    return VistaColor(fOwn * m_a4fValues[0] + fFraction * colEnd.m_a4fValues[0],
        fOwn * m_a4fValues[1] + fFraction * colEnd.m_a4fValues[1],
        fOwn * m_a4fValues[2] + fFraction * colEnd.m_a4fValues[2], RGB);
  case HSVA:
    return VistaColor(fOwn * GetHue() + fFraction * colEnd.GetHue(),
        fOwn * GetHSVSaturation() + fFraction * colEnd.GetHSVSaturation(),
        fOwn * GetValue() + fFraction * colEnd.GetValue(),
        fOwn * m_a4fValues[3] + fFraction * colEnd.m_a4fValues[3], HSVA);
  case HSV:
    return VistaColor(fOwn * GetHue() + fFraction * colEnd.GetHue(),
        fOwn * GetHSVSaturation() + fFraction * colEnd.GetHSVSaturation(),
        fOwn * GetValue() + fFraction * colEnd.GetValue(), HSV);
  case HSLA:
    return VistaColor(fOwn * GetHue() + fFraction * colEnd.GetHue(),
        fOwn * GetHSLSaturation() + fFraction * colEnd.GetHSLSaturation(),
        fOwn * GetLightness() + fFraction * colEnd.GetLightness(),
        fOwn * m_a4fValues[3] + fFraction * colEnd.m_a4fValues[3], HSLA);
  case HSL:
    return VistaColor(fOwn * GetHue() + fFraction * colEnd.GetHue(),
        fOwn * GetHSLSaturation() + fFraction * colEnd.GetHSLSaturation(),
        fOwn * GetLightness() + fFraction * colEnd.GetLightness(), HSL);
  case LAB: {
    float a3Temp[3], a3Temp2[3];
    RGBtoLab(m_a4fValues, a3Temp);
    RGBtoLab(&colEnd[0], a3Temp2);
    float      aColorMix[3] = {fOwn * a3Temp[0] + fFraction * a3Temp2[0],
        fOwn * a3Temp[1] + fFraction * a3Temp2[1], fOwn * a3Temp[2] + fFraction * a3Temp2[2]};
    VistaColor colRes;
...
void VistaColor::ClampValues() {
  for (int i = 0; i < 4; ++i) {
    if (m_a4fValues[i] < 0.f)
      m_a4fValues[i] = 0.f;
    if (m_a4fValues[i] > 1.f)
      m_a4fValues[i] = 1.f;
  }
}
...
void VistaColor::LabtoRGB(const float a3fLAB[3], float a3fRGB[3]) {
  ...
  a3fRGB[0] =
      a3fRGB[0] > .0031308f ? std::pow(a3fRGB[0], .4166667f) * 1.055f - .055f : a3fRGB[0] * 12.92f;
  a3fRGB[1] =
      a3fRGB[1] > .0031308f ? std::pow(a3fRGB[1], .4166667f) * 1.055f - .055f : a3fRGB[1] * 12.92f;
  a3fRGB[2] =
      a3fRGB[2] > .0031308f ? std::pow(a3fRGB[2], .4166667f) * 1.055f - .055f : a3fRGB[2] * 12.92f;
}

void VistaColor::RGBtoLab(const float a3fRGB[3], float a3fLAB[3]) {
  float r =
      a3fRGB[0] > 0.04045f ? std::pow((a3fRGB[0] + 0.055f) / 1.055f, 2.4f) : a3fRGB[0] / 12.92f;
  float g =
      a3fRGB[1] > 0.04045f ? std::pow((a3fRGB[1] + 0.055f) / 1.055f, 2.4f) : a3fRGB[1] / 12.92f;
  float b =
      a3fRGB[2] > 0.04045f ? std::pow((a3fRGB[2] + 0.055f) / 1.055f, 2.4f) : a3fRGB[2] / 12.92f;
