...
VistaSerializingToolset::eEndianess VistaSerializingToolset::GetPlatformEndianess() {
  if (thisMachine == VST_NONEENDIAN) {
    ...
    if (iVal.m_nVal == 1)
      ...
      thisMachine = VST_BIGENDIAN; // well, no, we do not... this is littleendian? (we might inspect
                                   // this closer, but we do not ;)
...
void VistaSerializingToolset::Swap(
    void* pBuffer, VistaType::uint32 iLength, VistaType::uint32 iCount) {
#if 1
  void (*pSwapFunc)(void*);
  switch (iLength) {
  case 2:
...
VistaSerializingToolset::GetDefaultPlatformSwapBehavior() {
  return (GetPlatformEndianess() == VST_BIGENDIAN ? DOES_NOT_SWAP_MULTIBYTE_VALUES
