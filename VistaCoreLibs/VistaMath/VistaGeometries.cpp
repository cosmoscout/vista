...
bool VistaPlane::CalcIntersection(const VistaRay& ray, float& lambda) const
{
	...
	{
		//if(std::abs(m_parD + m_vNormVector * ray.GetOrigin()) < Vista::Epsilon) // the ray lies in the plane
		//{
		//	lambda = 0.0f;
		//	return true;
		//}
										...
										VistaVector3D& resolutionPoint) const
{
	...
	else
	{
		nLineFactor    
			=  ( ( CalcDistanceToZero() - 
			   ( m_vNormVector * testLine.GetOrigin() ) )
...
int VistaPlane::CalcOrientation(const float px, const float py, const float pz) const
{
	...
	if (std::abs(planeEq) <= TOLERANCE) return 0; 
