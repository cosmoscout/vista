...
VistaPropertyFunctorRegistry* VistaPropertyFunctorRegistry::m_pSingleton = NULL;
...
VistaPropertyFunctorRegistry::~VistaPropertyFunctorRegistry() {
  for (GETFMAP::iterator it = m_mpGetters.begin(); it != m_mpGetters.end(); ++it) {
    delete (*it).second; // free memory for functor
  }

  for (SETFMAP::iterator it1 = m_mpSetters.begin(); it1 != m_mpSetters.end(); ++it1) {
...
bool VistaPropertyFunctorRegistry::RegisterGetter(const std::string& sPropertyName,
    const std::string& sClassType, IVistaPropertyGetFunctor* pFunctor, bool bForce) {
  ...
  if (cit == m_mpGetters.end()) {
    ...
    if (bForce) {
      ...
      if (m_bTalkative) {
        // utter a warning
        vstr::warnp() << "VistaPropertyFunctorRegistry::RegisterGetter(" << sPropertyName << ", "
                      << sClassType << ", " << pFunctor << ", " << (bForce ? "TRUE" : "FALSE")
                      << ") -- functor for prop and class already registered" << std::endl;
      }
    }
  }
...
bool VistaPropertyFunctorRegistry::UnregisterGetter(
    const std::string& sPropName, const std::string& sClassType) {
  ...
  if (sit != (*cmp).second.end()) {
    ...
    if (m_bTalkative) {
      vstr::outi() << "VistaPropertyFunctorRegistry::UnregisterGetter(" << sPropName << ", "
                   << sClassType << ")" << std::endl;
    }
...
IVistaPropertyGetFunctor* VistaPropertyFunctorRegistry::GetGetFunctor(const std::string& sPropName,
    const std::string& sClassType, const std::list<std::string>& rLiBaseCl, bool bUseCache) const {
  if (bUseCache) {
  ...
  if (pit == m_mpGProps.end()) {
    if (m_bTalkative) {
      ...
      vstr::warnp() << "VistaPropertyFunctorRegistry::GetGetFunctor(" << sPropName << ", "
                    << sClassType << ") -- no prop for this name defined" << std::endl;
    }
    return NULL;
  }
  ...
  if (!sTmp.empty()) {
    ...
    if (cit == m_mpGetters.end()) {
      if (m_bTalkative) {
        // weird error
        vstr::warnp() << "VistaPropertyFunctorRegistry::GetGetFunctor(" << sPropName << ", "
                      << sClassType << ") -- no functor for class (should not happen!)"
                      << std::endl;
      }
      return NULL;
    }
...
bool VistaPropertyFunctorRegistry::RegisterSetter(const std::string& sPropertyName,
    const std::string& sClassType, IVistaPropertySetFunctor* pFunctor, bool bForce) {
  // first lookup key in attrib to class map
  ...
  if (cit == m_mpSetters.end()) {
    ...
    if (bForce) {
      ...
      if (m_bTalkative) {
        // utter a warning
        vstr::warnp() << "VistaPropertyFunctorRegistry::RegisterSetter(" << sPropertyName << ", "
                      << sClassType << ", " << pFunctor << ", " << (bForce ? "TRUE" : "FALSE")
                      << ") -- functor for prop and class already registered" << std::endl;
      }
    }
...
bool VistaPropertyFunctorRegistry::UnregisterSetter(
    const std::string& sPropName, const std::string& sClassType) {
  ...
  if (sit != (*cmp).second.end()) {
    ...
    m_mpSetterCache.clear(); // clear cache map
    if (m_bTalkative) {
      vstr::outi() << "VistaPropertyFunctorRegistry::UnregisterSetter(" << sPropName << ", "
                   << sClassType << ")" << std::endl;
    }
    return true;
  }
...
IVistaPropertySetFunctor* VistaPropertyFunctorRegistry::GetSetFunctor(const std::string& sPropName,
  ...
  if (pit == m_mpSProps.end()) {
    if (m_bTalkative) {
      // we can abort the search here
      vstr::warnp() << "VistaPropertyFunctorRegistry::GetSetFunctor(" << sPropName << ", "
                    << sClassType << ") -- no prop for this name defined" << std::endl;
    }
    return NULL;
  }
  ...
  // ok, search the set for the least known symbol
  std::string sTmp;
  // for(std::list<std::string>::const_iterator lcit = rLiBaseCl.begin(); lcit != rLiBaseCl.end();
  // ++lcit)
  for (std::list<std::string>::const_reverse_iterator lcit = rLiBaseCl.rbegin();
       lcit != rLiBaseCl.rend(); ++lcit) {
    if ((*pit).second.find(*lcit) != (*pit).second.end()) {
...
std::list<IVistaPropertyGetFunctor*> VistaPropertyFunctorRegistry::GetGetterByClass(
    const std::string& strExactMatch) const {
...
std::list<IVistaPropertySetFunctor*> VistaPropertyFunctorRegistry::GetSetterByClass(
    const std::string& strExactMatch) const {
...
int VistaPropertyFunctorRegistry::GetGetterSymbolSet(
    std::set<std::string>& setStore, const std::list<std::string>& rLiBaseCl) const {
  for (GETFMAP::const_iterator cit = m_mpGetters.begin(); cit != m_mpGetters.end(); ++cit) {
    std::list<std::string>::const_iterator lit =
        find(rLiBaseCl.begin(), rLiBaseCl.end(), (*cit).first.second);
...
template <class T>
class _printElement //: public std::unary_function<std::map<std::pair<std::string, std::string>,
                    //typename T*>::value_type, void>
{
 public:
  ...
  void operator()(const typename MAPTYPE::value_type& value) {
    m_oStream << "[" << value.first.first << " ; " << value.first.second << "] -- "
              << (value.second ? value.second->GetNameForNameable() : "<NONE>?") << std::endl;
  }
...
void VistaPropertyFunctorRegistry::Print(std::ostream& oStream) const {
  ...
  std::for_each(
      m_mpGetters.begin(), m_mpGetters.end(), _printElement<IVistaPropertyGetFunctor>(oStream));

  oStream << "[SETTERS]" << std::endl;
  std::for_each(
      m_mpSetters.begin(), m_mpSetters.end(), _printElement<IVistaPropertySetFunctor>(oStream));

  oStream << "[GetPMaps]" << std::endl;
  std::for_each(m_mpGProps.begin(), m_mpGProps.end(), _printElement<STRINGMAP>(oStream));

  oStream << "[GetSMaps]" << std::endl;
  std::for_each(m_mpSProps.begin(), m_mpSProps.end(), _printElement<STRINGMAP>(oStream));
}
