...
double VistaAspectsConversionStuff::ConvertToDouble(const string& sValue, bool bCheckPoint) {
  if (bCheckPoint) {
    ...
    for (string::const_iterator cit = sValue.begin(); cit != sValue.end(); ++cit) {
      if (*cit == ',')
        vTmp.push_back('.');
      else
        vTmp.push_back(*cit);
...
unsigned int VistaAspectsConversionStuff::ConvertToList(
    const string& sValue, list<string>& liStrings, char cSep, bool bTrim) {
  if (sValue.empty())
    return 0;
  ...
  while (iPos0 < sValue.size()) {
    if (bTrim) {
      ...
      while (iPos0 < sValue.size() && isspace(sValue[iPos0]))
        ++iPos0;
    }

    // find the separator
    iPos1 = iPos0;
    while (iPos1 < sValue.size() && sValue[iPos1] != cSep)
      ++iPos1;

    iPos2 = iPos1;
    if (bTrim) {
      // remove trailing spaces
      while (iPos0 < iPos1 && isspace(sValue[iPos1 - 1]))
        --iPos1;
    }
...
unsigned int VistaAspectsConversionStuff::ConvertToList(
    const string& sValue, list<double>& liDouble, char cSep) {
...
unsigned int VistaAspectsConversionStuff::ConvertToList(
    const string& sValue, list<float>& liFloat, char cSep) {
  list<string> liStrings;
  if (ConvertToList(sValue, liStrings, cSep)) {
    liFloat.clear();
    list<string>::iterator itString;
...
unsigned int VistaAspectsConversionStuff::ConvertToList(
    const string& sValue, list<bool>& liBool, char cSep) {
  list<string> liStrings;
  if (ConvertToList(sValue, liStrings, cSep)) {
    liBool.clear();
    list<string>::iterator itString;
...
unsigned int VistaAspectsConversionStuff::ConvertToList(
    const string& sValue, list<int>& liInt, char cSep) {
  list<string> liStrings;
  if (ConvertToList(sValue, liStrings, cSep)) {
    liInt.clear();
    list<string>::iterator itString;
...
T* ConvertToArrayT(const list<T>& liInput) {
  ...
  int iPos  = 0;
  for (LCIT cit = liInput.begin(); cit != liInput.end(); ++cit, ++iPos) {
    pTemp[iPos] = (*cit);
  };
  return pTemp;
}
...
int VistaAspectsConversionStuff::ConvertToList(
    const bool* pArray, int iCount, list<bool>& liOutput) {
  return ConvertToListT(pArray, iCount, liOutput);
}

int VistaAspectsConversionStuff::ConvertToList(
    const char* pArray, int iCount, list<char>& liOutput) {
  return ConvertToListT(pArray, iCount, liOutput);
}
...
int VistaAspectsConversionStuff::ConvertToList(
    const unsigned int* pArray, int iCount, list<unsigned int>& liOutput) {
  return ConvertToListT(pArray, iCount, liOutput);
}

int VistaAspectsConversionStuff::ConvertToList(
    const float* pArray, int iCount, list<float>& liOutput) {
  return ConvertToListT(pArray, iCount, liOutput);
}

int VistaAspectsConversionStuff::ConvertToList(
    const double* pArray, int iCount, list<double>& liOutput) {
  return ConvertToListT(pArray, iCount, liOutput);
}

bool VistaAspectsConversionStuff::ConvertStringTo2Float(
    const std::string& sString, float& a, float& b) {
  return (sscanf(sString.c_str(), "%f, %f", &a, &b) == 2);
}

bool VistaAspectsConversionStuff::ConvertStringTo2Double(
    const std::string& sString, double& a, double& b) {
  return (sscanf(sString.c_str(), "%lf, %lf", &a, &b) == 2);
}

bool VistaAspectsConversionStuff::ConvertStringTo3Float(
    const std::string& sString, float& a, float& b, float& c) {
  return (sscanf(sString.c_str(), "%f, %f, %f", &a, &b, &c) == 3);
}

bool VistaAspectsConversionStuff::ConvertStringTo4Float(
    const std::string& sString, float& a, float& b, float& c, float& d) {
...
bool VistaAspectsConversionStuff::ConvertStringTo3Int(
    const std::string& sString, int& a, int& b, int& c) {
  return (sscanf(sString.c_str(), "%d, %d, %d", &a, &b, &c) == 3);
}

bool VistaAspectsConversionStuff::ConvertStringTo4Double(
    const std::string& sString, double& a, double& b, double& c, double& d) {
...
bool VistaAspectsComparisonStuff::StringCaseInsensitiveEquals(
    const std::string& sString1, const std::string& sString2) {
#ifdef WIN32
...
bool VistaAspectsComparisonStuff::StringLess(
    const std::string& sString1, const std::string& sString2) {
  return (strcmp(sString1.c_str(), sString2.c_str()) < 0);
}

bool VistaAspectsComparisonStuff::StringCaseInsensitiveLess(
    const std::string& sString1, const std::string& sString2) {
#ifdef WIN32
  return (stricmp(sString1.c_str(), sString2.c_str()) < 0);
#elif defined(LINUX) || defined(SUNOS) || defined(IRIX) || defined(HPUX)
...
VistaAspectsComparisonStuff::StringCompareObject::StringCompareObject(const bool bCaseSensitive)
...
VistaAspectsComparisonStuff::StringLessObject::StringLessObject(const bool bCaseSensitive)
    : m_bCaseSensitive(bCaseSensitive) {
}
