...
int IVistaObserveable::DetachObserver(IVistaObserver *pObserver)
{
	...
	std::vector<OBSERVER_INFO>::iterator it = std::remove(
			m_vecVistaObservers.begin(), m_vecVistaObservers.end(), pObserver );
...
{
	std::vector<OBSERVER_INFO>::const_iterator cit = std::find(m_vecVistaObservers.begin(),
		m_vecVistaObservers.end(), pObs);
...
void IVistaObserveable::AttachObserverDeferred(IVistaObserver* pObs, int eTicket /*= TICKET_NONE*/)
{
...
void IVistaObserveable::ProcessDeferredActions()
{
	...
	for (size_t i = 0; i < m_vecDeferredActions.size(); ++i)
	{
		...
		{
			AttachObserver(
				m_vecDeferredActions[i].first.m_pObserver,
				m_vecDeferredActions[i].first.m_eTicket);
		}
		else // detach
		{
