 ...
 *  - when C is an A, and A:prop[R](f1) and C:prop[R](f2) then f2 will be used to resolve 'prop' on
 * a C
 *
...
* @param pFunctor the functor to call, must not be NULL.
   ...
   *         else true
   */
  bool RegisterGetter(const std::string& sPropertyName, const std::string& sClassType,
      IVistaPropertyGetFunctor* pFunctor, bool bForce = false);
   ...
   * @return true for:
             - properties that were not registered before
                     ...
                     - properties that were unregistered, so
GetGetFunctor(sPropName,sClassType,baseClasses) will return NULL

                     false else.
*
   */
  bool UnregisterGetter(const std::string& sPropName, const std::string& sClassType);
  ...
  IVistaPropertyGetFunctor* GetGetFunctor(const std::string& sPropName,
      const std::string& sClassType, const std::list<std::string>& rLiBaseCl,
...
* @param pFunctor the functor to call, must not be NULL.
   ...
   *         else true
   */
  bool RegisterSetter(const std::string& sPropertyName, const std::string& sClassType,
      IVistaPropertySetFunctor* pFunctor, bool bForce = false);

  /**
   * @brief unregister getter functors for a property and class.
   *
   * Note that functors associated with the property are not deleted during unregistration.
   * @see GetSetFunctor() to claim the pointer for a delete if you do not hold it elsewhere before
unregistering.
   *
   * @param sPropName the name of the property to unregister
   * @param sClassType the class type for the property to unregister
...
*
   */
  bool UnregisterSetter(const std::string& sPropName, const std::string& sClassType);
   ...
   * @return setStore.size()
   */
  int GetSymbolSet(std::set<std::string>& setStore, const std::list<std::string>& rLiBaseCl) const;
