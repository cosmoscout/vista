 ...
 * be printed!
 */
VISTATOOLSAPI bool MakeStreamThreadSafe(std::ostream& oStream, const bool bBufferInternally = true);
VISTATOOLSAPI bool MakeStreamThreadSafe(std::ostream* pStream, const bool bBufferInternally = true);
 ...
 * Syntax:
 * THREADSAFE = TRUE | FALSE			determines wether or not default streams should be
 *threadsafe (default TRUE) OUT | WARN | ERR | DEBUG | customname = COUT | CLOG | CERR
 *outputs to standard output stream
 *			stream					    outputs to another stream (defined in
 *this section or already registered with stream manager) NULL
 *outputs nothing FILE( filename, [ADD_TIME], [ADD_NODENAME], [APPEND_TO_FILE] )	outputs to a
 *file COLOR( textcolor, [backgroundcolor], [USE_ERROR_STREAM] )	outputs to a color stream
 *			SPLIT( stream1, stream2 ... )	outputs to all streams in the list
 *          BUILDTYPE( releasestream, debugstream )	outputs to where stream1 is used in release
 *mode and stream2 in debug PREFIX( stream, prefix, [INDENT], [ADD_NODENAME] ) for each newline
 *written to PREFIX, the prefix string is written to (minus optional surrounding "") if ADD_NODENAME
 *is set, a nodename (formatted '[nodename]: ' ) ist prepended to the prefix if INDENT is set, the
 *vtsr::indent is preprended may be used recursive, e.g. BUILDTYPE( PREFIX( COLOR( GREEN ),
 *"GreenStream - " ), NULL )
 */
VISTATOOLSAPI bool CreateStreamsFromProplist(
    const VistaPropertyList& oConfig, const bool bAllowColorStreams = true);
VISTATOOLSAPI bool CreateStreamsFromProplist(const VistaPropertyList& oConfig,
    const std::string& sNodename, const bool bAllowColorStreams = true);
} // namespace VistaStreams
...
class VISTATOOLSAPI VistaColorOutstream : public std::ostream {
 ...
 public:
  VistaColorOutstream(const CONSOLE_COLOR iTextColor = CC_DEFAULT,
      const CONSOLE_COLOR iBackgroundColor = CC_DEFAULT, const bool bUseErrorStream = false,
...
class VISTATOOLSAPI VistaCallbackPrefixOutstream : public VistaPrefixOutstream {
 public:
  ...
  class StaticFunctionCallback : public ICallback {
   public:
    ...
    StaticFunctionCallback(CallbackFunction pfCallback)
  ...
  class MemberFunctionCallback : public ICallback {
   public:
    ...
    MemberFunctionCallback(T* pObject, CallbackFunction pfCallback)
        : m_pObject(pObject)
        , m_pfCallback(pfCallback){};
    virtual std::string GetPrefix() const {
 ...
 public:
  VistaCallbackPrefixOutstream(
      std::ostream* oOriginalStream, ICallback* pCallback, bool bManageCallbackDeletion = false);
  ~VistaCallbackPrefixOutstream();
...
class VISTATOOLSAPI VistaNullOutstream : public std::ostream {
 public:
  ...
  template <typename T>
  inline VistaNullOutstream& operator<<(const T&) {
    return *this;
  }
  inline VistaNullOutstream& operator<<(std::ostream& (*pf)(std::ostream&)) {
    return *this;
  }
  inline VistaNullOutstream& operator<<(std::ios& (*pf)(std::ios&)) {
    return *this;
  }
  inline VistaNullOutstream& operator<<(std::ios_base& (*pf)(std::ios_base&)) {
    return *this;
  }
