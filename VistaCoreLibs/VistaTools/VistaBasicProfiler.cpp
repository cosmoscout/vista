...
VistaBasicProfiler::ProfileScopeObject::ProfileScopeObject(
    const std::string& sName, VistaBasicProfiler* pProf)
    : m_sName(sName)
    , m_pProfiler(pProf) {
  m_pProfiler->StartSection(sName);
}
...
bool VistaBasicProfiler::StopSection(const std::string& sName) {
  ...
  if (m_pProfileCurrent->m_sName == sName) {
    ...
    std::cerr << "[VistaBasicProfiler]: trying to end section [" << sName << "], but section ["
              << m_pProfileCurrent->m_sName << "] is active!" << std::endl;
    return false;
  }
}
...
void VistaBasicProfiler::PrintProfile(std::ostream& oStream, int iMaxDepth) {
  ...
  oStream << std::string(iMaxDepth + m_iMaxNameLength - 8, ' ') << "SectionName"
...
bool VistaBasicProfiler::PrintMultipleProfiles(std::ostream& oStream,
    const std::vector<std::string>& vecNames, const std::vector<VistaBasicProfiler*>& vecProfilers,
    int iMaxDepth) {
  ...
  int nRowWidth =
      nSectionNameColWidth + 2 + nTypeColWidth + 3 + (nMeasureColWidth + 2) * (int)vecNodes.size();
  ...
  oStream << "\n";
  oStream << std::string(nRowWidth, '-') << "\n";
  DoPrintMultipleProfiles(
      oStream, vecNodes, 0, iMaxDepth, nSectionNameColWidth, nMeasureColWidth, nTypeColWidth);
  oStream << std::string(nRowWidth, '-') << "\n";
...
void VistaBasicProfiler::DoPrintMultipleProfiles(std::ostream& oStream,
    const std::vector<ProfileTreeNode*>& vecProfilers, int nDepth, int iMaxDepth, int nNameColWidth,
    int nEntryColWidth, int nTypeColWidth) {
  int nRowWidth =
      nNameColWidth + 2 + nTypeColWidth + 3 + (nEntryColWidth + 2) * (int)vecProfilers.size();
  oStream << std::string(nRowWidth, '-') << "\n";
  ...
  oStream << std::string(nDepth, '=') << sName
          << std::string(std::max(nNameColWidth - nDepth - (int)sName.size(), 0), ' ') << " |"
  ...
  for (std::size_t i = 0; i < vecProfilers.size(); ++i) {
    if (vecProfilers[i] == NULL)
      continue;
    for (std::list<ProfileTreeNode*>::const_iterator itChild =
             vecProfilers[i]->m_liChildren.begin();
  ...
  for (;;) {
    ...
    for (std::size_t i = 0; i < vecProfilers.size(); ++i) {
      ...
      if (sChosenSection == sSection)
        continue;
      if (sChosenSection.empty() == false &&
          mapSectionOccurences[sChosenSection] <= mapSectionOccurences[sSection])
        continue;
    ...
    DoPrintMultipleProfiles(
        oStream, vecChildren, nDepth + 1, iMaxDepth, nNameColWidth, nEntryColWidth, nTypeColWidth);
  }
}
...
VistaBasicProfiler::ProfileTreeNode::ProfileTreeNode(
    const std::string& sName, ProfileTreeNode* pParent)
    : m_pParent(pParent)
    , m_sName(sName)
    , m_dCurrentFrame(0)
    , m_dLastFrame(0)
...
VistaBasicProfiler::ProfileTreeNode* VistaBasicProfiler::ProfileTreeNode::Sub(
...
void VistaBasicProfiler::ProfileTreeNode::Print(
    std::ostream& oStream, int iDepth, int iMaxDepth, int iMaxNameLength) {
  if (m_mapChildren.empty()) {
    oStream << std::string(iDepth, ' ') << "---" << std::left
            << std::setw(iMaxNameLength + iMaxDepth - iDepth) << m_sName << std::right << " || "
...
int VistaBasicProfiler::ProfileTreeNode::SerializeNode(IVistaSerializer& oSerializer) {
  ...
  for (std::list<ProfileTreeNode*>::iterator itNode = m_liChildren.begin();
       itNode != m_liChildren.end(); ++itNode) {
...
VistaBasicProfiler::ProfileTreeNode* VistaBasicProfiler::ProfileTreeNode::GetChild(
    const std::string& sName, bool bSearchWholeSubtree) const {
...
bool VistaBasicProfiler::ProfileTreeNode::operator<(
    const VistaBasicProfiler::ProfileTreeNode& oCompare) {
