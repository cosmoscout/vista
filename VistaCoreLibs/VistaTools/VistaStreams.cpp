...
void ApplyBackgroundColor(
    const VistaColorOutstream::CONSOLE_COLOR iColor, WORD& oConsoleAttributes) {
  oConsoleAttributes &=
...
class VistaColorOutstream::Buffer : public std::streambuf {
 public:
  Buffer(const size_t iBufferSize, const bool bUseErrorStream)
      : std::streambuf()
    ...
    if (m_ConsoleRefCount == 0) {
      m_hConsole = CreateFile(
          "CONOUT$", GENERIC_READ | GENERIC_WRITE, FILE_SHARE_WRITE, NULL, OPEN_EXISTING, 0, NULL);
    }
    ++m_ConsoleRefCount;
 ...
 protected:
  ...
  bool Flush() {
    ...
    // WindowsXP has problems with directly writing to the console, so we just use the stream
    // instead
    if (IsWinXPOrEarlier()) {
...
VistaColorOutstream::VistaColorOutstream(const CONSOLE_COLOR iTextColor,
    const CONSOLE_COLOR iBackgroundColor, const bool bUseErrorStream, const size_t iBufferSize)
...
VistaColorOutstream::CONSOLE_COLOR VistaColorOutstream::GetConsoleColorFromString(
...
class VistaPrefixOutstream::Buffer : public std::streambuf {
 public:
  ...
  bool GetPrefixIndent() const {
    return m_bPrefixIndent;
  }
  void SetPrefixIndent(const bool oValue) {
    m_bPrefixIndent = oValue;
  }
...
VistaPrefixOutstream::VistaPrefixOutstream(const VistaPrefixOutstream& oCopy)
  ...
  const Buffer* pOrigBuffer = static_cast<Buffer*>(oCopy.rdbuf());
  m_pBuffer                 = static_cast<Buffer*>(rdbuf());
...
VistaCallbackPrefixOutstream::VistaCallbackPrefixOutstream(
    std::ostream* pOriginalStream, ICallback* pCallback, bool bManageCallbackDeletion)
...
void VistaCallbackPrefixOutstream::SetCallback(
    ICallback* pfCallback, bool bManageCallbackDeletion) {
...
class ThreadSafeStreamBuffer : public std::streambuf {
 public:
  ThreadSafeStreamBuffer(std::streambuf* pOrigBuffer, const bool bBufferInternally)
      : std::streambuf()
 ...
 protected:
  ...
  virtual std::streambuf* setbuf(char_type*, std::streamsize) {
    VISTA_THROW(
        "ThreadSafeStreamBuffer does not allow changing the target buffer after creation", -1);
  }
...
bool VistaStreams::MakeStreamThreadSafe(std::ostream& oStream, const bool bBufferInternally) {
...
VISTATOOLSAPI bool VistaStreams::MakeStreamThreadSafe(
    std::ostream* pStream, const bool bBufferInternally) {
...
std::ostream* CreateStreamFromDescription(const std::string& sDefinition,
    const VistaPropertyList& oConfig, const bool bThreadSafeStream,
    const std::list<std::string>& liStreamsToCreate, const std::string& sNodename, int& nResult,
    ...
    for (std::size_t nPos = nBraceStart + 1; nPos != nBraceEnd; ++nPos) {
      ...
      if (cChar == '(')
        ...
        if (bBeforeFirstSign == false) {
          vecArguments.push_back(
              sDefinition.substr(nCommandStart, nCommandEnd - nCommandStart + 1));
        }
  ...
  if (VistaAspectsComparisonStuff::StringCaseInsensitiveEquals(sCommand, "COUT")) {
    ...
    if (vecArguments.empty()) {
      vstr::warnp() << "VistaStreams::CreateStreamsFromProplist -- "
                    << "Call to \"FILE\" missing arguments - specify at least filename"
                    << std::endl;
      return NULL;
    }
    ...
    for (std::vector<std::string>::const_iterator itParam = vecArguments.begin() + 1;
      ...
      if (VistaAspectsComparisonStuff::StringCaseInsensitiveEquals((*itParam), "ADD_TIME")) {
        sFilename += "_" + VistaTimeUtils::ConvertToLexicographicDateString(
                               VistaTimeUtils::GetStandardTimer().GetSystemTime());
      } else if (VistaAspectsComparisonStuff::StringCaseInsensitiveEquals(
                     (*itParam), "ADD_NODENAME")) {
        if (sNodename.empty() == false)
          sFilename += "_" + sNodename;
      } else if (VistaAspectsComparisonStuff::StringCaseInsensitiveEquals(
                     (*itParam), "APPEND_TO_FILE"))
        bAppend = true;
      else {
        vstr::warnp() << "VistaStreams::CreateStreamsFromProplist -- "
                      << "unknown parameter \"" << (*itParam) << "\" to stream \"FILE\""
                      << std::endl;
      }
    }
    pStream = vstr::GetStreamManager()->CreateNewLogFileStream(
        sFilename, sExtension, true, false, false, bAppend);
    if (pStream == NULL)
      return NULL;
    ...
    if (vecArguments.size() < 2) {
      vstr::warnp()
          << "VistaStreams::CreateStreamsFromProplist -- "
          << "Call to \"PREFIX\" with invalid arguments - specify at least stream and prefix"
          << std::endl;
      return NULL;
    }

    std::ostream* pSubStream = CreateStreamFromDescription(
        vecArguments[0], oConfig, false, liStreamsToCreate, sNodename, nResult, bAllowColorStreams);
    if (pSubStream == NULL) {
      if (nResult == 0) {
    ...
    for (std::size_t i = 2; i < vecArguments.size(); ++i) {
      if (VistaAspectsComparisonStuff::StringCaseInsensitiveEquals(
        ...
        vstr::warnp() << "VistaStreams::CreateStreamsFromProplist -- "
                      << "Call to \"PREFIX\" with invalid argument [" << vecArguments[i] << "]"
                      << std::endl;
      }
    }
    ...
    if (bAllowColorStreams) {
      ...
      if (vecArguments.size() > 0) {
        ...
        if (oTextColor == VistaColorOutstream::CONSOLE_COLOR(~0)) {
          vstr::warnp() << "VistaStreams::CreateStreamsFromProplist -- "
                        << "unknown color \"" << vecArguments[0] << "\" to stream \"COLOR\""
                        << std::endl;
          return NULL;
        }
      }
      if (vecArguments.size() > 1) {
        if (vecArguments.size() == 2 && VistaAspectsComparisonStuff::StringCaseInsensitiveEquals(
                                            vecArguments[1], "USE_ERROR_STREAM")) {
          ...
          if (oBackgroundColor == VistaColorOutstream::CONSOLE_COLOR(~0)) {
            vstr::warnp() << "VistaStreams::CreateStreamsFromProplist -- "
                          << "unknown color \"" << vecArguments[1] << "\" to stream \"COLOR\""
                          << std::endl;
            return NULL;
          }
          if (vecArguments.size() > 2 && VistaAspectsComparisonStuff::StringCaseInsensitiveEquals(
                                             vecArguments[2], "USE_ERROR_STREAM")) {
            bUseErrorStream = true;
          }
        }
      }
    ...
    if (vecArguments.empty()) {
      vstr::warnp() << "VistaStreams::CreateStreamsFromProplist -- "
                    << "Call to \"SPLIT\" missing arguments - specify at least one stream"
                    << std::endl;
      return NULL;
    }
    // to prevent memory leaks/multiple creations of substreams, we have to first wait till all our
    // strings can be created
    for (std::vector<std::string>::const_iterator itArg = vecArguments.begin();
         itArg != vecArguments.end(); ++itArg) {
      std::list<std::string>::const_iterator itWait =
          std::find(liStreamsToCreate.begin(), liStreamsToCreate.end(), (*itArg));
    ...
    for (std::vector<std::string>::const_iterator itArg = vecArguments.begin();
         itArg != vecArguments.end(); ++itArg) {
      std::ostream* pSubStream = CreateStreamFromDescription(
          (*itArg), oConfig, false, liStreamsToCreate, sNodename, nResult, bAllowColorStreams);
      if (pSubStream == NULL) {
        vstr::warnp() << "VistaStreams::CreateStreamsFromProplist -- "
                      << "\"SPLIT\"-command's substream \"" << (*itArg) << "\" could not be created"
                      << std::endl;
      } else {
...
#endif
    std::list<std::string>::const_iterator itWait =
        std::find(liStreamsToCreate.begin(), liStreamsToCreate.end(), sBuildTypeName);
    if (itWait != liStreamsToCreate.end()) {
      nResult = 0;
      return NULL;
    }
    pStream = CreateStreamFromDescription(
        sBuildTypeName, oConfig, true, liStreamsToCreate, sNodename, nResult, bAllowColorStreams);
    if (nResult != 1)
      return NULL;

  } else if (VistaAspectsComparisonStuff::StringCaseInsensitiveEquals(sCommand, "OUT")) {
    std::list<std::string>::const_iterator itWait =
        std::find(liStreamsToCreate.begin(), liStreamsToCreate.end(), "out");
    if (itWait != liStreamsToCreate.end()) {
      nResult = 0;
      return NULL;
    }
    ...
    std::list<std::string>::const_iterator itWait =
        std::find(liStreamsToCreate.begin(), liStreamsToCreate.end(), "warn");
    if (itWait != liStreamsToCreate.end()) {
      nResult = 0;
      return NULL;
    }
    ...
    std::list<std::string>::const_iterator itWait =
        std::find(liStreamsToCreate.begin(), liStreamsToCreate.end(), "err");
    if (itWait != liStreamsToCreate.end()) {
      nResult = 0;
      return NULL;
    }
    ...
    std::list<std::string>::const_iterator itWait =
        std::find(liStreamsToCreate.begin(), liStreamsToCreate.end(), "debug");
    if (itWait != liStreamsToCreate.end()) {
      nResult = 0;
      return NULL;
    }
    ...
    std::list<std::string>::const_iterator itWait =
        std::find(liStreamsToCreate.begin(), liStreamsToCreate.end(), sCommand);
    if (itWait != liStreamsToCreate.end()) {
      nResult = 0;
      return NULL;
    }
...
bool VistaStreams::CreateStreamsFromProplist(
    const VistaPropertyList& oConfig, const bool bAllowColorStreams) {
  return CreateStreamsFromProplist(oConfig, vstr::GetStreamManager()->GetNodeName());
}
bool VistaStreams::CreateStreamsFromProplist(
    const VistaPropertyList& oConfig, const std::string& sNodename, const bool bAllowColorStreams) {
  ...
  for (VistaPropertyList::const_iterator itEntry = oConfig.begin(); itEntry != oConfig.end();
       ++itEntry) {
    if (oCompare((*itEntry).first, "THREADSAFE"))
      continue;
    if (oCompare((*itEntry).first, "OUT") || oCompare((*itEntry).first, "WARN") ||
        oCompare((*itEntry).first, "ERR") || oCompare((*itEntry).first, "DEBUG")) {
