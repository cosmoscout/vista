...
std::ostream& operator<<(std::ostream& out, VistaWildcard& wildcard) {
  out << wildcard.m_cSymbol << " " << wildcard.m_nDigits << " " << wildcard.m_nMinRange << " "
      << wildcard.m_nMaxRange;
  return out;
}
...
std::string VistaFileDataSet::MatchPattern(
    std::string strPattern, int symbol, VistaWildcard pattern) {
...
bool VistaFileDataSet::MatchRecursivePattern(
    std::string strFile, std::list<VistaWildcard>::iterator itWildcard) {
  if (itWildcard == m_listWildcards.end()) {
    ...
    case DS_PATTERN: {
      ...
      if (m_strPattern != rhs.m_strPattern)
        ...
        if (m_listWildcards.size() != rhs.m_listWildcards.size())
          ...
          while ((itWildcard != m_listWildcards.end()) &&
                 (itWildcard2 != rhs.m_listWildcards.end()) && bEqual) {
