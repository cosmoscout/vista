...
void VistaOctree::GetNodeBoundingBox(
    const VistaOctreeNode& node, float& x, float& y, float& z, float& size) const {
  //	cout << "[VistaOctree::GetNodeBoundingBox] Processing node (" << node.m_xLocCode << ", " <<
  //node.m_yLocCode << ", " << node.m_zLocCode << ")" << std::endl;
  ...
  //	cout << "[VistaOctree::GetNodeBoundingBox] BBox of node: [(" << x << ", " << y << ", " << z
  //<< ") with size " << size << std::endl;
}

VistaOctreeNode* VistaOctree::Traverse(
    const int xLocCode, const int yLocCode, const int zLocCode, VistaOctreeNode* start) const {
  ...
  for (unsigned short nextLevel = node->m_level - 1; !(node->IsLeaf()); nextLevel--) {
    ...
    if (nextLevel > 1) {
      ...
      childIndex = ((xLocCode & childBranchBit) >> 1) + (yLocCode & childBranchBit) +
                   ((zLocCode & childBranchBit) << 1);
    } else // if(nextLevel==0)
    {
      childIndex = (xLocCode & childBranchBit) + ((yLocCode & childBranchBit) << 1) +
...
VistaOctreeNode* VistaOctree::LocateRegion(const float min[3], const float max[3]) {
  ...
  while ((!(diff[0] >> level[0])) && level[0] > 0)
    level[0]--;
  while ((!(diff[1] >> level[1])) && level[1] > level[0])
    level[1]--;
  while ((!(diff[2] >> level[2])) && level[2] > level[1])
    level[2]--;
  level[2]++;
...
VistaOctreeNode* VistaOctree::TraverseToLevel(const int xLocCode, const int yLocCode,
    const int zLocCode, const int level, VistaOctreeNode* start) const {
  VistaOctreeNode* node = start;
  if (!start)
    node = m_pRoot;
...
VistaOctreeNode* VistaOctree::GetCommonAncestor(
    VistaOctreeNode& node, int& nodeLevel, int diff) const {
